...
Lemma mk_mov_correct:
  ...
  mk_mov rd rs k = OK c ->
  exists rs2,
     exec_straight ge cp fn c rs1 m k rs2 m
  /\ rs2#rd = rs1#rs
...
Lemma mk_intconv_correct:
  ...
  (forall cp c rd rs r m,
   exec_instr ge cp c (mk rd rs) r m = Next (nextinstr (r#rd <- (sem r#rs))) m) ->
  exists rs2,
     exec_straight ge cp fn c rs1 m k rs2 m
  /\ rs2#rd = sem rs1#rs
...
Lemma mk_storebyte_correct:
  ...
  exists rs2,
     exec_straight ge cp fn c rs1 m1 k rs2 m2
...
Lemma mk_setcc_base_correct:
  ...
  exists rs2,
  exec_straight ge cp fn (mk_setcc_base cond rd k) rs1 m k rs2 m
...
Lemma mk_setcc_correct:
  forall cond rd k rs1 m,
  exists rs2,
  exec_straight ge cp fn (mk_setcc cond rd k) rs1 m k rs2 m
  /\ rs2#rd = Val.of_optbool(eval_extcond cond rs1)
  /\ forall r, data_preg r = true -> r <> RAX /\ r <> RCX -> r <> rd -> rs2#r = rs1#r.
Proof.
