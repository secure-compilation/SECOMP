...
Inductive estep: state -> trace -> state -> Prop :=
  ...
  | step_expr: forall f r k e m v ty,
      eval_simple_rvalue e (comp_of f) m r v ->
  ...
  | step_rvalof_volatile: forall f C l ty k e m b ofs t v,
      leftcontext RV RV C ->
      eval_simple_lvalue e (comp_of f) m l b ofs ->
      deref_loc ge ty (comp_of f) m b ofs t v ->
  ...
  | step_seqand_true: forall f C r1 r2 ty k e m v,
      leftcontext RV RV C ->
      eval_simple_rvalue e (comp_of f) m r1 v ->
  ...
  | step_assign: forall f C l r ty k e m b ofs v v' t m',
      ...
      assign_loc ge (typeof l) (comp_of f) m b ofs v' t m' ->
      ty = typeof l ->
  ...
  | step_assignop: forall f C op l r tyres ty k e m b ofs v1 v2 v3 v4 t1 t2 m' t,
      leftcontext RV RV C ->
      eval_simple_lvalue e (comp_of f) m l b ofs ->
      deref_loc ge (typeof l) (comp_of f) m b ofs t1 v1 ->
      eval_simple_rvalue e (comp_of f) m r v2 ->
      sem_binary_operation ge op v1 (typeof l) v2 (typeof r) m = Some v3 ->
      sem_cast v3 tyres (typeof l) m = Some v4 ->
      assign_loc ge (typeof l) (comp_of f) m b ofs v4 t2 m' ->
      ty = typeof l ->
      t = t1 ** t2 ->
  ...
  | step_assignop_stuck: forall f C op l r tyres ty k e m b ofs v1 v2 t,
      leftcontext RV RV C ->
      eval_simple_lvalue e (comp_of f) m l b ofs ->
      deref_loc ge (typeof l) (comp_of f) m b ofs t v1 ->
      eval_simple_rvalue e (comp_of f) m r v2 ->
      ...
      | Some v3 =>
          ...
          | None => True
          | Some v4 => forall t2 m', ~(assign_loc ge (typeof l) (comp_of f) m b ofs v4 t2 m')
          end
      end ->
      ty = typeof l ->
      estep (ExprState f (C (Eassignop op l r tyres ty)) k e m)
  ...
  | step_postincr: forall f C id l ty k e m b ofs v1 v2 v3 t1 t2 m' t,
      leftcontext RV RV C ->
      eval_simple_lvalue e (comp_of f) m l b ofs ->
      deref_loc ge ty (comp_of f) m b ofs t1 v1 ->
      sem_incrdecr ge id v1 ty m = Some v2 ->
      sem_cast v2 (incrdecr_type ty) ty m = Some v3 ->
      assign_loc ge ty (comp_of f) m b ofs v3 t2 m' ->
      ty = typeof l ->
      t = t1 ** t2 ->
  ...
  | step_postincr_stuck: forall f C id l ty k e m b ofs v1 t,
      leftcontext RV RV C ->
      eval_simple_lvalue e (comp_of f) m l b ofs ->
      deref_loc ge ty (comp_of f) m b ofs t v1 ->
      match sem_incrdecr ge id v1 ty m with
      | None => True
      | Some v2 =>
          match sem_cast v2 (incrdecr_type ty) ty m with
          | None => True
          | Some v3 => forall t2 m', ~(assign_loc ge (typeof l) (comp_of f) m b ofs v3 t2 m')
          end
      end ->
      ty = typeof l ->
      estep (ExprState f (C (Epostincr id l ty)) k e m)
  ...
  | step_paren: forall f C r tycast ty k e m v1 v,
      leftcontext RV RV C ->
      eval_simple_rvalue e (comp_of f) m r v1 ->
  ...
  | step_call: forall f C rf rargs ty k e m targs tres cconv vf vargs fd,
      ...
      eval_simple_list e (comp_of f) m rargs targs vargs ->
...
Definition safe (s: Csem.state) : Prop :=
  forall s', star (Csem.step pol) ge s E0 s' ->
  (exists r, final_state s' r) \/ (exists t, exists s'', (Csem.step pol) ge s' t s'').

Lemma safe_steps:
  forall s s',
  safe s -> star (Csem.step pol) ge s E0 s' -> safe s'.
Proof.
  intros; red; intros.
...
Lemma star_safe:
  forall s1 s2 t s3,
  safe s1 -> star (Csem.step pol) ge s1 E0 s2 -> (safe s2 -> star (Csem.step pol) ge s2 t s3) ->
  star (Csem.step pol) ge s1 t s3.
Proof.
...
Lemma plus_safe:
  forall s1 s2 t s3,
  safe s1 -> star (Csem.step pol) ge s1 E0 s2 -> (safe s2 -> plus (Csem.step pol) ge s2 t s3) ->
  plus (Csem.step pol) ge s1 t s3.
Proof.
...
Definition invert_expr_prop (cp: compartment) (a: expr) (m: mem) : Prop :=
  ...
  | Evalof (Eloc b ofs ty') ty =>
      ty' = ty /\ exists t, exists v, deref_loc ge ty cp m b ofs t v
  ...
  | Eassign (Eloc b ofs ty1) (Eval v2 ty2) ty =>
      ...
      ty = ty1 /\ sem_cast v2 ty2 ty1 m = Some v /\ assign_loc ge ty1 cp m b ofs v t m'
  | Eassignop op (Eloc b ofs ty1) (Eval v2 ty2) tyres ty =>
      exists t, exists v1,
      ty = ty1
      /\ deref_loc ge ty1 cp m b ofs t v1
  | Epostincr id (Eloc b ofs ty1) ty =>
      exists t, exists v1,
      ty = ty1
      /\ deref_loc ge ty cp m b ofs t v1
...
Lemma eval_simple_steps:
   (forall a v, eval_simple_rvalue e (comp_of f) m a v ->
    ...
    star (Csem.step pol) ge (ExprState f (C a) k e m)
                   E0 (ExprState f (C (Eval v (typeof a))) k e m))
/\ (forall a b ofs, eval_simple_lvalue e (comp_of f) m a b ofs ->
...
Corollary eval_simple_rvalue_safe:
  forall C a v,
  eval_simple_rvalue e (comp_of f) m a v ->
...
Corollary eval_simple_lvalue_safe:
  forall C a b ofs,
  eval_simple_lvalue e (comp_of f) m a b ofs ->
...
Lemma simple_can_eval:
  ...
  match from with
  | LV => exists b, exists ofs, eval_simple_lvalue e (comp_of f) m a b ofs
  | RV => exists v, eval_simple_rvalue e (comp_of f) m a v
  end.
Proof.
Ltac StepL REC C' a :=
...
Lemma simple_can_eval_rval:
  ...
  exists v, eval_simple_rvalue e (comp_of f) m r v
...
Lemma simple_can_eval_lval:
  ...
  exists b, exists ofs, eval_simple_lvalue e (comp_of f) m l b ofs
...
Lemma eval_simple_list_implies:
  forall rl tyl vl,
  eval_simple_list e (comp_of f) m rl tyl vl ->
...
Lemma eval_simple_list_steps:
  ...
  star (Csem.step pol) ge (ExprState f (C rl) k e m)
...
Lemma estep_simulation:
  forall S t S',
  estep S t S' -> plus (Csem.step pol) ge S t S'.
Proof.
  intros. inv H.
(* simple *)
...
+ (* assignop *)
  ...
  destruct (classic (exists t2, exists m', assign_loc ge (typeof b1) (comp_of f) m b ofs v4 t2 m')).
...
+ (* postincr *)
  ...
  destruct (classic (exists t2, exists m', assign_loc ge ty (comp_of f) m b1 ofs v3 t2 m')).
...
Theorem step_simulation:
  forall S1 t S2,
  step S1 t S2 -> plus (Csem.step pol) ge S1 t S2.
Proof.
  intros. inv H.
...
Definition semantics (p: program) :=
  ...
  Semantics_gen (step pol) (initial_state p) final_state ge ge.
...
Remark deref_loc_receptive:
  forall ge ty cp m b ofs ev1 t1 v ev2,
  deref_loc ge ty cp m b ofs (ev1 :: t1) v ->
  match_traces ge (ev1 :: nil) (ev2 :: nil) ->
  t1 = nil /\ exists v', deref_loc ge ty cp m b ofs (ev2 :: nil) v'.
Proof.
  intros.
...
Remark assign_loc_trace:
  forall ge ty cp m b ofs t v m',
  assign_loc ge ty cp m b ofs v t m' ->
...
Remark assign_loc_receptive:
  forall ge ty cp m b ofs ev1 t1 v m' ev2,
  assign_loc ge ty cp m b ofs v (ev1 :: t1) m' ->
  match_traces ge (ev1 :: nil) (ev2 :: nil) ->
  ev1 :: t1 = ev2 :: nil.
Proof.
  intros.
...
(* receptiveness *)
  ...
  destruct (classic (exists t2', exists m'', assign_loc ge (typeof l) (comp_of f) m b ofs v4' t2' m'')).
  ...
  destruct (classic (exists t2', exists m'', assign_loc ge (typeof l) (comp_of f) m b ofs v3' t2' m'')).
  destruct H1 as [t2' [m'' P]].
...
Theorem strategy_simulation:
  forall p, backward_simulation (Csem.semantics pol p) (semantics p).
Proof.
  intros.
...
(** [eval_expression ge c e m1 a t m2 a'] describes the evaluation of the
...
Inductive eval_expression: compartment -> env -> mem -> expr -> trace -> mem -> val -> Prop :=
  | eval_expression_intro: forall c e m a t m' a' v,
      eval_expr c e m RV a t m' a' -> eval_simple_rvalue ge e c m' a' v ->
      eval_expression c e m a t m' v

with eval_expr: compartment -> env -> mem -> kind -> expr -> trace -> mem -> expr -> Prop :=
  | eval_val: forall c e m v ty,
      eval_expr c e m RV (Eval v ty) E0 m (Eval v ty)
  | eval_var: forall c e m x ty,
      eval_expr c e m LV (Evar x ty) E0 m (Evar x ty)
  | eval_field: forall c e m a t m' a' f ty,
      eval_expr c e m RV a t m' a' ->
      eval_expr c e m LV (Efield a f ty) t m' (Efield a' f ty)
  | eval_valof: forall c e m a t m' a' ty,
      type_is_volatile (typeof a) = false ->
      eval_expr c e m LV a t m' a' ->
      eval_expr c e m RV (Evalof a ty) t m' (Evalof a' ty)
  | eval_valof_volatile: forall c e m a t1 m' a' ty b ofs t2 v,
      type_is_volatile (typeof a) = true ->
      eval_expr c e m LV a t1 m' a' ->
      eval_simple_lvalue ge e c m' a' b ofs ->
      deref_loc ge (typeof a) c m' b ofs t2 v ->
      ty = typeof a ->
      eval_expr c e m RV (Evalof a ty) (t1 ** t2) m' (Eval v ty)
  | eval_deref: forall c e m a t m' a' ty,
      eval_expr c e m RV a t m' a' ->
      eval_expr c e m LV (Ederef a ty) t m' (Ederef a' ty)
  | eval_addrof: forall c e m a t m' a' ty,
      eval_expr c e m LV a t m' a' ->
      eval_expr c e m RV (Eaddrof a ty) t m' (Eaddrof a' ty)
  | eval_unop: forall c e m a t m' a' op ty,
      eval_expr c e m RV a t m' a' ->
      eval_expr c e m RV (Eunop op a ty) t m' (Eunop op a' ty)
  | eval_binop: forall c e m a1 t1 m' a1' a2 t2 m'' a2' op ty,
      eval_expr c e m RV a1 t1 m' a1' -> eval_expr c e m' RV a2 t2 m'' a2' ->
      eval_expr c e m RV (Ebinop op a1 a2 ty) (t1 ** t2) m'' (Ebinop op a1' a2' ty)
  | eval_cast: forall c e m a t m' a' ty,
      eval_expr c e m RV a t m' a' ->
      eval_expr c e m RV (Ecast a ty) t m' (Ecast a' ty)
  | eval_seqand_true: forall c e m a1 a2 ty t1 m' a1' v1 t2 m'' a2' v2 v,
      eval_expr c e m RV a1 t1 m' a1' -> eval_simple_rvalue ge e c m' a1' v1 ->
      bool_val v1 (typeof a1) m' = Some true ->
      eval_expr c e m' RV a2 t2 m'' a2' -> eval_simple_rvalue ge e c m'' a2' v2 ->
      sem_cast v2 (typeof a2) type_bool m'' = Some v ->
      eval_expr c e m RV (Eseqand a1 a2 ty) (t1**t2) m'' (Eval v ty)
  | eval_seqand_false: forall c e m a1 a2 ty t1 m' a1' v1,
      eval_expr c e m RV a1 t1 m' a1' -> eval_simple_rvalue ge e c m' a1' v1 ->
      bool_val v1 (typeof a1) m' = Some false ->
      eval_expr c e m RV (Eseqand a1 a2 ty) t1 m' (Eval (Vint Int.zero) ty)
  | eval_seqor_false: forall c e m a1 a2 ty t1 m' a1' v1 t2 m'' a2' v2 v,
      eval_expr c e m RV a1 t1 m' a1' -> eval_simple_rvalue ge e c m' a1' v1 ->
      bool_val v1 (typeof a1) m' = Some false ->
      eval_expr c e m' RV a2 t2 m'' a2' -> eval_simple_rvalue ge e c m'' a2' v2 ->
      sem_cast v2 (typeof a2) type_bool m'' = Some v ->
      eval_expr c e m RV (Eseqor a1 a2 ty) (t1**t2) m'' (Eval v ty)
  | eval_seqor_true: forall c e m a1 a2 ty t1 m' a1' v1,
      eval_expr c e m RV a1 t1 m' a1' -> eval_simple_rvalue ge e c m' a1' v1 ->
      bool_val v1 (typeof a1) m' = Some true ->
      eval_expr c e m RV (Eseqor a1 a2 ty) t1 m' (Eval (Vint Int.one) ty)
  | eval_condition: forall c e m a1 a2 a3 ty t1 m' a1' v1 t2 m'' a' v' b v,
      eval_expr c e m RV a1 t1 m' a1' -> eval_simple_rvalue ge e c m' a1' v1 ->
      bool_val v1 (typeof a1) m' = Some b ->
      eval_expr c e m' RV (if b then a2 else a3) t2 m'' a' -> eval_simple_rvalue ge e c m'' a' v' ->
      sem_cast v' (typeof (if b then a2 else a3)) ty m'' = Some v ->
      eval_expr c e m RV (Econdition a1 a2 a3 ty) (t1**t2) m'' (Eval v ty)
  | eval_sizeof: forall c e m ty' ty,
      eval_expr c e m RV (Esizeof ty' ty) E0 m (Esizeof ty' ty)
  | eval_alignof: forall c e m ty' ty,
      eval_expr c e m RV (Ealignof ty' ty) E0 m (Ealignof ty' ty)
  | eval_assign: forall c e m l r ty t1 m1 l' t2 m2 r' b ofs v v' t3 m3,
      eval_expr c e m LV l t1 m1 l' -> eval_expr c e m1 RV r t2 m2 r' ->
      eval_simple_lvalue ge e c m2 l' b ofs ->
      eval_simple_rvalue ge e c m2 r' v ->
      sem_cast v (typeof r) (typeof l) m2 = Some v' ->
      assign_loc ge (typeof l) c m2 b ofs v' t3 m3 ->
      ty = typeof l ->
      eval_expr c e m RV (Eassign l r ty) (t1**t2**t3) m3 (Eval v' ty)
  | eval_assignop: forall c e m op l r tyres ty t1 m1 l' t2 m2 r' b ofs
                          v1 v2 v3 v4 t3 t4 m3,
      eval_expr c e m LV l t1 m1 l' -> eval_expr c e m1 RV r t2 m2 r' ->
      eval_simple_lvalue ge e c m2 l' b ofs ->
      deref_loc ge (typeof l) c m2 b ofs t3 v1 ->
      ...
      assign_loc ge (typeof l) c m2 b ofs v4 t4 m3 ->
      ty = typeof l ->
      eval_expr c e m RV (Eassignop op l r tyres ty) (t1**t2**t3**t4) m3 (Eval v4 ty)
  | eval_postincr: forall c e m id l ty t1 m1 l' b ofs v1 v2 v3 m2 t2 t3,
      eval_expr c e m LV l t1 m1 l' ->
      eval_simple_lvalue ge e c m1 l' b ofs ->
      deref_loc ge ty c m1 b ofs t2 v1 ->
      sem_incrdecr ge id v1 ty m1 = Some v2 ->
      sem_cast v2 (incrdecr_type ty) ty m1 = Some v3 ->
      assign_loc ge ty c m1 b ofs v3 t3 m2 ->
      ty = typeof l ->
      eval_expr c e m RV (Epostincr id l ty) (t1**t2**t3) m2 (Eval v1 ty)
  | eval_comma: forall c e m r1 r2 ty t1 m1 r1' v1 t2 m2 r2',
      eval_expr c e m RV r1 t1 m1 r1' ->
      eval_simple_rvalue ge e c m1 r1' v1 ->
      eval_expr c e m1 RV r2 t2 m2 r2' ->
      ty = typeof r2 ->
...
with eval_exprlist: compartment -> env -> mem -> exprlist -> trace -> mem -> exprlist -> Prop :=
  | eval_nil: forall c e m,
      eval_exprlist c e m Enil E0 m Enil
  | eval_cons: forall c e m a1 al t1 m1 a1' t2 m2 al',
      eval_expr c e m RV a1 t1 m1 a1' -> eval_exprlist c e m1 al t2 m2 al' ->
      eval_exprlist c e m (Econs a1 al) (t1**t2) m2 (Econs a1' al')

(** [exec_stmt ge c e m1 s t m2 out] describes the execution of
...
with exec_stmt: compartment -> env -> mem -> statement -> trace -> mem -> outcome -> Prop :=
  | exec_Sskip:   forall c e m,
      exec_stmt c e m Sskip
               E0 m Out_normal
  | exec_Sdo:     forall c e m a t m' v,
      eval_expression c e m a t m' v ->
      exec_stmt c e m (Sdo a)
                t m' Out_normal
  | exec_Sseq_1:   forall c e m s1 s2 t1 m1 t2 m2 out,
      exec_stmt c e m s1 t1 m1 Out_normal ->
      exec_stmt c e m1 s2 t2 m2 out ->
      exec_stmt c e m (Ssequence s1 s2)
                (t1 ** t2) m2 out
  | exec_Sseq_2:   forall c e m s1 s2 t1 m1 out,
      exec_stmt c e m s1 t1 m1 out ->
      out <> Out_normal ->
      exec_stmt c e m (Ssequence s1 s2)
                t1 m1 out
  | exec_Sifthenelse: forall c e m a s1 s2 t1 m1 v1 t2 m2 b out,
      eval_expression c e m a t1 m1 v1 ->
      bool_val v1 (typeof a) m1 = Some b ->
      exec_stmt c e m1 (if b then s1 else s2) t2 m2 out ->
      exec_stmt c e m (Sifthenelse a s1 s2)
                (t1**t2) m2 out
  | exec_Sreturn_none:   forall c e m,
      exec_stmt c e m (Sreturn None)
               E0 m (Out_return None)
  | exec_Sreturn_some: forall c e m a t m' v,
      eval_expression c e m a t m' v ->
      exec_stmt c e m (Sreturn (Some a))
                t m' (Out_return (Some(v, typeof a)))
  | exec_Sbreak:   forall c e m,
      exec_stmt c e m Sbreak
               E0 m Out_break
  | exec_Scontinue:   forall c e m,
      exec_stmt c e m Scontinue
               E0 m Out_continue
  | exec_Swhile_false: forall c e m a s t m' v,
      eval_expression c e m a t m' v ->
      bool_val v (typeof a) m' = Some false ->
      exec_stmt c e m (Swhile a s)
                t m' Out_normal
  | exec_Swhile_stop: forall c e m a s t1 m1 v t2 m2 out' out,
      eval_expression c e m a t1 m1 v ->
      bool_val v (typeof a) m1 = Some true ->
      exec_stmt c e m1 s t2 m2 out' ->
      out_break_or_return out' out ->
      exec_stmt c e m (Swhile a s)
                (t1**t2) m2 out
  | exec_Swhile_loop: forall c e m a s t1 m1 v t2 m2 out1 t3 m3 out,
      eval_expression c e m a t1 m1 v ->
      bool_val v (typeof a) m1 = Some true ->
      exec_stmt c e m1 s t2 m2 out1 ->
      out_normal_or_continue out1 ->
      exec_stmt c e m2 (Swhile a s) t3 m3 out ->
      exec_stmt c e m (Swhile a s)
                (t1 ** t2 ** t3) m3 out
  | exec_Sdowhile_false: forall c e m s a t1 m1 out1 t2 m2 v,
      exec_stmt c e m s t1 m1 out1 ->
      out_normal_or_continue out1 ->
      eval_expression c e m1 a t2 m2 v ->
      bool_val v (typeof a) m2 = Some false ->
      exec_stmt c e m (Sdowhile a s)
                (t1 ** t2) m2 Out_normal
  | exec_Sdowhile_stop: forall c e m s a t m1 out1 out,
      exec_stmt c e m s t m1 out1 ->
      out_break_or_return out1 out ->
      exec_stmt c e m (Sdowhile a s)
                t m1 out
  | exec_Sdowhile_loop: forall c e m s a t1 m1 out1 t2 m2 v t3 m3 out,
      exec_stmt c e m s t1 m1 out1 ->
      out_normal_or_continue out1 ->
      eval_expression c e m1 a t2 m2 v ->
      bool_val v (typeof a) m2 = Some true ->
      exec_stmt c e m2 (Sdowhile a s) t3 m3 out ->
      exec_stmt c e m (Sdowhile a s)
                (t1 ** t2 ** t3) m3 out
  | exec_Sfor_start: forall c e m s a1 a2 a3 out m1 m2 t1 t2,
      exec_stmt c e m a1 t1 m1 Out_normal ->
      exec_stmt c e m1 (Sfor Sskip a2 a3 s) t2 m2 out ->
      exec_stmt c e m (Sfor a1 a2 a3 s)
                (t1 ** t2) m2 out
  | exec_Sfor_false: forall c e m s a2 a3 t m' v,
      eval_expression c e m a2 t m' v ->
      bool_val v (typeof a2) m' = Some false ->
      exec_stmt c e m (Sfor Sskip a2 a3 s)
                t m' Out_normal
  | exec_Sfor_stop: forall c e m s a2 a3 t1 m1 v t2 m2 out1 out,
      eval_expression c e m a2 t1 m1 v ->
      bool_val v (typeof a2) m1 = Some true ->
      exec_stmt c e m1 s t2 m2 out1 ->
      out_break_or_return out1 out ->
      exec_stmt c e m (Sfor Sskip a2 a3 s)
                (t1 ** t2) m2 out
  | exec_Sfor_loop: forall c e m s a2 a3 t1 m1 v t2 m2 out1 t3 m3 t4 m4 out,
      eval_expression c e m a2 t1 m1 v ->
      bool_val v (typeof a2) m1 = Some true ->
      exec_stmt c e m1 s t2 m2 out1 ->
      out_normal_or_continue out1 ->
      exec_stmt c e m2 a3 t3 m3 Out_normal ->
      exec_stmt c e m3 (Sfor Sskip a2 a3 s) t4 m4 out ->
      exec_stmt c e m (Sfor Sskip a2 a3 s)
                (t1 ** t2 ** t3 ** t4) m4 out
  | exec_Sswitch:   forall c e m a sl t1 m1 v n t2 m2 out,
      eval_expression c e m a t1 m1 v ->
      sem_switch_arg v (typeof a) = Some n ->
      exec_stmt c e m1 (seq_of_labeled_statement (select_switch n sl)) t2 m2 out ->
...
(** [evalinf_expr ge c e m1 K a T] denotes the fact that expression [a]
  diverges in initial state [m1].  [T] is the trace of input/output
  events performed during this evaluation.  *)

CoInductive evalinf_expr: compartment -> env -> mem -> kind -> expr -> traceinf -> Prop :=
  | evalinf_field: forall c e m a t f ty,
      evalinf_expr c e m RV a t ->
      evalinf_expr c e m LV (Efield a f ty) t
  | evalinf_valof: forall c e m a t ty,
      evalinf_expr c e m LV a t ->
      evalinf_expr c e m RV (Evalof a ty) t
  | evalinf_deref: forall c e m a t ty,
      evalinf_expr c e m RV a t ->
      evalinf_expr c e m LV (Ederef a ty) t
  | evalinf_addrof: forall c e m a t ty,
      evalinf_expr c e m LV a t ->
      evalinf_expr c e m RV (Eaddrof a ty) t
  | evalinf_unop: forall c e m a t op ty,
      evalinf_expr c e m RV a t ->
      evalinf_expr c e m RV (Eunop op a ty) t
  | evalinf_binop_left: forall c e m a1 t1 a2 op ty,
      evalinf_expr c e m RV a1 t1 ->
      evalinf_expr c e m RV (Ebinop op a1 a2 ty) t1
  | evalinf_binop_right: forall c e m a1 t1 m' a1' a2 t2 op ty,
      eval_expr c e m RV a1 t1 m' a1' -> evalinf_expr c e m' RV a2 t2 ->
      evalinf_expr c e m RV (Ebinop op a1 a2 ty) (t1 *** t2)
  | evalinf_cast: forall c e m a t ty,
      evalinf_expr c e m RV a t ->
      evalinf_expr c e m RV (Ecast a ty) t
  | evalinf_seqand: forall c e m a1 a2 ty t1,
      evalinf_expr c e m RV a1 t1 ->
      evalinf_expr c e m RV (Eseqand a1 a2 ty) t1
  | evalinf_seqand_2: forall c e m a1 a2 ty t1 m' a1' v1 t2,
      eval_expr c e m RV a1 t1 m' a1' -> eval_simple_rvalue ge e c m' a1' v1 ->
      bool_val v1 (typeof a1) m' = Some true ->
      evalinf_expr c e m' RV a2 t2 ->
      evalinf_expr c e m RV (Eseqand a1 a2 ty) (t1***t2)
  | evalinf_seqor: forall c e m a1 a2 ty t1,
      evalinf_expr c e m RV a1 t1 ->
      evalinf_expr c e m RV (Eseqor a1 a2 ty) t1
  | evalinf_seqor_2: forall c e m a1 a2 ty t1 m' a1' v1 t2,
      eval_expr c e m RV a1 t1 m' a1' -> eval_simple_rvalue ge e c m' a1' v1 ->
      bool_val v1 (typeof a1) m' = Some false ->
      evalinf_expr c e m' RV a2 t2 ->
      evalinf_expr c e m RV (Eseqor a1 a2 ty) (t1***t2)
  | evalinf_condition: forall c e m a1 a2 a3 ty t1,
      evalinf_expr c e m RV a1 t1 ->
      evalinf_expr c e m RV (Econdition a1 a2 a3 ty) t1
  | evalinf_condition_2: forall c e m a1 a2 a3 ty t1 m' a1' v1 t2 b,
      eval_expr c e m RV a1 t1 m' a1' -> eval_simple_rvalue ge e c m' a1' v1 ->
      bool_val v1 (typeof a1) m' = Some b ->
      evalinf_expr c e m' RV (if b then a2 else a3) t2 ->
      evalinf_expr c e m RV (Econdition a1 a2 a3 ty) (t1***t2)
  | evalinf_assign_left: forall c e m a1 t1 a2 ty,
      evalinf_expr c e m LV a1 t1 ->
      evalinf_expr c e m RV (Eassign a1 a2 ty) t1
  | evalinf_assign_right: forall c e m a1 t1 m' a1' a2 t2 ty,
      eval_expr c e m LV a1 t1 m' a1' -> evalinf_expr c e m' RV a2 t2 ->
      evalinf_expr c e m RV (Eassign a1 a2 ty) (t1 *** t2)
  | evalinf_assignop_left: forall c e m a1 t1 a2 op tyres ty,
      evalinf_expr c e m LV a1 t1 ->
      evalinf_expr c e m RV (Eassignop op a1 a2 tyres ty) t1
  | evalinf_assignop_right: forall c e m a1 t1 m' a1' a2 t2 op tyres ty,
      eval_expr c e m LV a1 t1 m' a1' -> evalinf_expr c e m' RV a2 t2 ->
      evalinf_expr c e m RV (Eassignop op a1 a2 tyres ty) (t1 *** t2)
  | evalinf_postincr: forall c e m a t id ty,
      evalinf_expr c e m LV a t ->
      evalinf_expr c e m RV (Epostincr id a ty) t
  | evalinf_comma_left: forall c e m a1 t1 a2 ty,
      evalinf_expr c e m RV a1 t1 ->
      evalinf_expr c e m RV (Ecomma a1 a2 ty) t1
  | evalinf_comma_right: forall c e m a1 t1 m1 a1' v1 a2 t2 ty,
      eval_expr c e m RV a1 t1 m1 a1' -> eval_simple_rvalue ge e c m1 a1' v1 ->
      ty = typeof a2 ->
      evalinf_expr c e m1 RV a2 t2 ->
      evalinf_expr c e m RV (Ecomma a1 a2 ty) (t1 *** t2)
  | evalinf_call_left: forall c e m a1 t1 a2 ty,
      evalinf_expr c e m RV a1 t1 ->
      evalinf_expr c e m RV (Ecall a1 a2 ty) t1
  | evalinf_call_right: forall c e m a1 t1 m1 a1' a2 t2 ty,
      eval_expr c e m RV a1 t1 m1 a1' ->
...
with evalinf_exprlist: compartment -> env -> mem -> exprlist -> traceinf -> Prop :=
  | evalinf_cons_left: forall c e m a1 al t1,
      evalinf_expr c e m RV a1 t1 ->
      evalinf_exprlist c e m (Econs a1 al) t1
  | evalinf_cons_right: forall c e m a1 al t1 m1 a1' t2,
      eval_expr c e m RV a1 t1 m1 a1' -> evalinf_exprlist c e m1 al t2 ->
      evalinf_exprlist c e m (Econs a1 al) (t1***t2)

(** [execinf_stmt ge c e m1 s t] describes the diverging execution of
  the statement [s].  *)

with execinf_stmt: compartment -> env -> mem -> statement -> traceinf -> Prop :=
  | execinf_Sdo:     forall c e m a t,
      evalinf_expr c e m RV a t ->
      execinf_stmt c e m (Sdo a) t
  | execinf_Sseq_1:   forall c e m s1 s2 t1,
      execinf_stmt c e m s1 t1 ->
      execinf_stmt c e m (Ssequence s1 s2) t1
  | execinf_Sseq_2:   forall c e m s1 s2 t1 m1 t2,
      exec_stmt c e m s1 t1 m1 Out_normal ->
      execinf_stmt c e m1 s2 t2 ->
      execinf_stmt c e m (Ssequence s1 s2) (t1***t2)
  | execinf_Sifthenelse_test: forall c e m a s1 s2 t1,
      evalinf_expr c e m RV a t1 ->
      execinf_stmt c e m (Sifthenelse a s1 s2) t1
  | execinf_Sifthenelse: forall c e m a s1 s2 t1 m1 v1 t2 b,
      eval_expression c e m a t1 m1 v1 ->
      bool_val v1 (typeof a) m1 = Some b ->
      execinf_stmt c e m1 (if b then s1 else s2) t2 ->
      execinf_stmt c e m (Sifthenelse a s1 s2) (t1***t2)
  | execinf_Sreturn_some: forall c e m a t,
      evalinf_expr c e m RV a t ->
      execinf_stmt c e m (Sreturn (Some a)) t
  | execinf_Swhile_test: forall c e m a s t1,
      evalinf_expr c e m RV a t1 ->
      execinf_stmt c e m (Swhile a s) t1
  | execinf_Swhile_body: forall c e m a s t1 m1 v t2,
      eval_expression c e m a t1 m1 v ->
      bool_val v (typeof a) m1 = Some true ->
      execinf_stmt c e m1 s t2 ->
      execinf_stmt c e m (Swhile a s) (t1***t2)
  | execinf_Swhile_loop: forall c e m a s t1 m1 v t2 m2 out1 t3,
      eval_expression c e m a t1 m1 v ->
      bool_val v (typeof a) m1 = Some true ->
      exec_stmt c e m1 s t2 m2 out1 ->
      out_normal_or_continue out1 ->
      execinf_stmt c e m2 (Swhile a s) t3 ->
      execinf_stmt c e m (Swhile a s) (t1***t2***t3)
  | execinf_Sdowhile_body: forall c e m s a t1,
      execinf_stmt c e m s t1 ->
      execinf_stmt c e m (Sdowhile a s) t1
  | execinf_Sdowhile_test: forall c e m s a t1 m1 out1 t2,
      exec_stmt c e m s t1 m1 out1 ->
      out_normal_or_continue out1 ->
      evalinf_expr c e m1 RV a t2 ->
      execinf_stmt c e m (Sdowhile a s) (t1***t2)
  | execinf_Sdowhile_loop: forall c e m s a t1 m1 out1 t2 m2 v t3,
      exec_stmt c e m s t1 m1 out1 ->
      out_normal_or_continue out1 ->
      eval_expression c e m1 a t2 m2 v ->
      bool_val v (typeof a) m2 = Some true ->
      execinf_stmt c e m2 (Sdowhile a s) t3 ->
      execinf_stmt c e m (Sdowhile a s) (t1***t2***t3)
  | execinf_Sfor_start_1: forall c e m s a1 a2 a3 t1,
      execinf_stmt c e m a1 t1 ->
      execinf_stmt c e m (Sfor a1 a2 a3 s) t1
  | execinf_Sfor_start_2: forall c e m s a1 a2 a3 m1 t1 t2,
      exec_stmt c e m a1 t1 m1 Out_normal -> a1 <> Sskip ->
      execinf_stmt c e m1 (Sfor Sskip a2 a3 s) t2 ->
      execinf_stmt c e m (Sfor a1 a2 a3 s) (t1***t2)
  | execinf_Sfor_test: forall c e m s a2 a3 t,
      evalinf_expr c e m RV a2 t ->
      execinf_stmt c e m (Sfor Sskip a2 a3 s) t
  | execinf_Sfor_body: forall c e m s a2 a3 t1 m1 v t2,
      eval_expression c e m a2 t1 m1 v ->
      bool_val v (typeof a2) m1 = Some true ->
      execinf_stmt c e m1 s t2 ->
      execinf_stmt c e m (Sfor Sskip a2 a3 s) (t1***t2)
  | execinf_Sfor_next: forall c e m s a2 a3 t1 m1 v t2 m2 out1 t3,
      eval_expression c e m a2 t1 m1 v ->
      bool_val v (typeof a2) m1 = Some true ->
      exec_stmt c e m1 s t2 m2 out1 ->
      out_normal_or_continue out1 ->
      execinf_stmt c e m2 a3 t3 ->
      execinf_stmt c e m (Sfor Sskip a2 a3 s) (t1***t2***t3)
  | execinf_Sfor_loop: forall c e m s a2 a3 t1 m1 v t2 m2 out1 t3 m3 t4,
      eval_expression c e m a2 t1 m1 v ->
      bool_val v (typeof a2) m1 = Some true ->
      exec_stmt c e m1 s t2 m2 out1 ->
      out_normal_or_continue out1 ->
      exec_stmt c e m2 a3 t3 m3 Out_normal ->
      execinf_stmt c e m3 (Sfor Sskip a2 a3 s) t4 ->
      execinf_stmt c e m (Sfor Sskip a2 a3 s) (t1***t2***t3***t4)
  | execinf_Sswitch_expr:   forall c e m a sl t1,
      evalinf_expr c e m RV a t1 ->
      execinf_stmt c e m (Sswitch a sl) t1
  | execinf_Sswitch_body:   forall c e m a sl t1 m1 v n t2,
      eval_expression c e m a t1 m1 v ->
      sem_switch_arg v (typeof a) = Some n ->
      execinf_stmt c e m1 (seq_of_labeled_statement (select_switch n sl)) t2 ->
      execinf_stmt c e m (Sswitch a sl) (t1***t2)
...
/\(forall c e m al t m' al',
   ...
   forall COMP: c = f.(fn_comp),
   simplelist al' = true /\
   star (step pol) ge (ExprState f (C (Ecall a1 (exprlist_app al2 al) ty)) k e m)
...
/\(forall c e m s t m' out,
   ...
   forall COMP: c = f.(fn_comp),
   exists S,
   star (step pol) ge (State f s k e m) t S /\ outcome_state_match e m' f k out S)
...
Lemma evalinf_funcall_steps:
  ...
  evalinf_funcall cp m fd args t ->
  forever_N (step pol) lt ge O (Callstate fd args k m) t.
Proof.
  cofix COF.
  ...
  assert (COS:
    ...
    forall COMP: c = f.(fn_comp),
    forever_N (step pol) lt ge O (State f s k e m) t).
  cofix COS.
  ...
  assert (COE:
    ...
    forall COMP: c = f.(fn_comp),
    leftcontext K RV C ->
    forever_N (step pol) lt ge (esize a) (ExprState f (C a) k e m) t).
  cofix COE.
  ...
  assert (COEL:
    ...
    forever_N (step pol) lt ge (esizelist a)
...
(* seq 2 *)
  destruct (exec_stmt_to_steps _ _ _ _ _ _ _ H0 f (Kseq s2 k)) as [S1 [A1 B1]]; auto. inv B1.
...
(* while body *)
  ...
  eapply COS; eauto. traceEq.
(* while loop *)
  destruct (exec_stmt_to_steps _ _ _ _ _ _ _ H2 f (Kwhile2 a s0 k)) as [S1 [A1 B1]]; auto.
  eapply forever_N_plus.
  eapply plus_left. right; constructor.
...
(* dowhile body *)
  ...
  eapply COS; eauto. traceEq.
(* dowhile test *)
  destruct (exec_stmt_to_steps _ _ _ _ _ _ _ H0 f (Kdowhile1 a s0 k)) as [S1 [A1 B1]]; auto.
  eapply forever_N_plus.
  eapply plus_left. right; constructor.
  eapply star_trans. eexact A1.
...
(* for start 1 *)
  ...
  eapply COS; eauto. traceEq.
(* for start 2 *)
  destruct (exec_stmt_to_steps _ _ _ _ _ _ _ H0 f (Kseq (Sfor Sskip a2 a3 s0) k)) as [S1 [A1 B1]]; auto. inv B1.
  eapply forever_N_plus.
...
(* for body *)
  ...
  eapply COS; eauto. traceEq.
(* for next *)
  destruct (exec_stmt_to_steps _ _ _ _ _ _ _ H2 f (Kfor3 a2 a3 s0 k)) as [S1 [A1 B1]]; auto.
  eapply forever_N_plus.
  eapply plus_left. right; apply step_for.
  eapply star_trans. eapply eval_expression_to_steps; eauto.
...
(* for loop *)
  destruct (exec_stmt_to_steps _ _ _ _ _ _ _ H2 f (Kfor3 a2 a3 s0 k)) as [S1 [A1 B1]]; auto.
  destruct (exec_stmt_to_steps _ _ _ _ _ _ _ H4 f (Kfor4 a2 a3 s0 k)) as [S2 [A2 B2]]; auto. inv B2.
  eapply forever_N_plus.
  eapply plus_left. right; apply step_for.
  eapply star_trans. eapply eval_expression_to_steps; eauto.
  eapply star_left. right; apply step_for_true; auto.
...
Inductive bigstep_program_terminates (p: program): trace -> int -> Prop :=
  | bigstep_program_terminates_intro: forall b f m0 m1 t r,
      ...
      eval_funcall pol ge default_compartment m0 f nil t m1 (Vint r) ->
...
Theorem bigstep_semantics_sound:
  forall p, bigstep_sound (bigstep_semantics p) (semantics pol p).
Proof.
  intros; constructor; intros.
(* termination *)
  inv H. econstructor; econstructor.
  split. econstructor; eauto.
  split. apply (eval_funcall_to_steps _ _ default_compartment); simpl; eauto.
  econstructor.
(* divergence *)
  inv H. econstructor.
  split. econstructor; eauto.
