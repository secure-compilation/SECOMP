...
Lemma rred_simple:
  forall cp r m t r' m', rred pol ge cp r m t r' m' -> simple r -> simple r'.
Proof.
  induction 1; simpl; intuition. destruct b; auto.
Qed.

Lemma rred_compat:
  forall e cp r m r' m', rred pol ge cp r m E0 r' m' ->
  simple r ->
...
Theorem constval_steps:
  ...
  star (step pol) ge (ExprState f r Kstop empty_env m) E0 (ExprState f (Eval v' ty) Kstop empty_env m') ->
  constval ge r = OK v ->
...
Proof.
  ...
  destruct (tr_init_sound pol ge) as (A & B & C).
