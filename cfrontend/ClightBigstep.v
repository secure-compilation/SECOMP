...
Inductive exec_stmt: env -> compartment -> temp_env ->
                     mem -> statement -> trace -> temp_env -> mem -> outcome -> Prop :=
  | exec_Sskip:   forall e c le m,
  ...
  | exec_Sseq_1:   forall e c le m s1 s2 t1 le1 m1 t2 le2 m2 out,
      exec_stmt e c le m s1 t1 le1 m1 Out_normal ->
      exec_stmt e c le1 m1 s2 t2 le2 m2 out ->
      exec_stmt e c le m (Ssequence s1 s2)
                (t1 ** t2) le2 m2 out
  | exec_Sseq_2:   forall e c le m s1 s2 t1 le1 m1 out,
      exec_stmt e c le m s1 t1 le1 m1 out ->
      out <> Out_normal ->
      exec_stmt e c le m (Ssequence s1 s2)
                t1 le1 m1 out
  | exec_Sifthenelse: forall e c le m a s1 s2 v1 b t le' m' out,
      ...
      exec_stmt e c le m (Sifthenelse a s1 s2)
                t le' m' out
  | exec_Sreturn_none:   forall e c le m,
  ...
  | exec_Sbreak:   forall e c le m,
      exec_stmt e c le m Sbreak
               E0 le m Out_break
  | exec_Scontinue:   forall e c le m,
      exec_stmt e c le m Scontinue
               E0 le m Out_continue
  | exec_Sloop_stop1: forall e c le m s1 s2 t le' m' out' out,
      exec_stmt e c le m s1 t le' m' out' ->
      out_break_or_return out' out ->
      exec_stmt e c le m (Sloop s1 s2)
                t le' m' out
  | exec_Sloop_stop2: forall e c le m s1 s2 t1 le1 m1 out1 t2 le2 m2 out2 out,
      exec_stmt e c le m s1 t1 le1 m1 out1 ->
      out_normal_or_continue out1 ->
      exec_stmt e c le1 m1 s2 t2 le2 m2 out2 ->
      out_break_or_return out2 out ->
      exec_stmt e c le m (Sloop s1 s2)
                (t1**t2) le2 m2 out
  | exec_Sloop_loop: forall e c le m s1 s2 t1 le1 m1 out1 t2 le2 m2 t3 le3 m3 out,
      exec_stmt e c le m s1 t1 le1 m1 out1 ->
      out_normal_or_continue out1 ->
      exec_stmt e c le1 m1 s2 t2 le2 m2 Out_normal ->
      exec_stmt e c le2 m2 (Sloop s1 s2) t3 le3 m3 out ->
      exec_stmt e c le m (Sloop s1 s2)
                (t1**t2**t3) le3 m3 out
...
with eval_funcall: compartment -> mem -> fundef -> list val -> trace -> mem -> val -> Prop :=
  | eval_funcall_internal: forall c le m f vargs t e m1 m2 m3 out vres m4,
      ...
      exec_stmt e c (create_undef_temps f.(fn_temps)) m2 f.(fn_body) t le m3 out ->
...
(** Coinductive semantics for divergence.
  [execinf_stmt ge c e m s t] holds if the execution of statement [s]
...
CoInductive execinf_stmt: env -> compartment -> temp_env -> mem -> statement -> traceinf -> Prop :=
  ...
  | execinf_Sseq_2:   forall e c le m s1 s2 t1 le1 m1 t2,
      exec_stmt e c le m s1 t1 le1 m1 Out_normal ->
  ...
  | execinf_Sloop_body1: forall e c le m s1 s2 t,
      execinf_stmt e c le m s1 t ->
      execinf_stmt e c le m (Sloop s1 s2) t
  | execinf_Sloop_body2: forall e c le m s1 s2 t1 le1 m1 out1 t2,
      exec_stmt e c le m s1 t1 le1 m1 out1 ->
      out_normal_or_continue out1 ->
      execinf_stmt e c le1 m1 s2 t2 ->
      execinf_stmt e c le m (Sloop s1 s2) (t1***t2)
  | execinf_Sloop_loop: forall e c le m s1 s2 t1 le1 m1 out1 t2 le2 m2 t3,
      exec_stmt e c le m s1 t1 le1 m1 out1 ->
      out_normal_or_continue out1 ->
      exec_stmt e c le1 m1 s2 t2 le2 m2 Out_normal ->
...
with evalinf_funcall: compartment -> mem -> fundef -> list val -> traceinf -> Prop :=
  | evalinf_funcall_internal: forall c m f vargs t e m1 m2,
      ...
      list_norepet (var_names f.(fn_params) ++ var_names f.(fn_vars)) ->
      bind_parameters ge e m1 f.(fn_params) vargs m2 ->
      execinf_stmt e c (create_undef_temps f.(fn_temps)) m2 f.(fn_body) t ->
...
/\
  (forall c m fd args t m' res,
   ...
   forall k,
   is_call_cont k ->
   star step1 ge (Callstate c fd args k m) t (Returnstate res k m')).
Proof.
...
Lemma evalinf_funcall_forever:
  ...
  forever_N step1 order ge tt (Callstate c fd args k m) T.
Proof.
  cofix CIH_FUN.
