...
+ set (rs1 := nextinstr (rs#x <- (Genv.symbol_address ge id Ptrofs.zero))).
  ...
  intros (rs2 & A & B & C).
  exists rs2; split.
  eapply exec_straight_step with (rs2 := rs1) (m2 := m); eauto. simpl. rewrite EQ'. auto.
...
+ eexists; split.
  econstructor; eauto. simpl. rewrite EQ'. eauto. eauto.
  split; eauto.
  Simpl.
  intros; Simpl.
- (* stackoffset *)
...
Lemma indexed_load_priv_access_correct:
  ...
  Mem.loadv chunk m (Val.offset_ptr rs#base ofs) top = Some v ->
  base <> X31 -> rd <> PC ->
  exists rs',
...
Proof.
  ...
  eapply exec_straight_opt_right. eexact A. eapply exec_straight_one. rewrite EXEC.
  unfold exec_load.
  assert (LOAD': Mem.loadv chunk m (Val.offset_ptr (rs base) ofs) top = Some v).
...
Lemma loadind_priv_correct:
  ...
  Mem.loadv (chunk_of_type ty) m (Val.offset_ptr rs#base ofs) top = Some v ->
  base <> X31 ->
  exists rs',
     exec_straight ge fn c rs m k rs' m
  /\ rs'#(preg_of dst) = v
...
Lemma loadind_priv_ptr_correct:
  ...
  Mem.loadv Mptr m (Val.offset_ptr rs#base ofs) top = Some v ->
  base <> X31 ->
  exists rs',
...
- (* global *)
  ...
  constructor. eapply exec_straight_one.
  simpl; eauto.
  destruct (Genv.allowed_addrof_b ge (comp_of fn) i) eqn:EQ; inv H0.
  auto. auto. auto. auto.
