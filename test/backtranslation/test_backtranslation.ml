...
let property_under_test asm_prog bundled_trace =
  ...
  Frontend.init ();
  Debug.init_compile_unit "autogenerated by backtranslation";
  Sections.initialize();
  CPragmas.reset();
  (* TODO: Bring compiler in same global state as it would be after C2C.convertProgram *)
  (* - add all functions from C2C.helper_functions to the src_program.prog_defs and src_program.prog_public *)
  (* - the above tasks seems impossible: compiler error indicates name of function whereas the src_program has no function names anymore??? *)
  (* - actually, we need to intern the strings and everything *)
  (* - perhaps it would just be easier to "lift" clight to C again, dump it as a file and then trigger the entire compilation again? *)
