...
Definition helper_declared {F V: Type} {CF: has_comp F} (p: AST.program (AST.fundef F) V) (id: ident) (name: string) (sg: signature) : Prop :=
  (prog_defmap p)!id = Some (Gfun (External (EF_runtime name sg))).

Definition helper_functions_declared {F V: Type} {CF: has_comp F} (p: AST.program (AST.fundef F) V) (hf: helper_functions) : Prop :=
...
Theorem eval_select_long:
  ...
  exists v,
     eval_expr ge sp e cp m le (select_long cond al a1 a2) v
