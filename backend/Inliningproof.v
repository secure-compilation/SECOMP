...
Inductive match_stacks (F: meminj) (m m': mem):
             compartment -> list stackframe -> list stackframe -> block -> Prop :=
  | match_stacks_nil: forall bound1 bound cp
        (MG: match_globalenvs F bound1)
        (BELOW: Ple bound1 bound),
      match_stacks F m m' cp nil nil bound
  | match_stacks_cons: forall cp res f sp pc rs stk f' sp' rs' stk' bound fenv ctx ty
      ...
      match_stacks F m m' cp
                   ...
                   (Stackframe (sreg ctx res) ty f' (Vptr sp' Ptrofs.zero) (spc ctx pc) rs' :: stk')
                   bound
  | match_stacks_untailcall: forall stk res f' sp' rpc rs' stk' bound ctx ty cp
        (MS: match_stacks_inside F m m' stk stk' f' ctx sp' rs')
      ...
      match_stacks F m m' cp
                   stk
                   (Stackframe res ty f' (Vptr sp' Ptrofs.zero) rpc rs' :: stk')
                   bound
...
with match_stacks_inside (F: meminj) (m m': mem):
  ...
  | match_stacks_inside_base: forall stk stk' f' ctx sp' rs'
        (MS: match_stacks F m m' (comp_of f') stk stk' sp')
        (RET: ctx.(retinfo) = None)
        (DSTK: ctx.(dstk) = 0),
      match_stacks_inside F m m' stk stk' f' ctx sp' rs'
  | match_stacks_inside_inlined: forall res f sp pc rs stk stk' f' fenv ctx sp' rs' ctx' ty
        (MS: match_stacks_inside F m m' stk stk' f' ctx' sp' rs')
        (SAMECOMP: comp_of f = comp_of f')
        (COMPAT: fenv_compat prog fenv)
        (FB: tr_funbody fenv f'.(fn_stacksize) ctx' f f'.(fn_code))
...
(* Inductive match_stacks: list stackframe -> list stackframe -> Prop := *)
(*   | match_stacks_nil: match_stacks nil nil *)
(*   | match_stacks_cons: forall stk stk', *)
(*       match_stacks_inside stk stk' f' -> *)
(*       (* opening an inlining block for f'. *) *)
(*       match_stacks (Stackframe _ ty f _ _ _ :: stk) (Stackframe _ ty f' _ _ _ :: stk') *)
(*   | match_stacks_untailcall: forall stk stk' f', *)
(*       match_stacks_inside stk stk' f' -> *)
(*       match_stacks stk (Stackframe _ ty f' _ _ _ :: stk') *)

(* with match_stacks_inside: list stackframe -> list stackframe -> function -> Prop := *)
(*   | match_stacks_inside_base: forall stk stk' f', *)
(*       match_stacks stk stk' -> *)
(*       match_stacks_inside stk stk' f' *)
(*   | match_stacks_inside_inlined: forall stk stk' f', *)
(*       match_stacks_inside stk stk' f' -> *)
(*       (* Relation: ty can be arbitrary because it comes from calling a function that is inlined *) *)
(*       (* relation between f and f' comes from inductive case *) *)
(*       match_stacks_inside (Stackframe _ ty f _ _ _ :: stk) stk' f'. *)

(* More generally: when match_stacks_inside stk stk' f', we're going to return something of type sig_res (fn_sig f')
                   when we return in sync; and that should also be the types in the stackframes *)
...
(* Lemma match_stacks_sig: *)
(*   forall stk stk' bound, *)
(*     match_stacks F m m' stk stk' bound -> *)
(*     call_ty stk = call_ty stk' *)
(* with match_stacks_inside_sig: *)
(*   forall stk stk' f ctx sp rs', *)
(*     match_stacks_inside F m m' stk stk' f ctx sp rs' -> call_ty stk' = sig_res (fn_sig f). *)
(* Proof. *)
(*   { induction 1. *)
(*     - eauto. *)
(*     - eauto. *)
(*     - eauto. } *)
(*   { induction 1. *)
(*     - specialize (match_stacks_sig _ _ _ MS). *)
(*     - eauto. } *)
(* Qed. *)

Lemma match_stacks_globalenvs:
  forall cp stk stk' bound,
  match_stacks F m m' cp stk stk' bound -> exists b, match_globalenvs F b
...
Lemma match_stacks_bound:
  forall cp stk stk' bound bound1,
  match_stacks F m m' cp stk stk' bound ->
  Ple bound bound1 ->
  match_stacks F m m' cp stk stk' bound1.
Proof.
  intros. inv H.
...
Lemma match_stacks_invariant:
  forall cp stk stk' bound, match_stacks F m m' cp stk stk' bound ->
  forall (INJ: forall b1 b2 delta,
         ...
         (PERM4: forall b, Plt b bound ->
                         Mem.block_compartment m1' b = Mem.block_compartment m' b),
         (* (PERM4: forall b cp, Plt b bound -> *)
         (*                 Mem.can_access_block m1' b cp <-> Mem.can_access_block m' b cp), *)
  match_stacks F1 m1 m1' cp stk stk' bound
...
Lemma match_stacks_empty:
  forall cp stk stk' bound,
  match_stacks F m m' cp stk stk' bound -> stk = nil -> stk' = nil
...
Lemma match_stacks_free_left:
  forall F m m' cp stk stk' sp b lo hi cp' m1,
  match_stacks F m m' cp stk stk' sp ->
  Mem.free m b lo hi cp' = Some m1 ->
  match_stacks F m1 m' cp stk stk' sp.
Proof.
...
Lemma match_stacks_free_right:
  forall F m m' cp stk stk' sp lo hi cp' m1',
  match_stacks F m m' cp stk stk' sp ->
  Mem.free m' sp lo hi cp' = Some m1' ->
  match_stacks F m m1' cp stk stk' sp.
Proof.
  intros. eapply match_stacks_invariant; eauto.
...
Lemma match_stacks_extcall:
  ...
  match_stacks F2 m2 m2' cp stk stk' bound
...
Proof.
  ...
  eapply match_stacks_untailcall; eauto.
    ...
    eapply range_private_extcall; eauto. red; extlia.
    intros. apply SSZ2; auto. apply MAXPERM'; auto. red; extlia.
    erewrite Mem.unchanged_on_own; eauto. red; extlia.
  induction 1; intros.
  eapply match_stacks_inside_base; eauto.
...
Inductive match_states: RTL.state -> RTL.state -> Prop :=
  | match_regular_states: forall stk stk' f sp pc rs m f' sp' rs' m' F fenv ctx
        (MS: match_stacks_inside F m m' stk stk' f' ctx sp' rs')
        (SAMECOMP: (comp_of f) = comp_of f')
        (* (SIGRES: retinfo ctx = None -> *)
        (*          sig_res (fn_sig f) = sig_res (fn_sig f')) *)
        (COMPAT: fenv_compat prog fenv)
        (FB: tr_funbody fenv f'.(fn_stacksize) ctx f f'.(fn_code))
        (AG: agree_regs F ctx rs rs')
        (SP: F sp = Some(sp', ctx.(dstk)))
        ...
        (AC: Mem.block_compartment m' sp' = comp_of f')
  ...
  | match_return_states: forall stk v m stk' v' m' F cp
        (MS: match_stacks F m m' cp stk stk' (Mem.nextblock m'))
...
+ (* within the original function *)
  ...
  assert (X: { m1' | Mem.free m'0 sp' 0 (fn_stacksize f') (fn_comp f') = Some m1'}).
    ...
    left; simpl; rewrite AC; auto with comps.
    (* left; eapply AC. *)
...
+ (* turned into a call *)
  ...
  assert (SAMECOMP': comp_of fd = comp_of f).
  { unfold find_function in *.
    ...
    - destruct (Genv.find_symbol ge i); try discriminate.
      inv Hvf. congruence. }
  (* TODO: Clean this up *)
  assert (FINDCOMP: Genv.find_comp_in_genv tge vf = comp_of f).
  { rewrite <- find_comp_translated, <- SAMECOMP'.
    unfold find_function in *.
    unfold find_function_ptr in *.
    destruct ros; simpl in *.
    - inv Hvf. rewrite (Genv.find_funct_find_comp_in_genv _ _ H0). congruence.
    - destruct (Genv.find_symbol ge i); try discriminate.
      inv Hvf. rewrite (Genv.find_funct_find_comp_in_genv _ _ H0). congruence. }
  left; econstructor; split.
  eapply plus_one. eapply exec_Icall; eauto.
  ...
  eapply sig_function_translated; eauto.
  rewrite <- SAMECOMP. left; rewrite FINDCOMP; apply flowsto_refl.
...
- (* builtin *)
  ...
  simpl; rewrite <- AC; auto with comps.
  erewrite Mem.unchanged_on_own; eauto.
  eapply range_private_extcall; eauto.
    intros; eapply external_call_max_perm; eauto.
  auto.
...
+ (* inlined *)
  ...
  eapply match_stacks_inside_invariant; eauto.
    intros. eapply Mem.perm_free_3; eauto.
  destruct or; simpl. apply agree_val_reg; eauto. eauto.
  eapply Mem.free_left_inject; eauto.
...
- (* internal function, inlined *)
  ...
  exploit Mem.alloc_left_mapped_inject.
    ...
    simpl; rewrite AC; auto.
    (* rewrite SAMECOMP. apply flowsto_refl. *)
...
+ (* untailcall case *)
  ...
  { clear -EV SAMECOMP. inv EV; auto.
    simpl in H. rewrite SAMECOMP in H.
    destruct (flowsto_dec (comp_of f') (comp_of f')); pose proof (flowsto_refl (comp_of f'));
    congruence. }
  subst t.
