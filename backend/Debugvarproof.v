...
Inductive match_stackframes: Linear.stackframe -> Linear.stackframe -> Prop :=
  | match_stackframe_intro:
      ...
      match_stackframes
        ...
        (Stackframe tf sg sp rs (add_delta_ranges before after tc)).
...
Inductive match_states: Linear.state ->  Linear.state -> Prop :=
  ...
  | match_states_call:
      forall s f rs m tf ts sig cp,
      list_forall2 match_stackframes s ts ->
      transf_fundef f = OK tf ->
      match_states (Callstate s f sig rs m cp)
                   (Callstate ts tf sig rs m cp)
  | match_states_return:
      forall s rs m cp ts,
      list_forall2 match_stackframes s ts ->
...
Proof.
  ...
  destruct tf as [tfi |].
  exists (Callstate nil (Internal tfi) signature_main (Locmap.init Vundef) m0 (comp_of_main prog)); split.
