...
Lemma allowed_call_translated_same:
  forall cp vf,
...
Lemma allowed_call_translated:
  forall cp vf vf' tf,
    ...
    Genv.allowed_call tge cp vf'.
Proof.
  intros cp vf vf' tf LD FIND H.
  inv LD; eauto.
  - now eapply allowed_call_translated_same.
  - inv FIND.
Qed.
...
Definition transl_expr_prop
  ...
  forall tm cs f map pr ns nd rd rs dst
    ...
    (PUB: public_preserving_extension ge tge m tm),
  exists rs', exists tm',
...
Proof.
  intros; red; intros. inv TE.
(* normal case *)
  exploit H0; eauto. intros [rs1 [tm1 [EX1 [ME1 [RR1 [RO1 [EXT1 PUB1]]]]]]].
...
(* Exec *)
  ...
  rewrite <- COMP.
  rewrite (@eval_operation_preserved CminorSel.fundef _ _ _ _ _ ge tge). eauto.
...
Proof.
  ...
  exploit H0; eauto. intros [rs1 [tm1 [EX1 [ME1 [RES1 [OTHER1 [EXT1 PUB1]]]]]]].
...
(* Exec *)
  ...
  apply eval_addressing_preserved. exact allowed_addrof_preserved. exact symbols_preserved. rewrite <- COMP.
  eassumption. traceEq.
(* Match-env *)
  split. eauto with rtlg.
(* Result *)
...
Proof.
  intros; red; intros; inv TE.
  exploit H0; eauto. intros [rs1 [tm1 [EX1 [ME1 [OTHER1 [EXT1 PUB1]]]]]].
  assert (tr_expr f.(fn_code) map pr (if va then ifso else ifnot) (if va then ntrue else nfalse) nd rd dst).
    destruct va; auto.
  exploit H2; eauto. intros [rs2 [tm2 [EX2 [ME2 [RES2 [OTHER2 [EXT2 PUB2]]]]]]].
...
Proof.
  ...
  intros [rs2 [tm2 [EX2 [ME3 [RES2 [OTHER2 [EXT2 PUB2]]]]]]].
  exists rs2; exists tm2.
(* Exec *)
...
Remark eval_builtin_args_trivial:
  ...
  eval_builtin_args ge cp (fun r => rs#r) sp m (List.map (@BA reg) rl) rs##rl.
Proof.
  induction rl; simpl.
- constructor.
...
(* Exec *)
  ...
  rewrite <- COMP.
  eapply external_conds_extends; eauto. eapply senv_preserved.
  rewrite <- COMP.
...
(* Mem *)
  split; auto.
  clear -PUB1 A TRANSL. unfold public_preserving_extension in *.
  intros.
  eapply PUB1; eauto.
  pose proof senv_preserved as (B & C & _ & D).
  rewrite B, C in *.
...
Proof.
  ...
  intros; red; intros. inv TE.
  exploit H3; eauto. intros [rs1 [tm1 [EX1 [ME1 [RR1 [RO1 [EXT1 PUB1]]]]]]].
  exploit external_call_mem_extends; eauto.
  intros [v' [tm2 [A [B [C D]]]]].
...
Proof.
  ...
  assert (tr_condition (fn_code f) map pr (if va then b else c) (if va then n2 else n3) ntrue nfalse).
    destruct va; auto.
  exploit H2; eauto. intros [rs2 [tm2 [EX2 [ME2 [OTHER2 [PUB2 EXT2]]]]]].
  exists rs2; exists tm2.
(* Exec *)
...
Proof.
  ...
  assert (map_wf (add_letvar map r)).
    eapply add_letvar_wf; eauto.
  exploit H2; eauto. eapply match_env_bind_letvar; eauto.
  intros [rs2 [tm2 [EX2 [ME3 [OTHER2 [EXT2 PUB2]]]]]].
  exists rs2; exists tm2.
(* Exec *)
...
- (* XEjumptable *)
  ...
  auto.
- (* XEcondition *)
  exploit transl_condexpr_correct; eauto. intros (rs1 & tm1 & EXEC1 & ME1 & RES1 & EXT1 & PUB1).
  exploit IHeval_exitexpr; eauto.
  instantiate (2 := if va then n2 else n3). destruct va; eauto.
  intros (nd & rs2 & tm2 & EXEC2 & EXIT2 & ME2 & EXT2 & PUB2).
  exists nd, rs2, tm2.
  split. eapply star_trans. apply plus_star. eexact EXEC1. eexact EXEC2. traceEq.
  auto.
- (* XElet *)
  exploit transl_expr_correct; eauto. intros (rs1 & tm1 & EXEC1 & ME1 & RES1 & PRES1 & EXT1 & PUB1).
  assert (map_wf (add_letvar map r)).
    eapply add_letvar_wf; eauto.
...
Lemma invert_eval_builtin_arg:
  ...
  /\ Events.eval_builtin_arg ge cp (fun v => v) sp m (fst (convert_builtin_arg a vl)) v
...
- econstructor; split; eauto with evalexpr. split. constructor. auto. auto.
...
Lemma invert_eval_builtin_args:
  ...
  /\ Events.eval_builtin_args ge cp (fun v => v) sp m (convert_builtin_args al vl') vl.
Proof.
  induction 1; simpl.
...
Lemma transl_eval_builtin_arg:
  ...
  Events.eval_builtin_arg ge cp (fun v => v) sp m (fst (convert_builtin_arg a vl)) v ->
  exists v',
     Events.eval_builtin_arg ge cp (fun r => rs#r) sp m (fst (convert_builtin_arg a rl)) v'
  /\ Val.lessdef v v'
...
Lemma transl_eval_builtin_args:
  ...
  Events.eval_builtin_args ge cp (fun v => v) sp m (convert_builtin_args al vl1) vl ->
  exists vl',
     Events.eval_builtin_args ge cp (fun r => rs#r) sp m (convert_builtin_args al rl) vl'
...
Inductive tr_cont:
  ...
  | tr_Kseq: forall ty c map s k nd nexits ngoto nret rret cs n,
      tr_stmt c map s nd n nexits ngoto nret rret ->
      tr_cont ty c map k n nexits ngoto nret rret cs ->
      tr_cont ty c map (Kseq s k) nd nexits ngoto nret rret cs
  | tr_Kblock: forall ty c map k nd nexits ngoto nret rret cs,
      tr_cont ty c map k nd nexits ngoto nret rret cs ->
      tr_cont ty c map (Kblock k) nd (nd :: nexits) ngoto nret rret cs
  | tr_Kstop: forall ty c map ngoto nret rret cs,
      c!nret = Some(Ireturn rret) ->
      match_stacks ty Kstop cs ->
      tr_cont ty c map Kstop nret nil ngoto nret rret cs
  | tr_Kcall: forall ty c map optid f sp e k ngoto nret rret cs,
      c!nret = Some(Ireturn rret) ->
...
with match_stacks: rettype -> CminorSel.cont -> list RTL.stackframe -> Prop :=
  ...
  | match_stacks_call: forall ty optid f sp e k r tf n rs cs map nexits ngoto nret rret,
      ...
      tr_cont (sig_res (CminorSel.fn_sig f)) tf.(fn_code) map k n nexits ngoto nret rret cs ->
      (* cond on ty *)
      match_stacks ty (Kcall optid f sp e k) (Stackframe r ty tf sp n rs :: cs).
...
Inductive match_states: CminorSel.state -> RTL.state -> Prop :=
  | match_state:
      forall f s k sp e m tm cs tf ns rs map ncont nexits ngoto nret rret
        ...
        (TK: tr_cont (sig_res (CminorSel.fn_sig f)) tf.(fn_code) map k ncont nexits ngoto nret rret cs)
...
Lemma match_stacks_call_cont:
  forall ty c map k ncont nexits ngoto nret rret cs,
  tr_cont ty c map k ncont nexits ngoto nret rret cs ->
  match_stacks ty (call_cont k) cs /\ c!nret = Some(Ireturn rret).
Proof.
...
Lemma tr_cont_call_cont:
  forall ty c map k ncont nexits ngoto nret rret cs,
  tr_cont ty c map k ncont nexits ngoto nret rret cs ->
  tr_cont ty c map (call_cont k) nret nil ngoto nret rret cs.
Proof.
  induction 1; simpl; auto; econstructor; eauto.
Qed.

Lemma tr_find_label:
  forall ty c map lbl n (ngoto: labelmap) nret rret s' k' cs,
  ngoto!lbl = Some n ->
  forall s k ns1 nd1 nexits1,
  find_label lbl s k = Some (s', k') ->
  tr_stmt c map s ns1 nd1 nexits1 ngoto nret rret ->
  tr_cont ty c map k nd1 nexits1 ngoto nret rret cs ->
  exists ns2, exists nd2, exists nexits2,
     c!n = Some(Inop ns2)
  /\ tr_stmt c map s' ns2 nd2 nexits2 ngoto nret rret
  /\ tr_cont ty c map k' nd2 nexits2 ngoto nret rret cs.
Proof.
...
Proof.
  ...
  assert ((fn_code tf)!ncont = Some(Ireturn rret)
          /\ match_stacks (sig_res (CminorSel.fn_sig f)) k cs).
  ...
  exploit transl_expr_correct; eauto.
  intros [rs'' [tm'' [G [J [K [L [M N]]]]]]].
  assert (Val.lessdef_list vl rs''##rl).
    replace (rs'' ## rl) with (rs' ## rl). auto.
    apply list_map_exten. intros. apply L. auto.
  edestruct eval_addressing_lessdef as [vaddr' []]; eauto.
  edestruct Mem.storev_extends as [tm''' []]; eauto.
  econstructor; split.
  left; eapply plus_right. eapply star_trans. eexact A. eexact G. reflexivity.
  eapply exec_Istore with (a := vaddr'). eauto.
  rewrite <- H4. rewrite <- COMP. apply eval_addressing_preserved. exact allowed_addrof_preserved. exact symbols_preserved.
  rewrite <- COMP; eauto. traceEq.
  econstructor; eauto. constructor. admit.
  ...
  (* indirect *)
  exploit transl_expr_correct; eauto.
  intros [rs' [tm' [A [B [C [D [X Z]]]]]]].
  exploit transl_exprlist_correct; eauto.
  intros [rs'' [tm'' [E [F [G [J [Y W]]]]]]].
  ...
  traceEq.
  rewrite COMP'.
  constructor; auto. rewrite SIG in U. auto. admit.
  (* direct *)
  exploit transl_exprlist_correct; eauto.
  intros [rs'' [tm'' [E [F [G [J [Y W]]]]]]].
  exploit functions_translated; eauto. intros [tf' [P Q]].
  ...
  traceEq.
  econstructor; eauto. constructor.
  eapply match_env_update_res; eauto. admit.
  ...
  inv TS.
  exploit transl_condexpr_correct; eauto. intros [rs' [tm' [A [B [C [D E]]]]]].
  econstructor; split.
  left. eexact A.
  ...
  simpl. rewrite COMP0. constructor; auto. admit.
  ...
  econstructor; eauto.
  inversion MS; subst; econstructor; eauto. admit.
