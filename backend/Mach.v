...
Inductive step: state -> trace -> state -> Prop :=
  ...
  | exec_Msetstack:
      ...
      step (State s f sp (Msetstack src ofs ty :: c) rs m)
        E0 (State s f sp c rs' m')
  | exec_Mgetparam:
      forall s fb cmp f sp ofs ty dst c rs m v rs',
  ...
  | exec_Mstore:
      ...
      step (State s f sp (Mstore chunk addr args src :: c) rs m)
        E0 (State s f sp c rs' m')
  | exec_Mcall:
      forall s fb sp sig ros c rs m cmp f f' ra,
      ...
      step (State s fb sp (Mcall sig ros :: c) rs m)
        E0 (Callstate (Stackframe fb sp (Vptr fb ra) c :: s)
                       f' rs m)
  | exec_Mtailcall:
      forall s fb stk soff sig ros c rs m cmp f f' m',
      find_function_ptr ge ros rs = Some f' ->
      Genv.find_funct_ptr ge fb = Some (cmp, Internal f) ->
  ...
  | exec_Mgoto:
      forall s fb cmp f sp lbl c rs m c',
      Genv.find_funct_ptr ge fb = Some (cmp, Internal f) ->
      find_label lbl f.(fn_code) = Some c' ->
      step (State s fb sp (Mgoto lbl :: c) rs m)
        E0 (State s fb sp c' rs m)
  | exec_Mcond_true:
      forall s fb cmp f sp cond args lbl c rs m c' rs',
  ...
  | exec_Mcond_false:
      ...
      step (State s f sp (Mcond cond args lbl :: c) rs m)
        E0 (State s f sp c rs' m)
  | exec_Mjumptable:
      forall s fb cmp f sp arg tbl c rs m n lbl c' rs',
      rs arg = Vint n ->
      ...
      step (State s fb sp (Mjumptable arg tbl :: c) rs m)
        E0 (State s fb sp c' rs' m)
  | exec_Mreturn:
      forall s fb stk soff c rs m cmp f m',
      Genv.find_funct_ptr ge fb = Some (cmp, Internal f) ->
      load_stack m (Vptr stk soff) Tptr f.(fn_link_ofs) = Some (parent_sp s) ->
      load_stack m (Vptr stk soff) Tptr f.(fn_retaddr_ofs) = Some (parent_ra s) ->
      Mem.free m stk 0 f.(fn_stacksize) = Some m' ->
  ...
  | exec_function_internal:
      forall s fb rs m cmp f m1 m2 m3 stk rs',
      Genv.find_funct_ptr ge fb = Some (cmp, Internal f) ->
  ...
  | exec_function_external:
      forall s fb rs m t rs' cmp ef args res m',
      Genv.find_funct_ptr ge fb = Some (cmp, External ef) ->
...
Inductive wf_frame: stackframe -> Prop :=
  | wf_stackframe_intro: forall fb sp ra c cmp f
        (CODE: Genv.find_funct_ptr ge fb = Some (cmp, Internal f))
        (LEAF: is_leaf_function f = false)
        (TAIL: is_tail c f.(fn_code)),
      wf_frame (Stackframe fb sp ra c).

Inductive wf_state: state -> Prop :=
  | wf_normal_state: forall s fb sp c rs m cmp f
