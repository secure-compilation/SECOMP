...
(** [transl_code_at_pc pc fb cp f c ep tf tc] holds if the code pointer [pc] points
  within the Asm code generated by translating Mach function [f] of compartment [cp],
  and [tc] is the tail of the generated code at the position corresponding
  to the code pointer [pc]. *)

Inductive transl_code_at_pc (ge: Mach.genv):
    val -> block -> compartment -> Mach.function -> Mach.code -> bool -> Asm.function -> Asm.code -> Prop :=
  transl_code_at_pc_intro:
    forall b ofs cp f c ep tf tc,
    Genv.find_funct_ptr ge b = Some(cp, Internal f) ->
    transf_function f = Errors.OK tf ->
    transl_code f c ep = OK tc ->
    code_tail (Ptrofs.unsigned ofs) (fn_code tf) tc ->
    transl_code_at_pc ge (Vptr b ofs) b cp f c ep tf tc.
...
Lemma return_address_offset_correct:
  forall ge b ofs fb cp f c tf tc ofs',
  transl_code_at_pc ge (Vptr b ofs) fb cp f c false tf tc ->
  return_address_offset f c ofs' ->
  ofs' = ofs.
Proof.
  intros. inv H. red in H0.
  exploit code_tail_unique. eexact H13. eapply H0; eauto. intro.
...
Inductive exec_straight: code -> regset -> mem ->
  ...
  | exec_straight_one:
      ...
      exec_instr ge cp fn i1 rs1 m1 = Next rs2 m2 ->
      rs2#PC = Val.offset_ptr rs1#PC Ptrofs.one ->
      exec_straight (i1 :: c) rs1 m1 c rs2 m2
  | exec_straight_step:
      forall i c rs1 m1 rs2 m2 c' rs3 m3,
      exec_instr ge cp fn i rs1 m1 = Next rs2 m2 ->
      rs2#PC = Val.offset_ptr rs1#PC Ptrofs.one ->
...
Lemma exec_straight_two:
  ...
  exec_instr ge cp fn i1 rs1 m1 = Next rs2 m2 ->
  exec_instr ge cp fn i2 rs2 m2 = Next rs3 m3 ->
  rs2#PC = Val.offset_ptr rs1#PC Ptrofs.one ->
...
Lemma exec_straight_three:
  ...
  exec_instr ge cp fn i1 rs1 m1 = Next rs2 m2 ->
  exec_instr ge cp fn i2 rs2 m2 = Next rs3 m3 ->
  exec_instr ge cp fn i3 rs3 m3 = Next rs4 m4 ->
  rs2#PC = Val.offset_ptr rs1#PC Ptrofs.one ->
  rs3#PC = Val.offset_ptr rs2#PC Ptrofs.one ->
...
Lemma exec_straight_steps_1:
  ...
  forall b ofs,
  rs#PC = Vptr b ofs ->
  Genv.find_funct_ptr ge b = Some (cp, Internal fn) ->
...
Inductive match_stack: list Mach.stackframe -> Prop :=
  | match_stack_nil:
      match_stack nil
  | match_stack_cons: forall fb sp ra c s cp f tf tc,
      Genv.find_funct_ptr ge fb = Some (cp, Internal f) ->
      transl_code_at_pc ge ra fb cp f c false tf tc ->
      sp <> Vundef ->
      match_stack s ->
