...
- destruct (Genv.find_symbol ge id) as [b|] eqn:FS; try discriminate.
  ...
  unfold Genv.find_funct in *. destruct Ptrofs.eq_dec; try discriminate.
  rewrite Genv.find_funct_ptr_iff in H0.
  exploit defs_inject; eauto. intros (A & B & C).
  rewrite <- Genv.find_funct_ptr_iff in A.
  auto.
...
Proof.
  ...
  - destruct (Genv.find_symbol ge id) as [b|] eqn:FS; try discriminate.
    exploit symbols_inject_2; eauto. intros (tb & P & Q). rewrite P.
    unfold Genv.find_funct in H0. destruct Ptrofs.eq_dec; try discriminate.
    rewrite Genv.find_funct_ptr_iff in H0.
    exploit defs_inject; eauto. intros (A & B & C).
    rewrite <- Genv.find_funct_ptr_iff in A.
    inv H1.
...
Lemma eval_builtin_arg_inject:
  forall rs cp sp m j rs' sp' m' a v,
  ...
  exists v',
     eval_builtin_arg tge (fun r => rs'#r) cp (Vptr sp' Ptrofs.zero) m' a v'
  /\ Val.inject j v v'.
Proof.
...
Lemma eval_builtin_args_inject:
  forall rs cp sp m j rs' sp' m' al vl,
  ...
  exists vl',
     eval_builtin_args tge (fun r => rs'#r) cp (Vptr sp' Ptrofs.zero) m' al vl'
...
Lemma call_trace_translated:
  forall j cp cp' vf tvf rs trs args tyargs t,
...
Proof.
  intros j cp cp' vf tvf rs trs args tyargs t Hregs Hvf Hinj Hnoptr Hcomp H.
  inv H.
  - constructor; eauto.
  - inv Hvf.
    ...
    eapply symbols_inject_1 in Hinj as [? ?]; eauto.
    apply regs_inject with (l := args) in Hregs.
    remember (rs ## args) as vargs.
    remember (trs ## args) as vargs'.
...
- (* op *)
  assert (A: exists tv,
            ...
            /\ Val.inject j v tv).
  { (* pose proof (@has_comp_fundef function has_comp_function) as CF1. *)
    ...
    intros.
    exploit (symbol_address_inject j id ofs).
...
- (* load *)
  assert (A: exists ta,
            ...
            /\ Val.inject j a ta).
  { apply eval_addressing_inj with (CF1 := @has_comp_fundef _ (has_comp_function)) (ge1 := ge) (sp1 := Vptr sp0 Ptrofs.zero) (vl1 := rs##args).
    intros. admit.
...
- (* store *)
  ...
  { apply eval_addressing_inj with (CF1 := @has_comp_fundef _ (has_comp_function))(ge1 := ge) (sp1 := Vptr sp0 Ptrofs.zero) (vl1 := rs##args).
    intros. admit.
    (* apply symbol_address_inject. eapply match_stacks_preserves_globals; eauto. *)
...
- (* call *)
  ...
  intros CROSS.
  (* TODO: write a lemma *)
  assert (regset_inject j rs trs ->
          Forall not_ptr rs ## args ->
          Forall not_ptr trs ## args).
  { clear.
    induction args.
    - eauto.
    - intros INJ H.
      inv H.
      constructor.
      + specialize (INJ a). inv INJ; simpl; eauto.
