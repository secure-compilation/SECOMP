...
Inductive rhs_eval_to (valu: valuation) (ge: genv) cp (sp: val) (m: mem):
                                                     rhs -> val -> Prop :=
  ...
  | load_eval_to: forall chunk addr vl a v p,
      ...
      rhs_eval_to valu ge cp sp m (Load chunk addr vl p) v
  | builtin_eval_to: forall bf args vargs v,
      eval_builtin_args ge cp valu sp m args vargs ->
      builtin_function_sem bf vargs = Some v ->
      rhs_eval_to valu ge cp sp m (Builtin bf args) v.

Lemma rhs_eval_to_compat: forall valu ge cp sp m rh v rh',
  rhs_eval_to valu ge cp sp m rh v ->
  rhs_compat rh rh' ->
  rhs_eval_to valu ge cp sp m rh' v.
Proof.
...
Inductive rhs_valid (valu: valuation) (ge: genv) cp: val -> rhs -> Prop :=
  | op_valid: forall sp op vl,
      rhs_valid valu ge cp sp (Op op vl)
  | load_valid: forall sp chunk addr vl p b ofs bc,
      eval_addressing ge cp (Vptr sp Ptrofs.zero) addr (map valu vl) = Some (Vptr b ofs) ->
      pmatch bc b ofs p -> genv_match bc ge -> bc sp = BCstack ->
      rhs_valid valu ge cp (Vptr sp Ptrofs.zero) (Load chunk addr vl p)
  | builtin_valid: forall sp bf args,
      rhs_valid valu ge cp sp (Builtin bf args).

Inductive equation_holds (valu: valuation) (ge: genv) cp (sp: val) (m: mem):
                                                      equation -> Prop :=
...
Record numbering_holds (valu: valuation) (ge: genv) cp (sp: val)
