...
(** Semantics of all the external functions, indexed by name and signature *)
Definition extcalls_sem: Type :=
  String.string -> signature -> extcall_sem.


(* APT: Following are dead? *)
...
(** To define trace matching, we need a notion of well-formed syscall events.
    ...
    in vanilla CompCert would fail.

    Clearly, the definition of well-formedness must depend on the particular
    system call in question.  It would be possible to define an independent
    notion of well-formedness indexed by function id, but we instead observe
    that well-formedness is a corollary of obeying the semantics of the call.
    So we say that an event is well-formed if it _could_ possibly be produced
    by the call behavior for _some_ choice of environment and memories.
 
    This makes the definition of trace equivalence, and hence statement and proof
    of the generic extcall_properties, dependent on the choice of extcalls_sems.  *)

Inductive well_formed_syscall_event
  (ecs_sem: extcalls_sem)
  (id:String.string) (sg: signature) (eargs: list eventval) (reads: list (list byte))
...
| wfse_intro: forall m m' args res env,
    ...
    ecs_sem id sg env args m (Event_syscall id eargs reads eres writes :: nil) res m' -> 
    well_formed_syscall_event ecs_sem id sg eargs reads eres writes.
 ...
 *)
  | match_traces_syscall: forall id sg args res1 res2 reads writes1 writes2,
      well_formed_syscall_event ecs_sem id sg args reads res1 writes1 ->
      well_formed_syscall_event ecs_sem id sg args reads res2 writes2 -> 
...
Lemma match_traces_preserved:
  forall t1 t2, match_traces ecs_sem ge1 t1 t2 -> match_traces ecs_sem ge2 t1 t2.
Proof.
  induction 1; econstructor; try eapply eventval_valid_preserved; eauto.
Qed.
...
(** We now specify the expected properties of the extcall_sem predicate. *)
...
Record extcall_properties (ecs_sem: extcalls_sem) (sem: extcall_sem) (cp: compartment) (sg: signature) : Prop :=
...
(** External calls must be receptive to changes of traces by another, matching trace. *)
  ec_receptive:
    ...
    sem ge vargs m t1 vres1 m1 -> match_traces ecs_sem ge t1 t2 ->
...
(** External calls must be deterministic up to matching between traces. *)
  ec_determ:
    ...
    match_traces ecs_sem ge t1 t2 /\ (t1 = t2 -> vres1 = vres2 /\ m1 = m2);
...
Lemma volatile_load_ok:
  forall ecs_sem chunk cp ,
  extcall_properties ecs_sem (volatile_load_sem cp chunk)
...
Lemma volatile_store_ok:
  forall ecs_sem cp chunk,
  extcall_properties ecs_sem (volatile_store_sem cp chunk)
...
Lemma extcall_malloc_ok:
  forall ecs_sem cp,
  extcall_properties ecs_sem (extcall_malloc_sem cp)
...
Lemma extcall_free_ok:
  forall ecs_sem cp,
  extcall_properties ecs_sem (extcall_free_sem cp)
...
Lemma extcall_memcpy_ok:
  forall ecs_sem cp sz al,
  extcall_properties ecs_sem (extcall_memcpy_sem cp sz al)
...
Lemma extcall_annot_ok:
  forall ecs_sem cp text targs,
  extcall_properties ecs_sem (extcall_annot_sem cp text targs)
...
Lemma extcall_annot_val_ok:
  forall ecs_sem cp text targ,
  extcall_properties ecs_sem (extcall_annot_val_sem cp text targ)
...
Lemma extcall_debug_ok:
  forall ecs_sem cp targs,
  extcall_properties ecs_sem (extcall_debug_sem cp)
                     cp (mksignature targs Tvoid cc_default).
Proof.
  intros; constructor; intros.
(* well typed *)
...
Lemma known_builtin_ok: forall ecs_sem bf cp,
  extcall_properties ecs_sem (known_builtin_sem bf) cp (builtin_function_sig bf).
Proof.
...
Axiom external_functions_properties:
  forall id sg cp, extcall_properties (external_functions_sem cp) (external_functions_sem cp id sg) cp sg.
...
Axiom inline_assembly_properties:
  forall cp id sg, extcall_properties (external_functions_sem cp) (inline_assembly_sem cp id sg) cp sg.
...
Lemma builtin_or_external_sem_ok: forall name sg cp,
  extcall_properties (external_functions_sem cp) (builtin_or_external_sem cp name sg) cp sg.
Proof.
...
Theorem external_call_spec:
  forall ef,
  extcall_properties (external_functions_sem (comp_of ef)) (external_call ef) (comp_of ef) (ef_sig ef).
Proof.
...
Lemma external_call_match_traces:
  ...
  match_traces (external_functions_sem (comp_of ef)) ge t1 t2.
Proof.
 ...
 *)
  ...
  Inductive extcall_read_sem (cp: compartment) (ge: Senv.t):
    list val -> mem -> trace -> val -> mem -> Prop :=
  | extcall_read_sem_ok: forall fd bytes sz (sz':Z) m m' bdst odst id,
      ...
      extcall_read_sem
        cp ge (Vint fd :: Vptr bdst odst :: Vlong sz :: nil) m
  ...
  | extcall_read_sem_fail: forall fd sz m bdst odst id,
      ...
      Mem.range_perm m bdst (Ptrofs.unsigned odst)
        (Ptrofs.unsigned odst + Int64.unsigned sz) Cur Writable -> (* needed for receptivity *)
      Mem.can_access_block m bdst (Some cp) -> (* needed for receptivity *)
      extcall_read_sem 
        cp ge (Vint fd :: Vptr bdst odst :: Vlong sz :: nil) m
        (Event_syscall "read" (EVint fd :: EVptr_global id odst :: EVlong sz :: nil)
  ...
  Lemma extcall_read_ok: forall (ecs_sem: extcalls_sem) (cp:compartment),
      (forall sg, ecs_sem "read"%string sg = extcall_read_sem cp) ->  (* a bit bogus *)
      extcall_properties ecs_sem (extcall_read_sem cp) cp read_sg. 
  Proof.
    ...
    - split.
      + inv H; inv H0;  
          ...
          apply match_traces_syscall with (sg := read_sg); econstructor; try rewrite EQ; repeat econstructor; eauto. 
      + intro; subst t1.
...
Lemma extcall_write_ok: forall (ecs_sem: extcalls_sem) (cp:compartment),
  (forall sg, ecs_sem "write"%string sg = extcall_write_sem cp) ->  (* a bit bogus *)
  extcall_properties ecs_sem (extcall_write_sem cp) cp write_sg. 
Proof.
  intros ec_sems cp EQ. 
  constructor; intros.
  (* well typed *)
  ...
  - split.
    + inv H; inv H0.  
      ...
      apply match_traces_syscall with (sg := read_sg); econstructor; try rewrite EQ; repeat econstructor; eauto; try lia. 
    + intro; subst t1.
