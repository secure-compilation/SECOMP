...
(** External calls must be receptive to changes of traces by another, matching trace. *)
  ec_receptive:
    ...
    sem ge vargs m t1 vres1 m1 -> match_traces wfse ge t1 t2 ->
...
Lemma volatile_load_ok:
  ...
  extcall_properties wfse (volatile_load_sem cp chunk)
...
Lemma volatile_store_ok:
  ...
  extcall_properties wfse (volatile_store_sem cp chunk)
...
Lemma extcall_malloc_ok:
  forall wfse cp,
  extcall_properties wfse (extcall_malloc_sem cp)
...
Lemma extcall_free_ok:
  forall wfse cp,
  extcall_properties wfse (extcall_free_sem cp)
...
Lemma extcall_memcpy_ok:
  ...
  extcall_properties wfse (extcall_memcpy_sem cp sz al)
...
Lemma extcall_annot_ok:
  ...
  extcall_properties wfse (extcall_annot_sem cp text targs)
...
Lemma extcall_annot_val_ok:
  ...
  extcall_properties wfse (extcall_annot_val_sem cp text targ)
...
Lemma extcall_debug_ok:
  ...
  extcall_properties wfse (extcall_debug_sem cp)
                     cp (mksignature targs Tvoid cc_default).
Proof.
  intros; constructor; intros.
(* well typed *)
...
Parameter external_functions_sem: String.string -> signature -> compartment -> extcall_sem.
...
Inductive well_formed_syscall_event 
  (efs_sem: String.string -> signature -> compartment -> extcall_sem)
...
| wfse_intro: forall m m' args res env,
    ...
    efs_sem id sg cp env args m (Event_syscall id eargs reads eres writes :: nil) res m' -> 
...
Axiom external_functions_properties:
  forall id sg cp, extcall_properties wf_syscall_event (external_functions_sem id sg cp) cp sg.
...
Axiom inline_assembly_properties:
  forall cp id sg, extcall_properties wf_syscall_event (inline_assembly_sem cp id sg) cp sg.
...
Definition builtin_or_external_sem cp name sg :=
  ...
  | None => external_functions_sem name sg cp
  end.

Lemma builtin_or_external_sem_ok: forall name sg cp,
  extcall_properties wf_syscall_event  (builtin_or_external_sem cp name sg) cp sg.
Proof.
...
Definition external_call (ef: external_function): extcall_sem :=
  match ef with
  | EF_external cp name sg  => external_functions_sem name sg cp
...
Theorem external_call_spec:
  forall ef,
  extcall_properties wf_syscall_event (external_call ef) (comp_of ef) (ef_sig ef).
Proof.
 ...
 *)
  ...
  Lemma extcall_read_ok: forall (efs_sem: String.string -> signature -> compartment -> extcall_sem)
                                (cp:compartment),
      (forall sg, efs_sem "read"%string sg = extcall_read_sem) ->  (* a bit bogus *)
      extcall_properties (well_formed_syscall_event efs_sem) (extcall_read_sem cp) cp read_sg. 
  Proof.
...
Lemma extcall_write_ok: forall (efs_sem: String.string -> signature -> compartment -> extcall_sem) (cp:compartment),
  (forall sg, efs_sem "write"%string sg = extcall_write_sem) ->  (* a bit bogus *)
  extcall_properties (well_formed_syscall_event efs_sem) (extcall_write_sem cp) cp write_sg. 
Proof.
  intros ec_sems cp EQ. 
  constructor; intros.
  (* well typed *)
