...
Section Invariants.
      ...
      forall bofs,
        ...
        (forall b o, sp1 = Vptr b o ->
                Mem.range_perm m1 b (Ptrofs.unsigned (Ptrofs.add o (Ptrofs.repr bofs)))
                  (Ptrofs.unsigned (Ptrofs.add o (Ptrofs.repr bofs)) + size_chunk (chunk_of_type ty)) Cur Readable) /\
  ...
  Definition not_fundef (ge: genv) (m: mem) (sp: val) :=
    match sp with
    | Vptr b _ => Genv.find_def ge b = None
    | _ => False
    end.
  ...
  Lemma empty_perm_loadv_none: forall m b chunk ofs cp,
    empty_perm m b ->
    Mem.loadv chunk m (Vptr b ofs) cp = None.
  Proof.
    unfold empty_perm. intros m b chunk ofs cp [_ NOPERM].
    simpl. destruct (Mem.load chunk m b (Ptrofs.unsigned ofs) cp) eqn:LOAD; auto.
    exfalso. apply Mem.load_valid_access in LOAD.
    destruct LOAD as [RPERM _].
    eapply (NOPERM (Ptrofs.unsigned ofs)).
    apply Mem.perm_cur_max. eapply Mem.perm_implies.
    apply RPERM. generalize (size_chunk_pos chunk). lia.
    constructor.
  Qed.
  ...
  Variant stackframe_rel (ge1: genv) (ge2: genv) (ge3: genv) (δ: side)
    ...
    | stackframe_related_opp_δ: forall cp cp' sg b1 b2 b3 sp1 sp2 sp3 ofs1 ofs2 ofs3
        ...
        Genv.find_comp_of_block ge3 b3 = cp ->
          ...
          forall (EMPTY2: empty_perm m2 dummy_sp2),
          forall (EMPTY3: empty_perm m3 dummy_sp3),
          forall (COMP_SP: (s, m1) |= sp1 ∈ opposite δ),
            stackframe_rel ge1 ge2 ge3 δ j__δ j__oppδ m1 m2 m3
              (Stackframe b1 sg cp' sp1 ofs1 dummy_ra1 dummy_sp1)
              (Stackframe b2 sg cp' sp2 ofs2 dummy_ra2 dummy_sp2)
              (Stackframe b3 sg cp' sp3 ofs3 dummy_ra3 dummy_sp3)
  ...
  Definition stack_nodup (st: stack) (f: stackframe) :=
    let 'Stackframe _ _ _ sp _ _ _ := f in
      Forall (fun f: stackframe =>
                let 'Stackframe _ _ _ sp' _ _ _ := f in
                match sp, sp' with
                | Vptr b _, Vptr b' _ => b <> b'
                | _, _ => True
                end) st.
  ...
  | stack_rel_cons: forall st1 st2 st3 f1 f2 f3,
      ...
      forall (NODUP1: stack_nodup st1 f1),
      forall (NODUP2: stack_nodup st2 f2),
      forall (NODUP3: stack_nodup st3 f3),
  ...
  Proof.
    ...
    - econstructor; eauto.
      ...
      + eapply stackframe_related_opp_δ; eauto.
        ...
        destruct (s cp'), (s (Genv.find_comp_of_block _ _)); eauto; congruence.
        now destruct (s (Genv.find_comp_of_block _ _)).
...
meminj_preserves_globals which will allow us to prove preservation of events.
  ...
  Record mem_rel (ge1 ge2: genv) (j: meminj) (δ: side) (m1 m2: mem) :=
    { same_dom: same_domain s ge1 j δ m1;
      ...
      (* The image of j lands on side δ or on global definitions *)
      image_same_domain: forall b b' d, j b = Some (b', d) ->
        (s, m2) |= b' ∈ δ \/
        exists gd, Genv.find_def ge2 b' = Some gd;

      (* same_high_half: forall id ofs, *)
      (*   Val.inject j (high_half ge1 id ofs) (high_half ge2 id ofs) *)
    }.

  Lemma image_disjoint:
    forall δ ge1' ge2' ge3' j1 j2 m1 m2 m3 b1 b3 d1,
      mem_rel ge1' ge3' j1 δ m1 m3 ->
      mem_rel ge2' ge3' j2 (opposite δ) m2 m3 ->
      j1 b1 = Some (b3, d1) ->
      (forall gd, Genv.find_def ge3' b3 <> Some gd) ->
      forall b d, j2 b <> Some (b3, d).
  Proof.
    intros δ ge1' ge2' ge3' j1 j2 m1 m2 m3 b1 b3 d1 m1_m3 m2_m3 Hj1 NOTDEF b d contra.
    exploit (image_same_domain _ _ _ _ _ _ m1_m3); eauto.
    intros [Hside1 | [gd1 Hgd1]]; [| exfalso; eapply NOTDEF; eauto].
    exploit (image_same_domain _ _ _ _ _ _ m2_m3); eauto.
    intros [Hside2 | [gd2 Hgd2]]; [| exfalso; eapply NOTDEF; eauto].
    simpl in Hside1, Hside2. rewrite Hside1 in Hside2.
    destruct δ; discriminate.
  Qed.
...
Proof.
  ...
  - eapply image_same_domain; eauto.
  - { rename H into j_b.
      induction st_rel.
      constructor; eauto.
      constructor; eauto.
      ...
      - eapply stackframe_related_opp_δ; eauto.
        ...
        + unfold empty_perm in *.
          split. eapply Mem.store_valid_block_1; eauto; eapply EMPTY1.
          intros ? ?. eapply EMPTY1. eapply Mem.perm_store_2; eauto.
        + destruct sp1; simpl in *; try exact COMP_SP.
...
Proof.
  ...
  - intros b b' d Hmap.
    ...
    rewrite same_inj in Hmap; eauto.
    eapply image_same_domain; eauto.
  - { eapply inject_incr_stack_rel1; eauto.
      ...
      - constructor; eauto.
        ...
        + simpl in *.
          ...
          * destruct sp1; simpl in *; try exact COMP_SP.
            erewrite Mem.alloc_block_compartment; eauto.
            destruct eq_block; [|exact COMP_SP].
            subst. exfalso. eapply Mem.fresh_block_alloc; eauto. eapply PERM1.
    }
Qed.
...
Proof.
  ...
  - (* image_same_domain *)
    ...
    + exploit inj_incr; eauto. intros E; rewrite Hmap in E; inv E.
      exploit (image_same_domain _ _ _ _ _ _ _ m1_m3); eauto.
      intros [Hside | Hdef]; eauto.
      left. simpl in *. erewrite <- ec_preserves_comp; eauto using external_call_spec.
      eapply Mem.valid_block_inject_2; eauto using partial_mem_inject.
    + exploit inj_sep; eauto. intros [_ Hinvalid].
      assert (Mem.valid_block m3' b').
      { eapply Mem.valid_block_inject_2; eauto. }
      left. simpl. exploit (ec_new_blocks_comp (external_call_spec ef cp)); eauto.
      intros ->. simpl. subst δ. auto.
  - eapply inject_incr_stack_rel1; eauto.
    induction st_rel;
      econstructor; eauto.
    inv H.
    + econstructor; eauto.
      * unfold same_content_stack in *.
        ...
        specialize (STACK_CONTENT1 _ R1) as [R' R'']; split; subst v; eauto.
      ...
      * unfold same_content_stack in *.
        ...
        { eapply X. }
        intros ? G''.
        specialize (STACK_CONTENT2 _ G'') as [R R']; split; eauto.
        destruct sp3; simpl in *; try congruence.
        eapply Mem.load_loadbytes in R' as R''.
        destruct R'' as [bytes' [R1' R2']].
        eapply ec_readonly' in R1'; eauto using external_call_spec.
...
Ltac simpl_nextinstr_PC r :=
  ...
  [subst r; Simpl; eapply Val.offset_ptr_inject; eauto| Simpl].
...
Section Lemmas.
  ...
  Proof.
      ...
      - { intros b. destruct (Pos.eq_dec b b1); subst; intros.
          ...
          + rewrite (diff _ n) in H4.
            ...
            * rewrite diff in H5; eauto.
              ...
              eapply meminj_injective; eauto. }
      - intros b0 b' d0 Hmap.
        destruct (Pos.eq_dec b0 b1); subst.
        + assert (b' = b3) by congruence. assert (d0 = 0) by congruence. subst.
          left. simpl. erewrite Mem.owned_new_block; eauto. simpl. assumption.
        + rewrite (diff _ n) in Hmap.
          exploit (image_same_domain _ _ _ _ _ _ _ m1_m3); eauto.
          intros [Hside | Hdef]; eauto.
          left. simpl in *.
          erewrite Mem.alloc_block_compartment; eauto.
    ...
    { clear dependent j__δ.
      ...
      - intros. intros n.
        ...
        eapply Plt_strict. eapply Plt_Ple_trans; eauto.
      - intros b0 b' d0 Hmap.
        exploit image_same_domain0; eauto. intros [Hside | Hdef]; eauto.
        left. simpl in *.
        erewrite Mem.alloc_block_compartment; eauto.
        destruct eq_block; eauto.
      ...
      - constructor; eauto.
        ...
        + simpl in *.
          ...
          * destruct sp1; simpl in *; try exact COMP_SP.
            erewrite Mem.alloc_block_compartment; eauto.
            destruct eq_block; [|exact COMP_SP].
            subst. exfalso. eapply (Mem.fresh_block_alloc _ _ _ _ m1'); eauto. eapply PERM1.
    }
  Qed.
  ...
  Proof.
    ...
    { clear dependent j__oppδ.
      ...
      - intros b0 b' d0 Hmap.
        assert (b0 <> b1) by congruence.
        rewrite diff in Hmap; eauto.
        exploit (image_same_domain _ _ _ _ _ _ _ m1_m3); eauto.
        intros [Hside | Hdef]; eauto.
        left. simpl in *. erewrite Mem.alloc_block_compartment; eauto.
        destruct eq_block; eauto.
        subst. exfalso. eapply Mem.fresh_block_alloc; eauto.
        eapply Mem.valid_block_inject_2; eauto using partial_mem_inject.
    }
  ...
  Proof.
    ...
    { clear dependent j__oppδ.
      ...
      - intros b0 b' d0 Hmap.
        destruct (Pos.eq_dec b0 b1); subst.
        + rewrite isnone in Hmap. discriminate.
        + rewrite (diff _ n) in Hmap.
          exploit (image_same_domain _ _ _ _ _ _ _ m1_m3); eauto.
          intros [Hside | Hdef]; eauto.
          left. simpl in *.
  ...
  Proof.
    ...
    { clear dependent j__oppδ.
      ...
      - intros b b' d Hmap.
        exploit (image_same_domain _ _ _ _ _ _ _ m1_m3); eauto.
        intros [Hside | Hdef]; eauto.
        left. simpl in *. erewrite <- Mem.free_preserves_comp; eauto.
    }
    { destruct m2_m3.
      constructor; auto.
  ...
  Proof.
    ...
    { clear dependent j__oppδ.
      ...
      - intros b b' d Hmap.
        exploit (image_same_domain _ _ _ _ _ _ _ m1_m3); eauto.
        intros [Hside | Hdef]; eauto.
        left. simpl in *. erewrite Mem.store_block_compartment; eauto.
    }
    { destruct m2_m3.
      constructor; eauto.
  ...
  Lemma set_perm_preserves_rel:
    forall cp_main j j' m1 m2 m3 m1' m2' b1 b2 b3 delta st1 st2 st3
      (inj_pres: meminj_preserves_globals s δ W1 W3 j)
      (inj_pres': meminj_preserves_globals s (opposite δ) W2 W3 j'),
      j b1 = Some (b3, delta) ->
      Mem.set_perm m1 b1 Readable = Some m1' ->
      forall (NOTDEF1: Genv.find_def ge1 b1 = None),
        Mem.set_perm m2 b2 Readable = Some m2' ->
        forall (NOTDEF2: Genv.find_def ge2 b2 = None),
        forall (NOTDEF3: Genv.find_def ge3 b3 = None),
          (forall b d, j' b <> Some (b3, d)) ->
          j' b2 = None ->
          mem_rel s ge1 ge3 j δ m1 m3 ->
  ...
  Proof.
    intros cp_main j j' m1 m2 m3 m1' m2' b1 b2 b3 delta st1 st2 st3
      inj_pres inj_pres' j_b1 set1 ? set2 ? ? Hnotgt Hb2none m1_m3 m2_m3 st_rel.
    exploit (Mem.set_parallel_inject j m1 m3); eauto;
      [eapply partial_mem_inject; eauto | |].
    { intros. eapply meminj_injective with (j := j); eauto. }
    ...
    - constructor; eauto.
      ...
      + intros b b' d Hmap.
        exploit (image_same_domain _ _ _ _ _ _ _ m1_m3); eauto.
        intros [Hside | Hdef]; eauto.
        left. simpl in *. erewrite <- Mem.set_preserves_comp; eauto.
    - constructor; eauto.
      ...
      + intros b b' d Hmap.
        exploit (image_same_domain _ _ _ _ _ _ _ m2_m3); eauto.
        intros [Hside | Hdef]; eauto.
        left. simpl in *. erewrite <- Mem.set_preserves_comp; eauto.
      ...
      + econstructor; eauto.
        * intros ?????.
          ...
          { intros ? ? A; inv A. intros ? ?.
            ...
            -- subst.
               ...
               destruct Mem.perm_dec; try constructor.
               destruct (((Mem.mem_access m1) !! b1 ofs0 Cur)) eqn:R; auto.
               unfold Mem.perm in n; rewrite R in n; auto.
        ...
        * destruct sp1; simpl in *; eauto.
          ...
          -- subst. intros. intros ?.
             ...
             rewrite PMap.gsspec in *. destruct peq; try congruence.
             specialize (H1 ofs).
             destruct Mem.perm_dec; auto; inv H2.
        ...
        * unfold empty_perm in *.
          ...
          eapply Mem.set_perm_perm in n as [? G]; eauto. eapply Mem.perm_implies in G. now eapply H1; eauto.
          constructor.
        * destruct sp2; simpl in *; try exact COMP_SP.
      ...
      + eapply stackframe_related_opp_δ; eauto.
        ...
        * destruct sp1; simpl in *; eauto.
          ...
          -- subst. intros. intros ?.
             ...
             rewrite PMap.gsspec in *. destruct peq; try congruence.
             specialize (H1 ofs).
             destruct Mem.perm_dec; auto; inv H7.
          -- intros ? ?. eapply H1. eapply Mem.perm_set_4; eauto.
        * destruct sp2; simpl in *; eauto.
          destruct PERM2.
          split; eauto using Mem.set_perm_valid_block_1.
  ...
  Lemma set_perm_preserves_rel':
    forall cp_main j j' m1 m2 m3 m1' m2' b1 b2 b3 delta f1 st1 f2 st2 f3 st3,
      match f1, f2, f3 with
      | Stackframe _ _ _ (Vptr b1' _)  _ _ _,
        Stackframe _ _ _ (Vptr b2' _)  _ _ _,
        Stackframe _ _ _ (Vptr b3' _)  _ _ _ => b1' = b1 /\ b2' = b2 /\ b3' = b3
      | _, _, _ => False
      end ->
      j b1 = Some (b3, delta) ->
      ...
      forall (NOTDEF1: Genv.find_def ge1 b1 = None),
        ...
        forall (NOTDEF3: Genv.find_def ge3 b3 = None),
          ...
          mem_rel s ge1 ge3 j δ m1 m3 ->
          mem_rel s ge2 ge3 j' (opposite δ) m2 m3 ->
          stack_rel s cp_main ge1 ge2 ge3 δ j j' m1 m2 m3 (f1 :: st1) (f2 :: st2) (f3 :: st3) ->
          exists m3', Mem.set_perm m3 b3 Freeable = Some m3' /\
                   mem_rel s ge1 ge3 j δ m1' m3' /\
                   mem_rel s ge2 ge3 j' (opposite δ) m2' m3' /\
                   stack_rel s cp_main ge1 ge2 ge3 δ j j' m1' m2' m3' st1 st2 st3.
  Proof.
    intros cp_main j j' m1 m2 m3 m1' m2' b1 b2 b3 delta f1 st1 f2 st2 f3 st3
      old_sp j_b1 set1 ? set2 ? ? Hnotgt Hb2none m1_m3 m2_m3 st_rel.
    exploit (Mem.set_parallel_inject j m1 m3); eauto;
      [eapply partial_mem_inject; eauto | |].
    { intros. eapply meminj_injective with (j := j); eauto. }
    intros [m3' [set3 m1'_m3'_inj]].
    ...
    - constructor; eauto.
      ...
      + intros b b' d Hmap.
        ...
        intros [Hside | Hdef]; eauto.
        left. simpl in *. erewrite <- Mem.set_preserves_comp; eauto.
    - inversion st_rel as [| ?????? st_rel' f_rel H H']; subst.
      clear H H' f_rel st_rel.
      revert f1 f2 f3 old_sp NODUP1 NODUP2 NODUP3.
      induction st_rel'; try now constructor.
      intros ??? OLD Y1 Y2 Y3.
      specialize (IHst_rel' _ _ _ OLD).
      assert (NODUP1': stack_nodup st1 f0).
      { clear -Y1.
        destruct f0; simpl in *. inv Y1; eauto. }
      assert (NODUP2': stack_nodup st2 f4).
      { clear -Y2.
        destruct f4; simpl in *. inv Y2; eauto. }
      assert (NODUP3': stack_nodup st3 f5).
      { clear -Y3.
        destruct f5; simpl in *. inv Y3; eauto. }
      specialize (IHst_rel' NODUP1' NODUP2' NODUP3').
      constructor; eauto.
      inv H.
      + econstructor; eauto.
        * intros ?????.
        ...
        * destruct sp1; simpl in *; eauto.
          ...
          split; eauto using Mem.set_perm_valid_block_1.
          destruct (Pos.eqb_spec b b1).
          -- subst. destruct f0, f4, f5; try congruence.
             destruct sp, sp0, sp1; try congruence.
             destruct OLD; subst; simpl in Y1.
             inv Y1; congruence.
          -- intros ? ?. eapply H1. eapply Mem.perm_set_4; eauto.
        * destruct sp2; simpl in *; eauto.
          destruct PERM2.
          ...
          destruct (Pos.eqb_spec b b2).
          -- subst. destruct f0, f4, f5; try congruence.
             destruct sp, sp0, sp2; try congruence.
             destruct OLD as [? [? ?]]; subst; simpl in Y2.
             inv Y2; congruence.
          -- intros ? ?. eapply H1. eapply Mem.perm_set_4; eauto.
        * destruct sp3; simpl in *; eauto.
          destruct PERM3.
          ...
          destruct (Pos.eqb_spec b b3).
          -- subst. destruct f0, f4, f5; try congruence.
             destruct sp, sp0, sp3; try congruence.
             destruct OLD as [? [? ?]]; subst; simpl in Y3.
             inv Y3; congruence.
          -- intros ? ?. eapply H1. eapply Mem.perm_set_4; eauto.
        * unfold empty_perm in *.
          destruct EMPTY1. split; eauto using Mem.set_perm_valid_block_1.
...
Section Theorems.
  ...
  Lemma call_trace_preserved:
    forall j__δ cp cp' v v' args args' sig t
      ...
      call_trace ge1 cp cp' v args (proj_sig_args sig) t ->
      call_trace ge3 cp cp' v' args' (proj_sig_args sig) t.
  Proof.
    ...
    - specialize (NPTR H).
      ...
      remember (proj_sig_args sig) as tys.
    ...
    forall j__δ st1 rs1 m1 s3 b ofs f i cp,
      ...
      exists st3 rs3 m3 b' f',
        s3 = State st3 rs3 m3 cp /\
          ...
          s (comp_of f) = δ.
  Proof.
    intros * inj_pres equiv eq_pc find_fun find_ins.
    ...
    forall j__δ st1 rs1 m1 s3 b ofs f cp,
      ...
      exists st3 rs3 m3 b',
        s3 = State st3 rs3 m3 cp /\
          ...
          regset_rel j__δ rs1 rs3.
  Proof.
    intros * inj_pres equiv eq_pc find_fun.
    assert (exists b', j__δ b = Some (b', 0)) as [b' inj_b_b'].
    { inv equiv.
      specialize (H4 PC). inv H4; try congruence.
      exploit delta_zero; eauto; intros ->; rewrite Ptrofs.add_zero in *.
  ...
  Lemma eval_builtin_arg_inject:
    forall cp (rs: regset) m j__δ rs' m' a v
      ...
      exists v',
        ...
        /\ Val.inject j__δ v v'.
  Proof.
    induction 1; intros SIDE NOTOP MINJ MR DZ RS MI.
  ...
  Proof.
    ...
    - (* Psel - conditional select *)
      simpl_before_exists.
      (eexists_and_split
         ltac:(fun j rs1 rs3 rs1_rs3 m1 m3 m1_m3 =>
                 (simpl; try reflexivity; try eassumption;
                  solve_simple_regset_rel j rs1 rs3 rs1_rs3 m1 m3 m1_m3; try reflexivity))).
      { pose proof (rs1_rs3 rcond) as Hcond.
        unfold Vone, Vzero in *.
        destruct (Val.eq (rs1 rcond) (Vint Int.one)) as [E1|E1].
        - rewrite E1 in Hcond. inv Hcond.
          destruct (Val.eq (rs3 rcond) (Vint Int.one)); [| congruence].
          apply rs1_rs3.
        - destruct (Val.eq (rs1 rcond) (Vint Int.zero)) as [F1|F1].
          + rewrite F1 in Hcond. inv Hcond.
            destruct (Val.eq (rs3 rcond) (Vint Int.one)); [congruence |].
            destruct (Val.eq (rs3 rcond) (Vint Int.zero)); [| congruence].
            apply rs1_rs3.
          + (* rs1 rcond is neither Vone nor Vzero, so result is Vundef *)
            constructor. }
    - destruct (Genv.allowed_addrof_b ge1 (has_comp_function f) symb) eqn:EQ; try discriminate.
      assert (KEPT: kept_genv s ge1 δ symb = true).
      { revert EQ. unfold kept_genv, Genv.allowed_addrof_b.
        destruct (Genv.find_symbol ge1 symb) eqn:FS; try discriminate.
        fold (Genv.find_def ge1 b).
        destruct Genv.find_def as [[] |] eqn:FIND; try discriminate; auto.
        simpl. intros H.
    ...
    - destruct (Genv.allowed_addrof_b ge1 (has_comp_function f) symb) eqn:EQ; try discriminate.
      ...
      exploit (symbol_address_inject s (s (has_comp_function f)) W1 W3 j__δ symb Ptrofs.zero);
        eauto using delta_zero.
    - destruct (Genv.allowed_addrof_b ge1 (has_comp_function f) id) eqn:EQ; try discriminate.
      assert (KEPT: kept_genv s ge1 δ id = true).
      { revert EQ. unfold kept_genv, Genv.allowed_addrof_b.
        destruct (Genv.find_symbol ge1 id) eqn:FS; try discriminate.
        fold (Genv.find_def ge1 b).
        destruct Genv.find_def as [[] |] eqn:FIND; try discriminate; auto.
        simpl. intros H.
        assert (has_comp_globvar v = has_comp_function f) as ->.
  ...
  Proof.
    intros j__δ j__oppδ f i rs2 rs2' m1 m2 m2' m3 st1 st2 st3
      ? ? side_f exec inj_pres m2_m3 st_rel.
  ...
  Proof.
    ...
    - destruct RES_NOT_PC as [reg ?]; subst res.
      ...
      rewrite 2!Asmgenproof0.undef_regs_other_2; auto. Simpl.
      apply Val.offset_ptr_inject. now apply H.
  Qed.
  ...
  Proof.
    ...
    destruct RES_NOT_PC as [reg ?]; subst res. Simpl.
  ...
  Proof.
        ...
        { eexists; eexists;
          ...
          + econstructor; [| now eapply star_refl | now traceEq].
            ...
            { inv st_rel; eauto.
              - simpl in *. congruence.
              - inv H13; inv H; eauto. }
            { intros ird Hrd. inv Hrd. unfold exec_load. rewrite H5. simpl.
              destruct (Mem.load _ m3 dsp3 _ _) eqn:LOAD; auto.
              exfalso.
              assert (EP: empty_perm m3 dsp3).
              { inv st_rel;
                  [simpl in *; unfold Vnullptr in *; destruct Archi.ptr64; congruence|].
                inv H12; simpl in *;
                match goal with
                | [ HH: Vptr ?b _ = Vptr dsp3 _, EP: empty_perm m3 ?b |- _ ] =>
                    inv HH; exact EP
                end. }
              apply Mem.load_valid_access in LOAD. destruct LOAD as [RPERM _].
              destruct EP as [_ NOPERM]. unfold Mem.range_perm in RPERM.
              apply (NOPERM (Ptrofs.unsigned (Ptrofs.add o' (eval_offset ge3 o)))).
              apply Mem.perm_cur_max. eapply Mem.perm_implies;
                [apply RPERM; generalize (size_chunk_pos ch); lia | constructor]. }
            { intros frd Hrd. discriminate. }
            { inv st_rel;
                [simpl in *; unfold Vnullptr in *; destruct Archi.ptr64; congruence|].
          ...
          + inv strong_s1_s3; econstructor; eauto.
            ...
            * eapply regset_rel_inject. eapply regset_rel_inject. eauto.
              eauto. Simpl. eapply Val.offset_ptr_inject; eauto.
          + inv weak_s2_s3; inv A; econstructor; eauto.
            * inv COMP2; econstructor; eauto.
              -- Simpl. rewrite eq_pc' in *; simpl; eauto.
            * inv COMP2; econstructor; eauto.
        ...
        { eexists; eexists; split; [| split; [| split; [| split; [| split]]]].
          + econstructor; [| now eapply star_refl | now traceEq].
            ...
            { inv st_rel; eauto.
              - simpl in *. congruence.
              - inv H13; inv H; eauto. }
            { intros ird Hrd. discriminate. }
            { intros frd Hrd. inv Hrd. unfold exec_load. rewrite H5. simpl.
              destruct (Mem.load _ m3 dsp3 _ _) eqn:LOAD; auto.
              exfalso.
              assert (EP: empty_perm m3 dsp3).
              { inv st_rel;
    ...
    - exploit strong_equiv_state_internal_inv; eauto.
      ...
      assert (INTERNAL_CALL: Genv.type_of_call (comp_of f) (Genv.find_comp_of_block ge1 b') <> Genv.CrossCompartmentCall)
        by now inv EV.
      clear ARGS CALLSIG NO_CROSS_PTR.
      ...
      assert (INTERNAL_CALL': Genv.type_of_call (comp_of f) (Genv.find_comp_of_block ge3 b3') <> Genv.CrossCompartmentCall).
      ...
      { eapply update_stack_call_preserved_internal with (st3 := st3) in STUPD
        ...
        exists (State st3 (invalidate_call rs3' sig) m3' (has_comp_function f)), j__δ';
          split; [| split; [| split; [| split; [| split]]]].
        + econstructor; [| now eapply star_refl | now traceEq].
          eapply exec_step_internal_call; eauto; try congruence.
    ...
    - exploit strong_equiv_state_internal_inv; eauto.
      ...
      + simpl; eauto.
        replace (mem_of_state s2) with m2; eauto. destruct s2; simpl in *; congruence.
      + inv strong_s1_s3; econstructor; eauto.
        * econstructor; eauto. Simpl. inv COMP1; eauto.
          rewrite H0 in *; simpl in *; unfold Genv.find_comp_of_block in *; rewrite H1 in *; eauto.
        * econstructor; eauto. Simpl. inv COMP2; eauto.
  ...
  Proof.
    ...
    - exploit weak_equivalence_inv; eauto.
      ...
      + intros; simpl_before_exists; eexists; repeat (split; eauto).
        { inv weak_s2_s3; inv B; econstructor; eauto.
          + constructor; eauto. Simpl. rewrite H0.
            inv COMP1; eauto.
            * now rewrite H0; simpl; unfold Genv.find_comp_of_block; rewrite H1.
          + constructor; eauto. Simpl. rewrite H0.
            inv COMP1; eauto.
  ...
  Lemma stack_rel_empty_alloc: forall δ j1 j1' j2 cp m1 m1' m2 m3 st1 st2 st3 b1,
      inject_incr j1 j1' ->
      Mem.alloc m1 cp 0 0 = (m1', b1) ->
      stack_rel s cp_main ge2 ge1 ge3 (opposite δ) j2 j1 m2 m1 m3 st2 st1 st3 ->
      stack_rel s cp_main ge1 ge2 ge3 δ j1' j2 m1' m2 m3 st1 st2 st3.
  Proof.
    clear.
    intros δ j1 j1' j2 cp m1 m1' m2 m3 st1 st2 st3 b1.
    intros incr alloc1 st_rel.
    eapply stack_rel_comm in st_rel.
    replace (opposite (opposite δ)) with δ in st_rel by now destruct δ.
    rename st_rel into H.
    clear -incr H alloc1.
    induction H.
    - constructor.
    - constructor; eauto.
      ...
      + econstructor; eauto.
        * destruct side_eq; eauto.
        * destruct side_eq; eauto.
        * unfold same_content_stack.
          clear -STACK_CONTENT1 alloc1.
          intros ofs ty bofs -> valid.
          exploit STACK_CONTENT1; eauto. intros [A B].
          split.
          -- intros b o -> ofs' in_range.
             eapply Mem.perm_alloc_1; eauto.
             eapply A; eauto.
          -- intros v load1; destruct sp1; try discriminate; simpl in *.
             eapply B.
             erewrite <- (Mem.load_alloc_unchanged _ _ _ _ m1'); eauto.
             eapply Mem.perm_valid_block.
             eapply A with
               (ofs := Ptrofs.unsigned
                         (Ptrofs.add i
                            (Ptrofs.repr (Stacklayout.fe_ofs_arg + 4 * ofs))));
               eauto.
             destruct ty; simpl; lia.
        * unfold at_most_readable.
          clear -PERM1 alloc1.
          destruct sp1; simpl in *; auto.
          destruct PERM1 as [A B].
          split; eauto using Mem.valid_block_alloc.
          intros ofs perm1''.
          eapply Mem.perm_alloc_inv in perm1''; eauto.
          destruct eq_block; [lia | eapply B; eauto].
        * unfold empty_perm.
          clear -EMPTY1 alloc1.
          destruct EMPTY1 as [A B].
          split; eauto using Mem.valid_block_alloc.
          intros ofs perm1''.
          eapply Mem.perm_alloc_inv in perm1''; eauto.
          destruct eq_block; [lia | eapply B; eauto].
      + eapply stackframe_related_opp_δ; eauto.
        * destruct side_eq; eauto.
        * destruct side_eq; eauto.
        * unfold same_content_stack. clear -STACK_CONTENT1 alloc1.
          intros ofs ty bofs -> valid.
          exploit STACK_CONTENT1; eauto. intros [A B].
          split.
          -- intros b o -> ofs' in_range.
  ...
  Lemma stack_content_pres: forall δ j sig args b1 b2 b3 ofs1 ofs2 rs1 rs2 m1 m2 m3,
    Forall not_ptr args ->
    j b1 = Some (b3, 0) ->
    mem_rel s ge1 ge3 j δ m1 m3 ->
    call_arguments rs1 (Vptr b1 ofs1) m1 sig args ->
    call_arguments rs2 (Vptr b2 ofs2) m2 sig args ->
    same_content_stack m1 m3 (Vptr b1 ofs1) (Vptr b3 ofs1) sig /\
    same_content_stack m2 m3 (Vptr b2 ofs2) (Vptr b3 ofs1) sig.
  Proof.
    clear.
    intros δ j sig args b1 b2 b3 ofs1 ofs2 rs1 rs2 m1 m2 m3 no_cross j_b1 m1_m3 call_args1 call_args2.
    split;
      intros ofs ty bofs -> valid;
      split.
    { intros ? ? A; symmetry in A; inv A.
      clear -valid call_args1.
      destruct valid as [? ? ? -> G |
                          ? ? ? ? ? G |
                          ? ? ? ? ? G].
      - unfold call_arguments in call_args1.
        pose proof (list_forall2_in_left _ call_args1 G) as [arg [IN P]].
        inversion P as [temp1 temp2 Q |]; subst temp1 temp2.
        inversion Q as [| ? ? ? ? ? ? load_stack]; subst.
        eapply Mem.load_valid_access in load_stack as [? [? ?]]. eauto.
      - unfold call_arguments in call_args1.
        exploit list_forall2_in_left; eauto.
        intros [arg [IN P]].
        inversion P as [|temp1 temp2 vhi vlo Qhi Qlo]; subst temp1 temp2.
      ...
      - unfold call_arguments in call_args1.
        ...
        eapply Mem.load_valid_access in load_stack as [? [? ?]]. eauto. }
    { intros v load1. clear call_args2.
      assert (v_not_ptr: not_ptr v).
      { destruct valid as [? ? ? -> G | ? ? ? ? G | ? ? ? ? G].
        - unfold call_arguments in call_args1.
          exploit list_forall2_in_left; eauto.
          intros [arg [IN P]].
          inversion P as [temp1 temp2 Q |]; subst temp1 temp2.
      ...
      split; auto.
      simpl in load1; simpl.
      eapply Mem.load_inject in load1 as [v' [R inj_v_v']]; eauto using partial_mem_inject.
      assert (v' = v) as ->.
      { inv inj_v_v'; auto; simpl in v_not_ptr; try contradiction. }
      rewrite Z.add_0_r in R. auto. }
    { intros ? ? A; symmetry in A; inv A.
      clear -valid call_args2.
      destruct valid as [? ? ? -> G |
                          ? ? ? ? ? G |
                          ? ? ? ? ? G].
      - unfold call_arguments in call_args2.
        pose proof (list_forall2_in_left _ call_args2 G) as [arg [IN P]].
        inversion P as [temp1 temp2 Q |]; subst temp1 temp2.
        inversion Q as [| ? ? ? ? ? ? load_stack]; subst.
        eapply Mem.load_valid_access in load_stack as [? [? ?]]. eauto.
      - unfold call_arguments in call_args2.
      ...
      - unfold call_arguments in call_args2.
        ...
        eapply Mem.load_valid_access in load_stack as [? [? ?]]. eauto. }
    { intros v load2.
      assert (load1: Mem.loadv (chunk_of_type ty) m1 (Val.offset_ptr (Vptr b1 ofs1)
                       (Ptrofs.repr (Stacklayout.fe_ofs_arg + 4 * ofs))) top =
               Some v).
      { destruct valid as [? ? ? -> G | ? ? ? ? -> G | ? ? ? ? -> G].
        - unfold call_arguments in call_args1, call_args2.

          pose proof parallel_list_forall2_in_left as H.
          specialize (H _ _ _ _ _ _ _ call_args1 call_args2 G).
          destruct H as [arg [IN [P1 P2]]].
          inversion P1 as [temp1 temp2 Q1 |]; subst temp1 temp2.
          inversion P2 as [temp1 temp2 Q2 |]; subst temp1 temp2.
          inversion Q1 as [| ? ? ? ? ? ? load_stack1]; subst.
        ...
        - unfold call_arguments in call_args1, call_args2.
          ...
          specialize (H _ _ _ _ _ _ _ call_args1 call_args2 G).
          destruct H as [arg [IN [P1 P2]]].
          inversion P1 as [| temp1 temp2 vhi1 vlo1 Qhi1 Qlo1]; subst temp1 temp2.
          inversion P2 as [| temp1 temp2 vhi2 vlo2 Qhi2 Qlo2]; subst temp1 temp2.
          inversion Qhi1 as [| ? ? ? ? ? ? load_stack1]; subst.
          inversion Qhi2 as [| ? ? ? ? ? ? load_stack2]; subst.
          exploit eq_distributes_longofwords; eauto;
            try (eapply Forall_forall; eauto; congruence).
          intros [-> ->].
          assert (vhi1 = v) as ->.
          { eapply Mem.load_Some_None in load_stack2.
            Local Transparent Stacklayout.fe_ofs_arg.
        ...
        - unfold call_arguments in call_args1, call_args2.
          ...
          inversion P1 as [| temp1 temp2 vhi1 vlo1 Qhi1 Qlo1]; subst temp1 temp2.
          inversion P2 as [| temp1 temp2 vhi2 vlo2 Qhi2 Qlo2]; subst temp1 temp2.
          inversion Qlo1 as [| ? ? ? ? ? ? load_stack1]; subst.
          inversion Qlo2 as [| ? ? ? ? ? ? load_stack2]; subst.
          exploit eq_distributes_longofwords; eauto;
            try (eapply Forall_forall; eauto; congruence).
          intros [-> ->].
          assert (vlo1 = v) as ->.
  ...
  Lemma update_stack_call_preserved_internal_cross:
    forall j1 st1 st1' rs1 rs1' m1 m1' b1 ofs1
      j2 st2 st2' rs2 rs2' m2 m2' b2 ofs2
      st3 rs3 m3
      sig cp cp' args e,
      (* First run *)
      update_stack_call ge1 st1 sig cp rs1 m1 = Some (st1', rs1', m1') ->
      Genv.find_comp_in_genv ge1 (rs1 PC) = cp' ->
      meminj_preserves_globals s δ W1 W3 j1 ->
      mem_rel s ge1 ge3 j1 δ m1 m3 ->
      rs1 X1 = Vptr b1 ofs1 ->
      Genv.find_comp_of_block ge1 b1 = cp ->
      regset_rel j1 rs1 rs3 ->

      (* events *)
      call_trace ge1 cp cp' (rs1 PC) args (proj_sig_args sig) (e :: nil) ->
      Forall not_ptr args ->
      call_arguments rs1 (rs1 X2) m1 sig args ->
      call_arguments rs2 (rs2 X2) m2 sig args ->

      (* second run *)
      update_stack_call ge2 st2 sig cp rs2 m2 = Some (st2', rs2', m2') ->
      Genv.find_comp_in_genv ge2 (rs2 PC) = cp' ->
      meminj_preserves_globals s (opposite δ) W2 W3 j2 ->
      mem_rel s ge2 ge3 j2 (opposite δ) m2 m3 ->
      rs2 X1 = Vptr b2 ofs2 ->
      Genv.find_comp_of_block ge2 b2 = cp ->
      Val.inject j2 (rs2 PC) (rs3 PC) ->
      (* common + third run *)
      Genv.find_comp_in_genv ge3 (rs3 PC) = cp' ->
      callee_comp cp_main st3 = cp ->
      s cp = δ ->
      cp' ⊈ cp ->
      cp <> bottom ->
      cp <> top ->
      ...
      cp' <> top ->
      stack_rel s cp_main ge1 ge2 ge3 δ j1 j2 m1 m2 m3 st1 st2 st3 ->
      exists j1' j2' dra1 dsp1 dra2 dsp2 st3' rs3' m3',
        st1' = Stackframe b1 sig cp' (rs1 X2) ofs1 dra1 dsp1 :: st1 /\
          st2' = Stackframe b2 sig cp' (rs2 X2) ofs2 dra2 dsp2 :: st2 /\
          update_stack_call ge3 st3 sig cp rs3 m3 = Some (st3', rs3', m3') /\
          meminj_preserves_globals s δ W1 W3 j1' /\
          meminj_preserves_globals s (opposite δ) W2 W3 j2' /\
          mem_rel s (Genv.globalenv W1) (Genv.globalenv W3) j1' δ m1' m3' /\
          mem_rel s (Genv.globalenv W2) (Genv.globalenv W3) j2' (opposite δ) m2' m3' /\
          inject_incr j1 j1' /\ inject_incr j2 j2' /\
          stack_rel s cp_main ge1 ge2 ge3 δ j1' j2' m1' m2' m3' st1' st2' st3' /\
          rs3' PC = rs3 PC /\
          (s cp' = δ -> regset_rel j1' rs1' rs3') /\
          (s cp' = opposite δ -> regset_rel j2' (invalidate_call rs2' sig)
                                  (invalidate_call rs3' sig)) /\
          call_arguments rs3 (rs3 X2) m3 sig args.
  Proof.
    intros j1 st1 st1' rs1 rs1' m1 m1'''' b1 ofs1
      j2 st2 st2' rs2 rs2' m2 m2'''' b2 ofs2
      st3 rs3 m3
      sig cp cp' args e.
    intros upd1 comp1 inj_pres1 m1_m3 rs1_X1 comp_b1 rs1_rs3
      ev no_cross call_args1 call_args2
      upd2 comp2 inj_pres2 m2_m3 rs2_X1 comp_b2 rs2_PC_rs3_PC
      comp3 callee_comp_st3 s_cp cp'_cp cp_bottom cp_top cp'_bottom cp'_top st_rel.

    unfold update_stack_call in upd1, upd2; unfold update_stack_call.
    rewrite comp1 in upd1; rewrite comp2 in upd2; rewrite comp3.
    destruct cp_eq_dec; [subst; exfalso; apply cp'_cp; auto with comps |].
    destruct (cp_eq_dec); [contradiction|].

    destruct (Mem.alloc m1 cp' 0 0) as [m1' dra1] eqn:alloc1,
          (Mem.alloc m1' cp' 0 0) as [m1'' dsp1] eqn:alloc1'.
    destruct (Mem.alloc m2 cp' 0 0) as [m2' dra2] eqn:alloc2,
          (Mem.alloc m2' cp' 0 0) as [m2'' dsp2] eqn:alloc2'.

    specialize (rs1_rs3 X2) as rs1_rs3_X2.
    destruct (rs1 X2) as [| | | | | b1' ofs1'] eqn:rs1_X2;
      try congruence.
    destruct (rs2 X2) as [| | | | | b2' ofs2'] eqn:rs2_X2;
      try congruence.
    destruct (rs3 X2) as [| | | | | b3' ofs3'] eqn:rs3_X2;
      try now inversion rs1_rs3_X2.

    destruct (Genv.find_def ge1 b1') eqn:fd1; try discriminate.
    destruct (Genv.find_def ge2 b2') eqn:fd2; try discriminate.

    assert (fd3: Genv.find_def ge3 b3' = None).
    { destruct (Genv.find_def ge3 b3') eqn:?; auto.
      inv rs1_rs3_X2.
      exploit (defs_rev_inject s _ W1 W3 j1 inj_pres1); eauto.
      intros [? [? _]]; congruence. }
    rewrite fd3.

    destruct (cp_eq_dec (Mem.block_compartment m1 b1') cp) eqn:bc1; try discriminate.
    destruct (cp_eq_dec (Mem.block_compartment m2 b2') cp) eqn:bc2; try discriminate.
    rename e0 into sp1_comp, e1 into sp2_comp.

    specialize (rs1_rs3 X1) as rs1_rs3_X1.
    rewrite rs1_X1 in upd1, rs1_rs3_X1; rewrite rs2_X1 in upd2.
    destruct (rs3 X1) as [| | | | | b3 ofs3] eqn:rs3_X1;
      try now inversion rs1_rs3_X1.
    assert (comp_b3: Genv.find_comp_of_block ge3 b3 = cp).
    { inv rs1_rs3_X1; eauto.
      erewrite <- (find_comp_of_block_preserved s W1 W3); eauto using delta_zero. }

    rewrite <- s_cp in *.

    assert (rs1_rs3_initial := rs1_rs3).

    eapply (alloc_preserves_rel s W1 W2 W3) in alloc1 as alloc1''; eauto using match_prog_unique.
    clear rs1_rs3.
    destruct alloc1'' as
      (j1' & m3' & dra3 & alloc3 & inj_pres1' & m1'_m3' & m2_m3' & rs1_rs3 & j1'_dra1 & j1_j1' & st_rel1').
    eapply (alloc_preserves_rel s W1 W2 W3) in alloc1' as alloc1''; eauto using match_prog_unique.
    clear rs1_rs3.
    destruct alloc1'' as
      (j1'' & m3'' & dsp3 & alloc3' & inj_pres1'' & m1''_m3'' & m2_m3'' & rs1_rs3 & j1''_dsp1 & j1'_j1'' & st_rel1'').
    rewrite alloc3, alloc3'.

    replace (s cp) with (opposite (opposite (s cp))) in m1_m3 by now destruct s.
    eapply stack_rel_comm in st_rel.
    eapply (alloc_preserves_rel_no_regset s W2 W1 W3) with (m2 := m1) (m3 := m3) in alloc2 as alloc2'';
      eauto using match_prog_unique.
    destruct alloc2'' as
      (j2' & m3_t & dra3_t & alloc3_t & inj_pres2' & m2'_m3' & m1_m3' & j2'_dra2 & j2_j2' & st_rel2').
    assert (m3_t = m3') as -> by congruence.
    assert (dra3_t = dra3) as -> by congruence. clear alloc3_t.
    eapply (alloc_preserves_rel_no_regset s W2 W1 W3) with (m2 := m1) (m3 := m3') in alloc2' as alloc2'';
      eauto using match_prog_unique.
    destruct alloc2'' as
      (j2'' & m3_t & dsp3_t & alloc3_t & inj_pres2'' & m2''_m3'' & m1_m3'' & j2''_dsp2 & j2'_j2'' & st_rel2'').
    assert (m3_t = m3'') as -> by congruence.
    assert (dsp3_t = dsp3) as -> by congruence. clear alloc3_t.

    destruct (Mem.perm_dec m1'' b1' 0 Max Freeable) as [get_perm1 |]; try discriminate.
    destruct (Mem.perm_dec m2'' b2' 0 Max Freeable) as [get_perm2 |]; try discriminate.

    inversion rs1_rs3_X1 as [ | | | | temp1 temp2 temp3 temp4 delta j1_b1 ofs3_eq1 | ];
      subst temp1 temp2 temp3 temp4.
    assert (delta = 0) as -> by now eapply (delta_zero _ _ _ j1); eauto.
    inversion rs1_rs3_X2 as [ | | | | temp1 temp2 temp3 temp4 delta j1_b1' ofs3_eq2 | ];
      subst temp1 temp2 temp3 temp4.
    assert (delta = 0) as -> by now eapply (delta_zero _ _ _ j1); eauto.

    assert (bc3: Mem.block_compartment m3 b3' = cp).
    { assert (perm3'': Mem.perm m3'' b3' 0 Max Freeable).
      { replace 0 with (0 + 0) by lia.
        eapply Mem.perm_inject with (f := j1''); eauto using partial_mem_inject. }
      assert (b3'_ne_dsp3: b3' <> dsp3).
      { intro; subst; eapply Mem.perm_alloc_3 in perm3''; eauto; lia. }
      assert (perm3': Mem.perm m3' b3' 0 Max Freeable).
      { eapply Mem.perm_alloc_4; eauto. }
      assert (b3'_ne_dra3: b3' <> dra3).
      { intro; subst; eapply Mem.perm_alloc_3 in perm3'; eauto; lia. }
      assert (b1'_ne_dsp1: b1' <> dsp1).
      { intro; subst; eapply Mem.perm_alloc_3 in get_perm1; eauto; lia. }
      assert (perm1': Mem.perm m1' b1' 0 Max Freeable).
      { eapply Mem.perm_alloc_4; eauto. }
      assert (b1'_ne_dra1: b1' <> dra1).
      { intro; subst; eapply Mem.perm_alloc_3 in perm1'; eauto; lia. }
      transitivity (Mem.block_compartment m3'' b3').
      - symmetry.
        erewrite Mem.alloc_block_compartment; [| exact alloc3'].
        destruct eq_block; [congruence |].
        erewrite Mem.alloc_block_compartment; [| exact alloc3].
        destruct eq_block; [congruence |].
        reflexivity.
      - transitivity (Mem.block_compartment m1'' b1').
        + symmetry.
          eapply (Mem.mi_access _ _ _
            (Mem.mi_inj _ _ _ (partial_mem_inject _ _ _ _ _ _ _ m1''_m3''))).
          * apply j1'_j1''; apply j1_j1'; exact j1_b1'.
          * exact get_perm1.
        + erewrite Mem.alloc_block_compartment; [| exact alloc1'].
          destruct eq_block; [congruence |].
          erewrite Mem.alloc_block_compartment; [| exact alloc1].
          destruct eq_block; [congruence |].
          exact sp1_comp. }
    destruct (cp_eq_dec (Mem.block_compartment m3 b3') cp); try congruence.

    assert (get_perm3: Mem.perm m3'' b3' 0 Max Freeable).
    { replace 0 with (0 + 0) by lia.
      eapply Mem.perm_inject with (f := j1''); eauto using partial_mem_inject. }
    destruct (Mem.perm_dec m3'' b3' 0 Max Freeable) as [yes|]; [clear yes | congruence].


    destruct (Mem.set_perm m1'' b1' Readable) as [m1''' |] eqn:set_perm1; try discriminate.
    destruct (Mem.set_perm m2'' b2' Readable) as [m2''' |] eqn:set_perm2; try discriminate.
    injection upd1. intros <- <- <-.
    injection upd2. intros <- <- <-.

    assert (inject_incr j1 j1'') as incr1 by (eapply inject_incr_trans; eauto).
    assert (inject_incr j2 j2'') as incr2 by (eapply inject_incr_trans; eauto).
    eapply stack_rel_empty_alloc with (j1' := j1') in st_rel2''; eauto.
    apply stack_rel_comm in st_rel2''.
    eapply stack_rel_empty_alloc in st_rel2'' as st_rel'; eauto.
    assert (Hnotgt: forall b d, j2'' b <> Some (b3', d)).
    { eapply image_disjoint.
      - exact m1''_m3''.
      - exact m2''_m3''.
      - exact (incr1 _ _ _ j1_b1').
      - intros gd contra. rewrite fd3 in contra. discriminate. }
    assert (Hb2none: j2'' b2' = None).
    { destruct (j2'' b2') as [[b'' d]|] eqn:Hj2; auto.
      exfalso.
      assert (b2' <> dsp2) as b2'_ne_dsp2.
      { intro Heq; subst. eapply Mem.perm_alloc_3 in get_perm2; eauto. lia. }
      assert (Mem.perm m2' b2' 0 Max Freeable) as perm_m2'.
      { eapply Mem.perm_alloc_4; eauto. }
      assert (b2' <> dra2) as b2'_ne_dra2.
      { intro Heq; subst. eapply Mem.perm_alloc_3 in perm_m2'; eauto. lia. }
      assert (Mem.block_compartment m2'' b2' = Mem.block_compartment m2 b2') as comp_eq.
      { erewrite Mem.alloc_block_compartment; eauto.
        rewrite dec_eq_false; auto.
        erewrite Mem.alloc_block_compartment; eauto.
        rewrite dec_eq_false; auto. }
      assert (j2'' b2' <> None) as Hne by congruence.
      destruct (proj1 (same_dom _ _ _ _ _ _ _ m2''_m3'' b2') Hne) as [[Hside _] | [fd Hfd]].
      - simpl in Hside. rewrite comp_eq, sp2_comp in Hside.
        destruct (s cp); discriminate.
      - rewrite fd2 in Hfd. discriminate. }
    eapply (set_perm_preserves_rel s W1 W2 W3) in st_rel'; eauto using match_prog_unique;
      try congruence.
    destruct st_rel' as [m3''' [set_perm3 [m1'''_m3''' [m2'''_m3''' st_rel'']]]].
    rewrite set_perm3.

    eexists j1'', j2'', dra1, dsp1, dra2, dsp2, _, _, m3'''.
    repeat (split; [now eauto |]).
    split; [| split; [| split; [| split]]].
    - constructor; eauto.


      assert (READABLE1: Mem.valid_block m1''' b1' /\ (forall ofs : Z, ~ Mem.perm m1''' b1' ofs Max Writable)).
      { split.
        - eapply Mem.set_perm_valid_block_1; eauto.
          eapply Mem.perm_valid_block; eauto.
        - intros ofs. revert set_perm1.
          clear.
          unfold Mem.set_perm; destruct plt; try discriminate.
          intros H; inv H; unfold Mem.perm; simpl.
          rewrite PMap.gss; auto.
          destruct Mem.perm_dec; auto. intros H; inv H. }
      assert (READABLE2: Mem.valid_block m2''' b2' /\ (forall ofs : Z, ~ Mem.perm m2''' b2' ofs Max Writable)).
      { split.
        - eapply Mem.set_perm_valid_block_1; eauto.
          eapply Mem.perm_valid_block; eauto.
        - intros ofs. revert set_perm2.
          clear.
          unfold Mem.set_perm; destruct plt; try discriminate.
          intros H; inv H; unfold Mem.perm; simpl.
          rewrite PMap.gss; auto.
          destruct Mem.perm_dec; auto. intros H; inv H. }
      assert (READABLE3: Mem.valid_block m3''' b3' /\ (forall ofs : Z, ~ Mem.perm m3''' b3' ofs Max Writable)).
      { split.
        - eapply Mem.set_perm_valid_block_1; eauto.
          eapply Mem.perm_valid_block; eauto.
        - intros ofs. revert set_perm3.
          clear.
          unfold Mem.set_perm; destruct plt; try discriminate.
          intros H; inv H; unfold Mem.perm; simpl.
          rewrite PMap.gss; auto.
          destruct Mem.perm_dec; auto. intros H; inv H. }
      assert (EMPTY1: empty_perm m1''' dsp1).
      { unfold empty_perm.
        split.
        - eapply Mem.set_perm_valid_block_1; eauto.
          eapply Mem.valid_new_block; eauto.
        - intros ofs perm1.
          eapply Mem.perm_set_4 in perm1; eauto.
          eapply Mem.perm_alloc_3 in perm1; eauto. lia.
          destruct (peq dsp1 b1').
          + right. subst. intros perm1'.
            eapply Mem.perm_alloc_3 in perm1'; eauto. lia.
          + now left. }
      assert (EMPTY2: empty_perm m2''' dsp2).
      { unfold empty_perm.
        split.
        - eapply Mem.set_perm_valid_block_1; eauto.
          eapply Mem.valid_new_block; eauto.
        - intros ofs perm2.
          eapply Mem.perm_set_4 in perm2; eauto.
          eapply Mem.perm_alloc_3 in perm2; eauto. lia.
          destruct (peq dsp2 b2').
          + right. subst. intros perm2'.
            eapply Mem.perm_alloc_3 in perm2'; eauto. lia.
          + now left. }
      assert (EMPTY3: empty_perm m3''' dsp3).
      { unfold empty_perm.
        split.
        - eapply Mem.set_perm_valid_block_1; eauto.
          eapply Mem.valid_new_block; eauto.
        - intros ofs perm3.
          eapply Mem.perm_set_4 in perm3; eauto.
          eapply Mem.perm_alloc_3 in perm3; eauto. lia.
          destruct (peq dsp3 b3').
          + right. subst. intros perm3'.
            eapply Mem.perm_alloc_3 in perm3'; eauto. lia.
          + now left. }
      assert (call_args1': call_arguments rs1 (Vptr b1' ofs1') m1''' sig args).
      { clear -call_args1 alloc1 alloc1' set_perm1.
        unfold call_arguments in *.
        remember (loc_parameters sig) as params eqn:sig_eq; clear sig_eq.
        remember (Vptr b1' ofs1') as sp1 eqn:eq_sp1.
        revert b1' ofs1' set_perm1 eq_sp1.
        induction call_args1.
        - constructor.
        - intros b1' ofs1' set_perm1 eq_sp1; subst sp1.
          specialize (IHcall_args1 _ _ set_perm1 eq_refl).
          constructor; eauto.
          destruct H as [l v H | hi lo vhi vlo Hhi Hlo].
          + constructor.
            ...
            eapply Mem.load_alloc_other; eauto.
          + constructor.
            * inv Hhi; econstructor; eauto.
              simpl in *.
              eapply Mem.load_set; [eauto | constructor |].
              eapply Mem.load_alloc_other; eauto.
              eapply Mem.load_alloc_other; eauto.
            * inv Hlo; econstructor; eauto.
              simpl in *.
              eapply Mem.load_set; [eauto | constructor |].
              eapply Mem.load_alloc_other; eauto.
              eapply Mem.load_alloc_other; eauto.
      ...
      assert (call_args2': call_arguments rs2 (Vptr b2' ofs2') m2''' sig args).
      { clear -call_args2 alloc2 alloc2' set_perm2.
        unfold call_arguments in *.
        remember (loc_parameters sig) as params eqn:sig_eq; clear sig_eq.
        remember (Vptr b2' ofs2') as sp2 eqn:eq_sp2.
        revert b2' ofs2' set_perm2 eq_sp2.
        induction call_args2.
        - constructor.
        - intros b2' ofs2' set_perm2 eq_sp2; subst sp2.
          specialize (IHcall_args2 _ _ set_perm2 eq_refl).
          constructor; eauto.
          destruct H as [l v H | hi lo vhi vlo Hhi Hlo].
          + constructor.
            inv H; econstructor; eauto.
      ...
      assert (content1: same_content_stack m1''' m3''' (Vptr b1' ofs1') (Vptr b3' ofs3') sig).
      { apply incr1 in j1_b1'.
        rewrite Ptrofs.add_zero in ofs3_eq2. subst ofs3'.
        eapply stack_content_pres; eauto. }
      assert (content2: same_content_stack m2''' m3''' (Vptr b2' ofs2') (Vptr b3' ofs3') sig).
      { rewrite Ptrofs.add_zero in ofs3_eq2. subst ofs3'.
        eapply stack_content_pres with (δ := s cp) (j := j1''); eauto. }

      assert (COMP_SP_b2': (s, m2''') |= Vptr b2' ofs2' ∈ s cp).
      { simpl.
        destruct (side_eq (s (Mem.block_compartment m2''' b2')) (s cp)) as [|n1]; auto.
        exfalso.
        assert (s (Mem.block_compartment m2''' b2') <> opposite (s cp)).
        { intros contra.
          assert (j2'' b2' <> None).
          { apply (same_dom _ _ _ _ _ _ _ m2'''_m3''' b2'). left. simpl. split; auto. apply READABLE2. }
          congruence. }
        destruct (s (Mem.block_compartment m2''' b2')), (s cp); simpl in *; tauto. }

      * destruct (side_eq (s cp') (s cp)) as [same_side | change_side].
        ...
        { eapply stackframe_related_δ; eauto; try congruence.
          -- destruct side_eq as [| _]; try contradiction.
             ...
             eapply j2'_dra2; simpl; eauto. now destruct (s cp), (s cp').
             reflexivity.
          -- destruct side_eq as [| _]; try contradiction.
             econstructor; eauto.
      ...
      * clear -get_perm1 st_rel1''. simpl in *.
        induction st_rel1''.
        -- constructor.
        -- constructor; eauto.
           inv H.
      ...
      * clear -get_perm2 st_rel2''. simpl in *.
        induction st_rel2''.
        -- constructor.
        -- constructor; eauto.
           inv H.
           ...
           ++ destruct sp1; auto.
              intros ?; subst. eapply PERM1.
              eapply Mem.perm_implies; eauto. constructor.
      * clear -get_perm3 st_rel2''. simpl in *.
        induction st_rel2''.
        -- constructor.
        -- constructor; eauto.
           inv H.
           ...
           ++ destruct sp3; auto.
              ...
              eapply Mem.perm_implies; eauto. constructor.
    - Simpl.
    - clear -j1''_dsp1 j1'_dra1 j1'_j1'' incr1 rs1_rs3.
      simpl in *.
      intros R; specialize (j1'_dra1 R); specialize (j1''_dsp1 R).
      apply j1'_j1'' in j1'_dra1.
      eapply regset_rel_inject; eauto.
      eapply regset_rel_inject; eauto.
    - clear -j2''_dsp2 j2'_dra2 j2'_j2'' incr2 rs1_rs3 rs2_PC_rs3_PC
               call_args1 call_args2 no_cross.
      intros R; specialize (j2'_dra2 R); specialize (j2''_dsp2 R).
      apply j2'_j2'' in j2'_dra2.
      unfold invalidate_call.
      intros r; destruct (preg_eq r PC) as [-> |];
        [simpl; eapply val_inject_incr; eauto |].
      destruct (preg_eq r X1) as [-> |];
        [simpl; econstructor; eauto |].
      destruct (preg_eq r X2) as [-> |];
        [simpl; econstructor; eauto |].
      destruct in_dec as [i | ]; simpl; Simpl; eauto.

      clear -call_args1 call_args2 rs1_rs3 i no_cross.

      eapply list_in_map_inv in i as [R [? G]]. subst r.
      eapply filter_In in G as [_ G].
      unfold LTL.parameters_mregs in *.

      inversion call_args1 as [H | a1 al1 arg1 args1 CA1 FA1 H1 H1'];
        [rewrite <- H in G; simpl in G; inv G |].
      inversion call_args2 as [| a2 al2 arg2 args2 CA2 FA2 H2 H2'];
        [congruence | rewrite <- H2' in H1'; inversion H1'; subst arg2 args2 args; clear H1'].
      assert (a2 = a1) by congruence; subst a2.
      assert (al2 = al1) by congruence; subst al2.
      rename a1 into a; rename al1 into al.
      clear H2; rename H1 into H. rewrite <- H in *. clear H.
      rename args1 into args; rename arg1 into arg.

      assert (HYP1: list_forall2 (call_arg_pair rs1 (Vptr b1' ofs1') m1) (a :: al) (arg :: args)) by now constructor; eauto.
      assert (HYP2: list_forall2 (call_arg_pair rs2 (Vptr b2' ofs2') m2) (a :: al) (arg :: args)) by now constructor; eauto.
      clear CA1 FA1 CA2 FA2 call_args1 call_args2.
      remember (a :: al) as l eqn:eq_l; clear eq_l.
      remember (arg :: args) as l' eqn:eq_l'; clear eq_l'.
      generalize dependent l'.
      generalize dependent l.
      clear -rs1_rs3.
      induction l as [| a al IH].
      + now simpl.
      + destruct a as [[r | ]| rhi rlo]; simpl.
        * destruct (mreg_eq R r); try subst R; simpl.
          -- intros _ l' no_cross FA1 FA2.
             inversion FA1 as [| temp1 temp2 v1 vl1 CA1 FA1']; subst temp1 temp2 l'.
             inversion FA2 as [| temp1 temp2 v2 vl2 CA2 FA2']; subst temp1 temp2 v2 vl2.
             inversion CA1 as [temp1 temp2 CA1' |]; subst temp1 temp2.
             inversion CA2 as [temp1 temp2 CA2' |]; subst temp1 temp2.
             inversion CA1' as [temp |]; subst temp v1.
             inversion CA2' as [temp R' H|]; subst temp.
             rewrite H.
             inv no_cross.
             specialize (rs1_rs3 (preg_of r)).
             destruct (rs1 (preg_of r)); inv rs1_rs3; now simpl in *.
          -- intros IN l' no_cross FA1 FA2.
             specialize (rs1_rs3 (preg_of r)).
             inv FA1; inv FA2; inv no_cross.
             exploit IH; eauto.
        * intros IN l' no_cross FA1 FA2.
          inv FA1; inv FA2; inv no_cross.
          exploit IH; eauto.
        * destruct rhi as [rhi | ]; destruct rlo as [rlo |]; eauto; simpl.
          -- destruct (mreg_eq R rhi);
               [subst R | destruct (mreg_eq R rlo);
                          [subst R| ]]; simpl.
             ++ intros _ l' no_cross FA1 FA2.
                inversion FA1 as [| temp1 temp2 v1 vl1 CA1 FA1']; subst temp1 temp2 l'.
                inversion FA2 as [| temp1 temp2 v2 vl2 CA2 FA2']; subst temp1 temp2 v2 vl2.
                inversion CA1 as [|temp1 temp2 vhi1 vlo1 CAhi1 CAlo1 R]; subst temp1 temp2 v1.
                inversion CA2 as [|temp1 temp2 vhi2 vlo2 CAhi2 CAlo2 T R]; subst temp1 temp2.
                inv CAhi1; inv CAhi2.
                inv CAlo1; inv CAlo2.
                assert (nptr1: not_ptr (Val.longofwords (rs1 (preg_of rhi)) (rs1 (preg_of rlo)))).
                { inv no_cross; eauto. }
                assert (nptr2: not_ptr (Val.longofwords (rs2 (preg_of rhi)) (rs2 (preg_of rlo)))).
                { rewrite R; eauto. }
                apply eq_distributes_longofwords in R as [-> _]; eauto.
                assert (nptr_rhi: not_ptr (rs1 (preg_of rhi))).
                { destruct (rs1 (preg_of rhi)); now simpl in *. }
                specialize (rs1_rs3 (preg_of rhi)).
                destruct (rs1 (preg_of rhi)); inv rs1_rs3; now simpl in *.
             ++ intros _ l' no_cross FA1 FA2.
                inversion FA1 as [| temp1 temp2 v1 vl1 CA1 FA1']; subst temp1 temp2 l'.
                inversion FA2 as [| temp1 temp2 v2 vl2 CA2 FA2']; subst temp1 temp2 v2 vl2.
                inversion CA1 as [|temp1 temp2 vhi1 vlo1 CAhi1 CAlo1 R]; subst temp1 temp2 v1.
                ...
                { rewrite R; eauto. }
                apply eq_distributes_longofwords in R as [_ ->]; eauto.
                assert (nptr_rhi: not_ptr (rs1 (preg_of rlo))).
                { destruct (rs1 (preg_of rhi)), (rs1 (preg_of rlo)); now simpl in *. }
                specialize (rs1_rs3 (preg_of rlo)).
                destruct (rs1 (preg_of rlo)); inv rs1_rs3; now simpl in *.
             ++ intros IN l' no_cross FA1 FA2.
                inv FA1; inv FA2; inv no_cross.
                exploit IH; eauto.
          -- destruct (mreg_eq R rhi); try subst R; simpl.
             ++ intros _ l' no_cross FA1 FA2.
                inversion FA1 as [| temp1 temp2 v1 vl1 CA1 FA1']; subst temp1 temp2 l'.
                inversion FA2 as [| temp1 temp2 v2 vl2 CA2 FA2']; subst temp1 temp2 v2 vl2.
                inversion CA1 as [|temp1 temp2 vhi1 vlo1 CAhi1 CAlo1 R]; subst temp1 temp2 v1.
                ...
                inv CAhi1; inv CAhi2.
                inv CAlo1; inv CAlo2.
                assert (nptr1: not_ptr (Val.longofwords (rs1 (preg_of rhi)) vlo1)).
                { inv no_cross; eauto. }
                assert (nptr2: not_ptr (Val.longofwords (rs2 (preg_of rhi)) vlo2)).
                { rewrite R; eauto. }
                apply eq_distributes_longofwords in R as [-> _]; eauto.
                assert (nptr_rhi: not_ptr (rs1 (preg_of rhi))).
                { destruct (rs1 (preg_of rhi)); now simpl in *. }
             ...
             ++ intros IN l' no_cross FA1 FA2.
                inv FA1; inv FA2; inv no_cross.
                exploit IH; eauto.
          -- destruct (mreg_eq R rlo); try subst R; simpl.
             ++ intros _ l' no_cross FA1 FA2.
                inversion FA1 as [| temp1 temp2 v1 vl1 CA1 FA1']; subst temp1 temp2 l'.
                inversion FA2 as [| temp1 temp2 v2 vl2 CA2 FA2']; subst temp1 temp2 v2 vl2.
                inversion CA1 as [|temp1 temp2 vhi1 vlo1 CAhi1 CAlo1 R]; subst temp1 temp2 v1.
                ...
                inv CAhi1; inv CAhi2.
                inv CAlo1; inv CAlo2.
                assert (nptr1: not_ptr (Val.longofwords vhi1 (rs1 (preg_of rlo)))).
                { inv no_cross; eauto. }
                assert (nptr2: not_ptr (Val.longofwords vhi2 (rs2 (preg_of rlo)))).
                { rewrite R; eauto. }
                apply eq_distributes_longofwords in R as [? ->]; eauto.
                assert (nptr_rhi: not_ptr (rs1 (preg_of rlo))).
                { destruct vhi1, (rs1 (preg_of rlo)); now simpl in *. }
                specialize (rs1_rs3 (preg_of rlo)).
                destruct (rs1 (preg_of rlo)); inv rs1_rs3; now simpl in *.
             ++ intros IN l' no_cross FA1 FA2.
                inv FA1; inv FA2; inv no_cross.
                exploit IH; eauto.
          -- intros IN l' no_cross FA1 FA2.
             inv no_cross; inv FA1; inv FA2.
             exploit IH; eauto.
    - clear -δ no_cross s_cp m1_m3 rs1_rs3_initial rs1_rs3_X2 call_args1.
      exploit call_arguments_preserved; eauto.
      replace δ with (opposite (opposite δ)) by now destruct δ.
      rewrite <- s_cp; eauto.
      intros [args' [args_args' call_args3]].
      assert (args = args') as <-; auto.
      clear -args_args' no_cross.
      induction args_args'; auto.
      inv no_cross; rewrite IHargs_args'; auto.
      inv H; now simpl in *.
  Qed.
  ...
  Proof.
    ...
    - (* Call *)
      ...
      assert (INTERNAL_CALL: Genv.type_of_call (comp_of f) (Genv.find_comp_of_block ge1 b') = Genv.CrossCompartmentCall)
        by now inv EV.
      specialize (ARGS INTERNAL_CALL);
        specialize (CALLSIG INTERNAL_CALL);
        specialize (NO_CROSS_PTR INTERNAL_CALL).
      assert (INTERNAL_CALL0: Genv.type_of_call (comp_of f0) (Genv.find_comp_of_block ge2 b'0) = Genv.CrossCompartmentCall)
        by now inv EV0.
      specialize (ARGS0 INTERNAL_CALL0);
        specialize (CALLSIG0 INTERNAL_CALL0);
        specialize (NO_CROSS_PTR0 INTERNAL_CALL0).
      ...
      assert (sig = sig0) as <-.
      { destruct CALLSIG as [fd [Hfd ->]].
        destruct CALLSIG0 as [fd0 [Hfd0 ->]].
        clear -match_W1_W3 match_W2_W3 EV EV0 Hfd Hfd0 inj_pres' inj_pres_opp_δ.
        inv EV; inv EV0. inv H1; inv H13.
        apply Genv.invert_find_symbol in H2.
        apply Genv.invert_find_symbol in H14.
        apply (symbols_inject2 _ _ _ _ _ inj_pres') in H2 as [b3 [X Y]].
        apply (symbols_inject2 _ _ _ _ _ inj_pres_opp_δ) in H14 as [b3' [X' Y']].
        ...
        inv match_gd; inv match_gd0. inv H5; inv H7; auto.
        unfold kept_genv. rewrite H14. unfold Genv.find_def in Hfd0; rewrite Hfd0. auto.
        unfold kept_genv. rewrite H2. unfold Genv.find_def in Hfd; rewrite Hfd. auto.
      }

      assert (args = args0) as <-.
      { inv EV; inv EV0; eauto.
        clear -H19 H26 NO_CROSS_PTR.
        rename H19 into A. rename H26 into B.
        revert A B. generalize (proj_sig_args sig).
        generalize dependent args0. generalize dependent args.
        induction vl; intros; inv A; inv B; auto.
        inv NO_CROSS_PTR.
        specialize (IHvl vl0 H3 vl1 tyl H4 H7). rewrite IHvl.
        inv H1; inv H6; now auto. }

      assert (comp_of f0 = comp_of f) as comp_f0_f.
      { inv EV; inv EV0. simpl in *. auto. }

      assert (EV': call_trace ge3 (comp_of f) (Genv.find_comp_of_block ge3 b3') (Vptr b3' Ptrofs.zero)
                args (proj_sig_args sig) (e :: nil)).
      { specialize (rs1_rs3' PC); rewrite NEXTPC, rs3'_PC in rs1_rs3'.
        assert (T := rs1_rs3'); inversion T.
        erewrite <- (find_comp_of_block_preserved s W1 W3) with (b := b'); eauto using delta_zero.
        ...
        - inv NO_CROSS_PTR.
          constructor; eauto.
          destruct a; simpl in *; now auto. }

      assert (comp_of f'0 = comp_of nextf) as comp_f'0_nextf.
      { inv EV; inv EV'; inv EV0. simpl in *.
        unfold Genv.find_comp_of_block in H15; rewrite NEXT_INT, NEXT_INT0 in H15.
        auto. }

      exploit update_stack_call_preserved_internal_cross; eauto.
      unfold Genv.find_comp_of_block; rewrite H0; reflexivity.
      rewrite NEXTPC; simpl; eauto.

      intros G. specialize (G ARGS ARGS0).
      rewrite comp_f0_f in *.
      exploit G; eauto. rewrite NEXTPC0, NEXTPC; simpl.
      { inv EV; inv EV0; eauto. }
      unfold Genv.find_comp_of_block; rewrite H5; simpl in *; auto.
      rewrite NEXTPC0, rs3'_PC.
      { destruct CALLSIG0 as [fd0 [Hfd0 ->]].
        clear -match_W1_W3 match_W2_W3 EV' EV0 inj_pres' H9 Hfd0.
        rename H9 into inj_pres_opp_δ'.
        inv EV'; inv EV0. inv H1; inv H12.
        apply Genv.invert_find_symbol in H2.
        apply Genv.invert_find_symbol in H13.
        apply (symbols_inject3 _ _ _ _ _ inj_pres_opp_δ') in H2 as [b3 [X Y]].
        assert (b0 = b3) by congruence; subst b3.
        econstructor; eauto.
        unfold kept_genv. rewrite H13. unfold Genv.find_def in Hfd0; rewrite Hfd0. auto. }
      specialize (rs1_rs3' PC); symmetry; eapply find_comp_preserved; eauto using delta_zero.
      congruence.
      { inv strong_s1_s3. simpl.
        inv COMP1. rewrite <- H16, H.
        simpl; unfold Genv.find_comp_of_block; rewrite H0; auto. }
      { rewrite NEXTPC; simpl.
        clear -EV.
        inv EV; eauto.
        unfold Genv.type_of_call in H0.
        destruct flowsto_dec; try congruence. auto. }
      ...
      intros (j1' & j2' & dra1 & dsp1 & dra2 & dsp2 & st3' & rs3'' & m3'' & -> & ->
              & STUPD3 & inj_pres1 & inj_pres2 & mrel1 & mrel2 & incr1 &incr2 & strel &
                eq_pc & side1 & side2 & call_args3).


      assert (exists f', Genv.find_def ge3 b3' = Some (Gfun (Internal f')) /\
            (Genv.type_of_call (comp_of f) (Genv.find_comp_of_block ge1 b') = Genv.CrossCompartmentCall ->
             sig = fn_sig f')) as [f' [? ?]].
      { specialize (rs1_rs3' PC) as G'.
        rewrite NEXTPC, rs3'_PC in G'. inv G'.
        exploit defs_inject; eauto. intros [gd' [? [-> [MGD kept]]]].
        inv MGD; inv H12; eauto.
        eexists; split; eauto.
        intros ?.
        destruct CALLSIG as [? [? ?]]; subst.
        assert (x = (Internal {| fn_comp := cp; fn_sig := sig0; fn_code := code |}))
          as -> by congruence.
        reflexivity. }

      eexists (State st3' (invalidate_call rs3'' sig) m3'' _),
        j1', j2'; split; [| split; [| split; [| split]]]; try assumption.
      + econstructor; [| now eapply star_refl | now traceEq].
        ...
        * eapply allowed_call_preserved with (v := Vptr b' Ptrofs.zero); eauto using delta_zero.
          ...
          specialize (rs1_rs3' PC) as inj_pc. rewrite NEXTPC, rs3'_PC in inj_pc.
          inv inj_pc; try congruence. exploit (delta_zero s ge1 ge3); eauto; intros ->.
      + destruct (side_eq (s (Genv.find_comp_in_genv ge1 (rs' PC))) δ) as [e1 | n1].
        * left; split.
          -- econstructor; eauto.
             ++ econstructor.
             ...
             ++ econstructor.
                ...
                symmetry. eapply find_comp_preserved; eauto using delta_zero. congruence.
                rewrite NEXTPC; simpl; unfold Genv.find_comp_of_block; rewrite NEXT_INT.
                eapply no_bottom1; eauto.
             ++ rewrite NEXTPC; simpl; unfold Genv.find_comp_of_block; rewrite NEXT_INT.
                eapply no_top1; eauto.
             ++ rewrite NEXTPC; simpl; unfold Genv.find_comp_of_block; rewrite NEXT_INT.
                eapply no_bottom1; eauto.
             ++ inv strel; eauto. inv H18; eauto.
             ++ eapply regset_rel_invalidate_call. eapply side1. eauto.
          -- econstructor; eauto.
             Local Opaque opposite.
             ++ econstructor.
                erewrite invalidate_call_PC, update_stack_call_PC, NEXTPC0; simpl; eauto.
                unfold Genv.find_comp_of_block. rewrite NEXT_INT0. auto.
                eapply no_bottom1; eauto.
             ++ econstructor.
                erewrite invalidate_call_PC, update_stack_call_PC; simpl; eauto.
                erewrite <- (find_comp_preserved s W1 W3) with (v := rs' PC); eauto using delta_zero.
                rewrite NEXTPC; simpl; unfold Genv.find_comp_of_block; rewrite NEXT_INT; auto.
                congruence.
                eapply no_bottom1; eauto.
             ++ rewrite NEXTPC in e1. simpl in e1.
                unfold Genv.find_comp_of_block in e1; rewrite NEXT_INT in e1.
                now destruct δ.
        * right; split.
          -- econstructor; eauto.
             Local Opaque opposite.
             ++ econstructor.
                erewrite invalidate_call_PC, update_stack_call_PC, NEXTPC; simpl; eauto.
                unfold Genv.find_comp_of_block; rewrite NEXT_INT; auto.
                eapply no_bottom1; eauto.
             ++ econstructor.
                erewrite invalidate_call_PC, update_stack_call_PC; simpl; eauto.
                erewrite <- (find_comp_preserved s W1 W3) with (v := rs' PC); eauto using delta_zero.
                ...
                congruence.
                eapply no_bottom1; eauto.
             ++ rewrite NEXTPC in n1; simpl in n1; unfold Genv.find_comp_of_block in n1; rewrite NEXT_INT in n1.
                clear -n1. simpl in *.
                now destruct δ, (s (has_comp_function nextf)).
          -- replace (has_comp_function f0) with (comp_of f).
             ...
             ++ rewrite NEXTPC in n1; simpl in n1; unfold Genv.find_comp_of_block in n1; rewrite NEXT_INT in n1.
                clear -n1. simpl in *.
                now destruct δ, (s (has_comp_function nextf)).
             ++ inv strel. inv H18; simpl.
                rewrite NEXTPC; simpl; unfold Genv.find_comp_of_block; rewrite NEXT_INT; eauto.
                rewrite NEXTPC; simpl; unfold Genv.find_comp_of_block; rewrite NEXT_INT; eauto.
             ++ eapply side2.
                clear -n1.
                now destruct δ, (s (Genv.find_comp_in_genv ge1 (rs' PC))).
    ...
    - (* Return *)
      ...
      assert (exists frame3 st3', st3 = frame3 :: st3' /\
                               stackframe_rel s ge1 ge2 ge3 δ j__δ j__oppδ m m0 m3 frame1 frame2 frame3 /\
                               stack_rel s cp_main ge1 ge2 ge3 δ j__δ j__oppδ m m0 m3 st' st'0 st3')
      ...
      { inv frame_rel; eauto.
        ...
        - simpl. inv H10.
          erewrite <- find_comp_of_block_preserved; eauto using delta_zero.
          rewrite <- EQ_CP. subst cp'0. reflexivity. }

      assert (exists m3', Some m3' = match asm_parent_sp (frame3 :: st3') with
                                | Vptr bsp _ => Mem.set_perm m3 bsp Freeable
                                | _ => None
                                end /\
            mem_rel s ge1 ge3 j__δ δ m' m3' /\
     mem_rel s ge2 ge3 j__oppδ (opposite δ) m'0 m3' /\
     stack_rel s cp_main ge1 ge2 ge3 δ j__δ j__oppδ m' m'0 m3' st' st'0 st3') as [m3' [eq_m3' A]].
      { inv frame_rel; simpl in *.
        - destruct sp1; try congruence. inv H11; eauto.
          destruct sp2; try congruence.
          symmetry in MAKE_FREEABLE0. symmetry in MAKE_FREEABLE.
          exploit (set_perm_preserves_rel' s W1 W2 W3); simpl; eauto using match_prog_unique.
          + { simpl. eauto. }
          + { (* Hnotgt: j__oppδ doesn't map to the SP block in m3 *)
              eapply image_disjoint.
              - exact m_m3.
              - exact m1_m3.
              - exact H14.
              - simpl in *. intros gd. congruence. }
          + { (* Hb2none: SP block not in domain of j__oppδ *)
              destruct (j__oppδ b0) as [[b'' d']|] eqn:Hj; auto.
              exfalso.
              assert (j__oppδ b0 <> None) as Hne by congruence.
              destruct (proj1 (same_dom _ _ _ _ _ _ _ m1_m3 b0) Hne) as [[Hside _] | [fd Hfd]].
              - simpl in Hside, COMP_SP. rewrite COMP_SP in Hside.
                destruct (s (Genv.find_comp_of_block ge3 b3)); discriminate.
              - simpl in FD0. rewrite FD0 in Hfd. discriminate. }
          + intros [? [? [? [? ?]]]].
            eexists; split; [| split; [| split]]; eauto.
        - destruct sp1; try congruence.
          destruct sp2; try congruence. inv H11; eauto.
          symmetry in MAKE_FREEABLE0. symmetry in MAKE_FREEABLE.
          eapply stack_rel_comm in st_rel.
          assert (Hnotgt: forall b4 d, j__δ b4 <> Some (b5, d)).
          { eapply (image_disjoint s (opposite δ)).
            - exact m1_m3.
            - replace (opposite (opposite δ)) with δ by (now destruct δ).
              exact m_m3.
            - exact H15.
            - simpl in *. intros gd. congruence. }
          assert (Hb2none: j__δ b = None).
          { destruct (j__δ b) as [[b'' d']|] eqn:Hj; auto.
            exfalso.
            assert (j__δ b <> None) as Hne by congruence.
            destruct (proj1 (same_dom _ _ _ _ _ _ _ m_m3 b) Hne) as [[Hside _] | [fd Hfd]].
            - simpl in Hside, COMP_SP. rewrite COMP_SP in Hside.
              destruct δ; discriminate.
            - simpl in FD3. rewrite FD3 in Hfd. discriminate. }
          eapply (set_perm_preserves_rel' s W2 W1 W3) in st_rel;
            simpl; eauto using match_prog_unique; try (now destruct δ).
          destruct st_rel as [? [? [? [? ?]]]]. eexists; split; eauto.
          split; eauto.
          now destruct δ. split; eauto. eapply stack_rel_comm in H16; eauto.
          now destruct δ. }
      ...
      + econstructor; [| now eapply star_refl | now traceEq].
        econstructor; eauto.
        { intros b ofs DRA.
          destruct (Genv.find_def ge3 b) as [gd|] eqn:FD; [exfalso|auto].
          inv frame_rel; simpl in DRA; inv DRA;
            match goal with
            | [ INJ: Val.inject ?j (Vptr ?dra Ptrofs.zero) (Vptr b Ptrofs.zero) |- _ ] =>
                destruct (side_eq _ _) eqn:SD in INJ;
                [ inv INJ;
                  exploit (defs_rev_inject _ _ _ _ _ inj_pres_δ); eauto;
                  intros (? & CONTRA & _); specialize (NO_DEF _ _ eq_refl); congruence
                | inv INJ;
                  exploit (defs_rev_inject _ _ _ _ _ inj_pres_opp_δ); eauto;
                  intros (? & CONTRA & _); specialize (NO_DEF0 _ _ eq_refl); congruence ]
            end. }
             ...
             ++ intros x.
                ...
                ** Local Opaque all_mregs.
                   ...
                   --- unfold return_value in inj_res, NO_CROSS_PTR, NO_CROSS_PTR0, EV0, EV.
                       ...
                       +++ pose proof (filter_all_mregs_find_two rhi rlo) as CASE.
                         destruct CASE as [CASE | [CASE | CASE]];
                           rewrite <- CASE in *.
                         *** simpl in i. destruct i as [| []]; try contradiction. subst x.
                             simpl in *.
                             destruct flowsto_dec; try congruence.
                             specialize (NO_CROSS_PTR eq_refl).
                             specialize (NO_CROSS_PTR0 eq_refl).
                         ...
                         *** simpl in i. destruct i as [| []]; try contradiction. subst x.
                             ...
                             rewrite R in *. clear R.
                             eapply inject_distributes_longofwords in inj_res as [? ?].
                             { inv H0; auto. rewrite <- H1 in NO_CROSS_PTR; destruct (rs0 (preg_of rhi)); contradiction. }
                             eauto.  inv inj_res; try now auto.
                             rewrite <- H in *. contradiction.
                             rewrite <- H0 in *. contradiction.
                             subst x.
    ...
    - (* Builtin *)
      ...
      + assert (R: nextinstr (set_res res vres
        ...
        { clear -RES_NOT_PC0.
          destruct RES_NOT_PC0 as [reg ?]; subst res0. Simpl.
          rewrite Asmgenproof0.set_res_other; eauto.
    ...
    - (* builtin / external call *)
      ...
      exploit weak_equivalence_inv1; eauto. intros (st2 & rs2 & m2 & m2_m3 & A).
      injection A; intros -> -> ->. clear A.


      assert (SAMECOMP: comp_of f = comp_of f0).
      { inv weak_s2_s3; eauto.
        inv COMP1; inv COMP2; eauto.
        rewrite H4 in *; simpl in *; unfold Genv.find_comp_of_block in *; rewrite H5 in *.
        rewrite eq_pc' in *.

        simpl in H17. eapply Genv.find_funct_ptr_iff in find_funct.
        eapply Genv.find_funct_ptr_find_comp_of_block in find_funct.
        rewrite find_funct in H17; simpl in *; auto. }

      exploit exec_instr_preserves_weak; [| | | eassumption | | | |];
        simpl in *; rewrite <- SAMECOMP in *; eauto.
      intros [j__oppδ' [inj_pres_opp_δ' [m2'_m3 st_rel']]].
      clear inj_pres_opp_δ m2_m3 st_rel.
      ...
      exploit extcall_preserves_mem_rel_opp_side1; [| | | eassumption | | eassumption |];
        try now inv weak_s2_s3; eauto.
      { simpl in side_f. rewrite side_f. now destruct δ. }
      { eapply stack_rel_comm; eauto. }
      intros [m'0_m3' st_rel'''].
    ...
    - (* external_call / builtin *)
      ...
      exploit weak_equivalence_inv1; eauto. intros (st2 & rs2 & m2 & m2_m3 & A).
      injection A; intros <- <- <-.
      exploit exec_instr_preserved; eauto.
      intros [j__δ' [rs3' [m3' [exec_instr3 [inj_pres_δ' [m1'_m3' [m2_m3' [rs1'_rs3' [st_rel' inj_incr]]]]]]]]].
      clear m1_m3 rs1_rs3 st_rel.
      ...
      { inv weak_s2_s3; eauto.
        inv COMP1; inv COMP2; eauto.
        rewrite H6 in *; simpl in *; unfold Genv.find_comp_of_block in *; rewrite H7 in *.
        rewrite eq_pc' in *.
      ...
      exploit (extcall_preserves_mem_rel_opp_side1 s (comp_of f) cp_main ge2 ge1 ge3 j__oppδ j__δ'
                 (opposite δ)
                 m0 m'0 m' m3'); eauto.
      { rewrite side_f; now destruct δ. }
      { eapply stack_rel_comm; eauto. }
    ...
    - (* External call *)
      ...
      { inv weak_s2_s3; eauto.
        ...
        simpl in H19. eapply Genv.find_funct_ptr_iff in find_funct.
        eapply Genv.find_funct_ptr_find_comp_of_block in find_funct.
        rewrite find_funct in H19; simpl in *; auto. }
      ...
      exploit (extcall_preserves_mem_rel_opp_side1 s (comp_of f) cp_main ge2 ge1 ge3 j__oppδ' j__δ' (opposite δ)
                 m'0 m''0 m' m3'); eauto;
        try now inv weak_s2_s3; eauto.
      { simpl; rewrite side_f; now destruct δ. }
      { eapply stack_rel_comm; eauto. }
      intros [m'0_m3 st_rel'''].
...
Section Simulation.
  ...
  Proof.
    ...
    - intros b b' d Hmap. unfold init_meminj in Hmap.
      destruct (Genv.invert_symbol ge1 b) eqn:?; try discriminate.
      destruct (kept_genv s ge1 Left i) eqn:?; try discriminate.
      destruct (Genv.find_symbol ge3 i) eqn:?; try discriminate.
      inv Hmap.
      right. eapply Genv.find_symbol_find_def_inversion; eauto.
  Qed.
  ...
  Proof.
    ...
    - intros. unfold init_meminj in *.
      ...
      assert (b0 = b4) by congruence. contradiction.
    - intros b b' d Hmap. unfold init_meminj in Hmap.
      destruct (Genv.invert_symbol ge2 b) eqn:?; try discriminate.
      destruct (kept_genv s ge2 Right i) eqn:?; try discriminate.
      destruct (Genv.find_symbol ge3 i) eqn:?; try discriminate.
      inv Hmap.
      right. eapply Genv.find_symbol_find_def_inversion; eauto.
  Qed.

  Let single_L1 := semantics_single_events W1.
  Let single_L2 := semantics_single_events W2.
  Let single_L3 := semantics_single_events W3.
