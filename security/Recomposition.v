...
meminj_preserves_globals which will allow us to prove preservation of events.
  ...
  | weak_equivalence_State: forall st st' rs rs' m m' cp cp',
      Genv.find_comp_in_genv ge (rs PC) = cp ->
      Genv.find_comp_in_genv ge' (rs' PC) = cp ->
      s cp = opposite δ ->
      mem_rel ge ge' j δ m m' ->
  ...
  | weak_equivalence_State_ReturnState: forall st st' rs rs' m m' cp,
      ...
      s cp = opposite δ ->
      mem_rel ge ge' j δ m m' ->
      weak_equivalence ge ge' j δ (State st rs m cp) (ReturnState st' rs' m' cp)
  | weak_equivalence_ReturnState_State: forall st st' rs rs' m m' rec_cp,
      (* careful, the current comp in a returnstate is given by [callee_comp] *)
      s rec_cp = opposite δ ->
      Genv.find_comp_in_genv ge' (rs' PC) = rec_cp ->
      mem_rel ge ge' j δ m m' ->
      weak_equivalence ge ge' j δ (ReturnState st rs m rec_cp) (State st' rs' m' rec_cp)
  .
...
Ltac simpl_before_exists :=
  repeat (Simpl_all;
          ...
          | H: Mem.alloc ?m1 ?cp ?lo1 ?hi1 = (?m1', ?b1),
              m1_m3: mem_rel _ _ _ ?j__δ ?δ ?m1 ?m3,
                m2_m3: mem_rel _ _ _ ?j__oppδ (opposite ?δ) ?m2 ?m3,
                  ...
                  rs1_rs3: regset_rel _ _ _ |- _ =>
                    (* inj_pres : meminj_preserves_globals _ ?δ _ _ ?j__δ |- _ => *)
              idtac "alloc case";
              ...
              eapply (alloc_preserves_rel _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ agr addr m1_m3 m2_m3 rs1_rs3) in H as
                  [j__δ' [m3' [b3 [alloc3 [agr' [addr' [m1'_m3' [m2_m3' [? [proj incr]]]]]]]]]];
              idtac "done with alloc";
              clear m1_m3 rs1_rs3 m2_m3 agr addr
          | H: ?s ?cp = ?δ -> _,
              side_cp: ?s ?cp = ?δ |- _ =>
          ...
          | H: Mem.store ?ch ?m1 ?b1 ?ofs (?rs1 ?r) ?cp = Some ?m1',
              ...
              idtac "store case";
              let m3' := fresh "m3" in
              eapply (store_preserves_rel _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _  ptr_inj m1_m3 m2_m3 (rs1_rs3 r)) in H as
                  [m3' [? [? ?]]];
          ...
          | H: Mem.free ?m1 ?b1 ?lo ?hi ?cp = Some ?m1',
              ...
              idtac "free case";
              let m3' := fresh "m3" in
              eapply (free_preserves_rel _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ ptr_inj m1_m3 m2_m3) in H as
                  [m3' [? [? ?]]];
...
Section Theorems.
  ...
  Proof.
    intros j__δ cp cp' v v' args args' sig t (* inj_pres *) agr delta_zero inj_v inj_args NPTR EV.
    inv EV.
    - constructor; auto.
    - specialize (NPTR H).
      inv inj_v; eauto.
  ...
  Proof.
    intros until st3.
    intros side_cp inj_pres addressable m1_m3 m2_m3 rs1_rs3 st1_st3 exec.
    ...
    - (eexists_and_split
      ...
      exploit (symbol_address_inject s δ W1 W3 (init_meminj W1 W3) symb Ptrofs.zero);
        eauto using init_meminj_preserves_globals.
      ...
      econstructor; eauto.
      { assert (exists b1 delta', j__δ b = Some (b1, delta')) as (b1 & delta' & ?).
        { unfold Genv.symbol_address in FS.
          destruct (Genv.find_symbol ge1 symb) eqn:?; eauto; try congruence.
          inv FS.
          exploit addressable; eauto. left; eauto. rewrite Heqo.
          apply Pos.eqb_eq in Heqb0. simpl in *; congruence. }
        specialize (inj_pres b b1 b2 delta' delta H H3) as [? ?]; subst; eauto. }
      destruct (Genv.find_def ge1 b) as [[] |] eqn:?; eauto.
      exploit defs_inject; eauto using init_meminj_preserves_globals.
      intros ([] & ? & _ & ? & ?). rewrite H.
      econstructor; eauto.
      { assert (exists b1 delta', j__δ b = Some (b1, delta')) as (b1 & delta' & ?).
        { unfold Genv.symbol_address in FS.
          destruct (Genv.find_symbol ge1 symb) eqn:?; eauto; try congruence.
          inv FS.
          exploit addressable; eauto. }
        specialize (inj_pres b b1 b2 delta' delta H4 H3) as [? ?]; subst; eauto. }
      inv H0.
      intros ????. exploit init_meminj_invert; eauto. intros []; eauto.
    - (eexists_and_split
         ltac:(fun j rs1 rs3 rs1_rs3 m1 m3 m1_m3 =>
                 (simpl; try reflexivity; try eassumption;
                  solve_simple_regset_rel j rs1 rs3 rs1_rs3 m1 m3 m1_m3; try reflexivity))).
      ...
      inv H0.
      intros ????. exploit init_meminj_invert; eauto. intros []; eauto.
    (* - (* Not sure why this one breaks *) *)
    (*   simpl_before_exists. *)
    (*   (eexists_and_split *)
    (*      ltac:(fun j rs1 rs3 rs1_rs3 m1 m3 m1_m3 => *)
    (*              (simpl; try reflexivity; try eassumption; *)
    (*               solve_simple_regset_rel j rs1 rs3 rs1_rs3 m1 m3 m1_m3; try reflexivity))). *)
    (*   assert (m4 = m5) by congruence; subst; eauto. *)
    (*   assert (m4 = m5) by congruence; subst; eauto. *)
    (*   congruence. *)
    - (eexists_and_split
         ltac:(fun j rs1 rs3 rs1_rs3 m1 m3 m1_m3 =>
      ...
      exploit (symbol_address_inject s δ W1 W3 (init_meminj W1 W3) id ofs);
        eauto using init_meminj_preserves_globals.
      rewrite FS. intros H. inv H.
      erewrite <- (find_comp_of_block_preserved s W1 W3);
        eauto using init_meminj_preserves_globals.
      ...
      { assert (exists b1 delta', j__δ b = Some (b1, delta')) as (b1 & delta' & ?).
        { unfold Genv.symbol_address in FS.
          destruct (Genv.find_symbol ge1 id) eqn:?; eauto; try congruence.
          inv FS.
          exploit addressable; eauto. left; eauto. rewrite Heqo.
          apply Pos.eqb_eq in Heqb0. simpl in *; congruence. }
        specialize (inj_pres b b1 b2 delta' delta H H3) as [? ?]; subst; eauto. }
    ...
    - (eexists_and_split
      ...
      Unshelve.
      all: try assumption.
      all: now eapply match_prog_unique; eauto.
  Qed.
  ...
  Proof.
    ...
    - (* free *)
      simpl_before_exists.
      exists j__oppδ; split; [| split; [| split]]; auto.
      constructor.
  ...
  Proof.
    ...
    (* assert (exists j, meminj_preserves_globals s δ W1 W3 j /\ *)
    (*              agrees_with j__δ j /\ *)
    (*              Mem.inject j m1 m3 /\ *)
    (*              Val.inject_list j vargs vargs') as [j [? [? [? ?]]]]. *)
    (* { exists (fun b => match init_meminj W1 W3 b with *)
    (*            | Some p => Some p *)
    (*            | None => j__δ b *)
    (*            end). *)
    (*   pose proof (init_meminj_preserves_globals s δ W1 W3 match_W1_W3) as X. *)
    (*   split; [| split; [| split]]. *)
    (*   - constructor. *)
    (*     + intros. destruct (init_meminj W1 W3 b) as [[] |] eqn:G; try congruence. *)
    (*       inv H. exploit symbols_inject1; eauto. *)
    (*       unfold init_meminj in G. *)
    (*       exploit (transform_find_symbol_1 s δ W1 W3); eauto. intros [? ?]. *)
    (*       apply Genv.find_invert_symbol in H0. rewrite H0 in G. *)
    (*       rewrite H1 in G. congruence. *)
    (*     + intros. exploit symbols_inject2; eauto. *)
    (*       intros [? [-> ->]]. eauto. *)
    (*     + intros. exploit symbols_inject3; eauto. *)
    (*       intros [? [A B]]. eexists; rewrite A, B; eauto. *)
    (*     + intros. destruct (init_meminj W1 W3 b) as [[] |] eqn:G; try congruence. *)
    (*       inv H. exploit defs_inject; eauto. *)
    (*       exploit Genv.find_def_find_symbol_inversion; eauto. intros [? ?]. *)
    (*       exploit (transform_find_symbol_1 s δ W1 W3); eauto. intros [? ?]. *)
    (*       apply Genv.find_invert_symbol in H1. *)
    (*       unfold init_meminj in G. rewrite H1, H2 in G. congruence. *)
    (*     + intros. destruct (init_meminj W1 W3 b) as [[] |] eqn:G; try congruence. *)
    (*       inv H. exploit defs_rev_inject; eauto. *)
    (*       admit. *)
    (*   - intros ??????. *)
    (*     destruct (init_meminj W1 W3 b) as [[] |] eqn:?; try congruence. *)
    (*     intros ?. inv H0. eapply inj_pres; eauto. *)
    (*     intros. rewrite H0 in H. inv H. eauto. *)
    (*   - admit. *)
    (*   - admit. } *)
    ...
    (* exists (fun b => match Genv.find_comp_of_block ge1 b with *)
    (*          | Some cp => if side_eq (s cp) δ then *)
    (*                        j__δ' b *)
    (*                      else match Genv.find_def ge1 b with *)
    (*                           | Some (Gfun _) => j__δ' b *)
    (*                           | _ => None *)
    (*                           end *)
    (*          | None => j__δ' b *)
  ...
  Proof.
    ...
    destruct (Genv.type_of_call cp cp'); auto.
    destruct orb; auto.
  Qed.
  ...
  Lemma regset_rel_invalidate_cross_return: forall j rs1' rs3' cp cp',
      regset_rel j rs1' rs3' ->
      regset_rel j (invalidate_cross_return rs1' cp cp') (invalidate_cross_return rs3' cp cp').
  Proof.
    intros ? ? ? ? ? H.
    intros r. specialize (H r).
  ...
  Proof.
    (* simpl. *)
    intros s1 s1' H s2 s3 j__δ j__oppδ agr addr st_rel strong_s1_s3 weak_s2_s3.
    ...
    - exploit strong_equiv_state_internal_inv; eauto.
      ...
      intros (j__δ' & rs3' & m3' & exec_instr' & agr' & addr' & m1_m3' & m2_m3' & rs1_rs3' & st_rel').
      ...
      + inv weak_s2_s3; inv A; econstructor; eauto.
        * erewrite <- (agrees_with_init_meminj_find_comp_preserved _ W1 W3 _ _ j__δ');
            eauto; try congruence.
          rewrite <- same_comp.
          assert (exists cp, Genv.find_comp ge1 (rs PC) = Some cp) as [cp' G].
          { rewrite H0; simpl.
            unfold Genv.find_comp_of_block.
            rewrite H1. eauto. }
          eapply (agrees_with_init_meminj_find_comp_preserved _ W1 W3 _ _ j__δ (rs PC))
                   in G as G'; eauto.
          assert (cp = cp') by congruence; now subst.
          congruence.
        * erewrite <- (agrees_with_init_meminj_find_comp_preserved _ W1 W3 _ _ j__δ');
            eauto; try congruence.
          rewrite <- same_comp.
          ...
          eapply (agrees_with_init_meminj_find_comp_preserved _ W1 W3 _ _ j__δ (rs PC))
                   in G as G'; eauto.
          assert (rec_cp = cp') by congruence; now subst.
          congruence.
    ...
    - exploit strong_equiv_state_internal_inv; eauto.
      ...
      { rewrite invalidate_cross_call_PC,
          invalidate_call_PC in same_comp. rewrite <- same_comp, H0; simpl.
        erewrite Genv.find_def_find_comp_of_block; eauto. reflexivity. }
      eapply update_stack_call_preserved_internal with (j__δ := j__δ') (st3 := st3) in STUPD as [? STUPD]; eauto using delta_zero; try congruence.
      subst st'.
      exploit call_arguments_preserved; eauto.
      intros [args' [inj_args call_args']].

      exists (State st3 (invalidate_call rs3' sig) m3'), j__δ';
        split; [| split; [| split; [| split; [| split]]]].
      + econstructor; [| now eapply star_refl | now traceEq].
        eapply exec_step_internal_call; eauto.
        * eapply allowed_call_preserved with (v := Vptr b' Ptrofs.zero)
          (j__δ := init_meminj W1 W3);
            eauto using delta_zero, init_meminj_preserves_globals.
          { intros ????. exploit init_meminj_invert; eauto. intros []; eauto. }
          congruence.

          assert (exists b delta, init_meminj W1 W3 b' = Some (b, delta)) as [? [? ?]].
          { unfold init_meminj.
            unfold Genv.find_comp_of_block in NEXTCOMP.
            destruct (Genv.find_def ge1 b') eqn:G; try congruence.
            exploit find_def_find_symbol; eauto.
            intros [? G']. apply Genv.find_invert_symbol in G'. rewrite G'.
            apply Genv.invert_find_symbol in G'.
            exploit (transform_find_symbol_1 s δ W1 W3); eauto.
            intros [? G'']. rewrite G''.
            exploit Genv.find_def_find_symbol_inversion; eauto.
            { now destruct match_W1_W3. } }

          specialize (rs1_rs3' PC) as inj_pc. rewrite NEXTPC, rs3'_PC in inj_pc.
          inv inj_pc; try congruence. exploit (delta_zero s ge1 ge3); eauto; intros ->.
          exploit agr'; eauto. intros [? ?]. subst.
          econstructor; eauto.
        ...
        * rewrite <- H, NEXTPC; simpl.
          ...
          simpl.
          (* assert (exists cp, Genv.find_comp_of_block ge1 b' = Some cp) as [cp G]. *)
          (* { rewrite NEXTCOMP. eauto. } *)
          exploit (agrees_with_init_meminj_find_comp_of_block_preserved s W1 W3); eauto.
          intros ->. rewrite NEXTCOMP. auto.
        * intros.
      ...
      + eauto.
      + eauto.
      + econstructor; eauto using regset_rel_invalidate_call.
        rewrite invalidate_cross_call_PC, invalidate_call_PC; eauto.
        unfold invalidate_cross_call.
        replace cp' with (has_comp_function f).
        unfold Genv.type_of_call; rewrite Pos.eqb_refl; eauto using regset_rel_invalidate_call.
        rewrite NEXTPC in H. simpl in H. rewrite NEXTCOMP in H. now inv H.
      + inv weak_s2_s3; inv A; econstructor; eauto.
        * rewrite invalidate_cross_call_PC, invalidate_call_PC in *.
          erewrite <- (agrees_with_init_meminj_find_comp_preserved _ W1 W3 _ _ j__δ');
            eauto; try congruence.
          rewrite <- same_comp.
          assert (exists cp, Genv.find_comp ge1 (rs PC) = Some cp) as [cp'' G].
    ...
    - exploit strong_equiv_state_internal_inv; eauto.
      ...
      + inv weak_s2_s3; inv A.
        * econstructor; eauto.
          rewrite H7, <- H9.
          assert (H: Genv.find_comp ge1 (rs PC) = Some (comp_of f)) by eauto.
          eapply (agrees_with_init_meminj_find_comp_preserved s W1 W3 _ _ j__δ) in H; eauto.
          congruence.
    ...
    - exploit strong_equiv_returnstate_inv; eauto.
      ...
      (* inv weak_s2_s3. *)
      rewrite invalidate_cross_return_PC, invalidate_return_PC in same_comp.
      symmetry in same_comp.
      ...
      exists (State st3 (invalidate_cross_return (invalidate_return rs3 (sig_of_call st3)) rec_cp rec_cp) m3), j__δ; split; [| split; [| split; [| split; [| split]]]].
      + econstructor; [| now eapply star_refl | now traceEq].
      ...
      + econstructor; try rewrite same_sg; eauto using regset_rel_invalidate_return.
        rewrite invalidate_cross_return_PC, invalidate_return_PC; eauto.
        now inv strong_s1_s3.
        intros ?. rewrite invalidate_cross_return_int.
        rewrite invalidate_cross_return_int.
        exploit regset_rel_invalidate_return; eauto.
    ...
    (*              Val.inject_list j vargs vl' /\ *)
    (*              mem_rel s ge1 ge3 j δ m m3 /\ *)
    (*              regset_rel j rs rs3 /\ *)
    (*              stack_rel s ge3 δ j j__oppδ st st2 st3) as *)
    (*     (j & ? & ? & ? & ? & ?). *)
    (*   { admit. } *)
    ...
    (*   eexists; exists j'; split; [| split; [| split; [| split; [| split]]]]. *)
    (*   + econstructor; [| now eapply star_refl | now traceEq]. *)
    (*     eapply exec_step_builtin; eauto. *)
    (*     { destruct ef; simpl in *; auto. *)
    (*       - unfold visible_fo in *. destruct ECC as [ECC1 ECC2]. *)
    (*         admit. *)
    (*       - admit. *)
    (*       - admit. *)
    (*       - admit. *)
    ...
    (*   exploit external_call_inject_left; eauto using partial_mem_inject. *)
    (*   intros (j__δ' & vres' & m3' & extcall' & inj_res & unchanged1 & unchanged2 & incr & sep & inj_pres' & m'_m3' & m2_m3' & rs_rs3' & st_rel'). *)
    (*   eexists; exists j__δ'; split; [| split; [| split; [| split]]]. *)
    (*   + econstructor; [| now eapply star_refl | now traceEq]. *)
      ...
      forall (s1 s3: state) j__δ j__oppδ,
        ...
        exists j__oppδ',
          ...
          stack_rel s ge3 δ j__δ j__oppδ' (stack_of_state s1) (stack_of_state s2') (stack_of_state s3) /\
                    ...
                    weak_equivalence s ge2 ge3 j__oppδ' (opposite δ) s2' s3.
  Proof.
    intros s2 s2' H s1 s3 j__left j__right inj_pres1 inj_pres2 addr1 addr2 st_rel strong_s1_s3 weak_s2_s3.
    exploit step_E0_same_cp; eauto. intros same_comp.
    simpl in H.
    inv H; simpl in same_comp.
    ...
    - exploit weak_equivalence_inv; eauto.
      ...
      intros (j__oppδ' & agr' & addr' & m'_m3 & st_rel').
      eexists.
      repeat (split; eauto).
      inv weak_s2_s3; inv B;
    ...
    - exploit weak_equivalence_inv; eauto.
      ...
      { inv weak_s2_s3; inv B.
        ...
        - rewrite H0 in H7; simpl in H7;
          ...
          now destruct δ. }
      exploit exec_instr_preserves_weak; eauto.
      intros (j__right' & agr' & addr' & m'_m3 & st_rel').
    ...
    - exploit weak_equivalence_inv; eauto.
      ...
      repeat (split; eauto).
      inv weak_s2_s3; inv B.
      + rewrite H7 in same_comp; inv same_comp; auto.
        constructor; eauto.
      + rewrite H7 in same_comp; inv same_comp; auto.
        constructor; eauto.
    ...
    - exploit weak_equivalence_inv; eauto.
      ...
      rewrite invalidate_cross_return_PC in same_comp.
      rewrite invalidate_return_PC in same_comp; rewrite NEXTCOMP in same_comp; inv same_comp.
      ...
      inv weak_s2_s3; inv B;
        econstructor; eauto.
      rewrite invalidate_cross_return_PC, invalidate_return_PC. auto.
      rewrite invalidate_cross_return_PC, invalidate_return_PC. auto.
  ...
  Proof.
    intros s1 s1' s2 s2' e step1 step2 s3 j__δ j__oppδ agr1 agr2 addr1 addr2
      (* inj_pres_δ inj_pres_opp_δ *)
    ...
    - (* Call *)
      ...
      intros (j__oppδ' & agr2' & addr2' & m'0_m3' & st_rel'').
      ...
      { exploit CALLSIG; eauto.
        ...
        intros [fd0 [Hfd0 ->]].
        (* clear -STUPD STUPD0. *)
        clear -match_W1_W3 match_W2_W3 EV EV0 Hfd Hfd0 agr' agr2'.
        inv EV; inv EV0. inv H1; inv H12.
        apply Genv.invert_find_symbol in H2.
        apply Genv.invert_find_symbol in H13.
        eapply (symbols_inject2 _ _ _ _ (init_meminj W1 W3)) in H2 as [b3 [X Y]];
          eauto using init_meminj_preserves_globals.
        eapply (symbols_inject2 _ _ _ _ (init_meminj W2 W3)) in H13 as [b3' [X' Y']];
          eauto using init_meminj_preserves_globals.
        ...
        eapply (defs_inject _ _ _ _  (init_meminj W1 W3)) in Hfd as [gd [find_gd [_ [match_gd ?]]]];
          eauto using init_meminj_preserves_globals.
        eapply (defs_inject _ _ _ _ (init_meminj W2 W3)) in Hfd0 as [gd0 [find_gd0 [_ [match_gd0 ?]]]];
          eauto using init_meminj_preserves_globals.
      ...
      exists (State (Stackframe b3 sig (rs3' X2) (Ptrofs.add ofs Ptrofs.one) :: st3)
        ...
        j__δ', j__oppδ'; split; [| split; [| split; [| split; [| split; [| split]]]]]; try assumption.
      + econstructor; [| now eapply star_refl | now traceEq].
        eapply exec_step_internal_call; eauto.
        * eapply allowed_call_preserved with
            (j__δ := init_meminj W1 W3)
            (v := Vptr b' Ptrofs.zero); eauto using delta_zero, init_meminj_preserves_globals.
          { intros ????. exploit init_meminj_invert; eauto. intros []; eauto. }
          congruence.
        ...
        * intros.
          ...
          inv rs1_rs3'.
          eapply (defs_inject _ _ _ _ (init_meminj W1 W3)) in Hfd
              as [gd [find_gd [_ [match_gd ?]]]]; eauto using init_meminj_preserves_globals.
          inv match_gd.
          inv H12; eexists; split; eauto.
          ...
          { unfold init_meminj.
            ...
            intros [? G']. apply Genv.find_invert_symbol in G'. rewrite G'.
            apply Genv.invert_find_symbol in G'.
            exploit (transform_find_symbol_1 s (s (comp_of f)) W1 W3); eauto.
            intros [? G'']. rewrite G''.
            exploit Genv.find_def_find_symbol_inversion; eauto.
            { now destruct match_W1_W3. } }
          exploit agr'; eauto. intros []; subst; eauto.
        ...
        * intros G. specialize (CHECKPUB G).
          (* clear -CHECKPUB. *)
          unfold public_first_order in *.
          intros.
          exploit (symbols_inject3 s δ W1 W3 (init_meminj W1 W3));
            eauto using init_meminj_preserves_globals.
          eapply FIND. intros [bid [findid init]].
          specialize (CHECKPUB id0 bid ofs1 findid).
          clear -CHECKPUB FIND bid findid init READABLE.
          assert (Mem.perm m bid ofs1 Cur Readable) by admit.
          specialize (CHECKPUB H).
          admit.
        * specialize (rs1_rs3' PC); rewrite rs3'_PC, NEXTPC in rs1_rs3'.
          (* TODO: factorize *)
          eapply call_trace_preserved with
            (v := Vptr b' Ptrofs.zero); eauto using delta_zero.
      + subst; simpl in *. econstructor; eauto.
        econstructor; auto.
        erewrite Genv.find_def_find_comp_of_block; eauto.
        reflexivity.
      + destruct (side_eq (s cp') δ) as [e1 | n1].
        * left; split.
          -- econstructor; eauto.
             rewrite invalidate_cross_call_PC, invalidate_call_PC, NEXTPC; simpl; auto.
             eauto using regset_rel_invalidate_cross_call, regset_rel_invalidate_call.
          -- econstructor; eauto.
             rewrite invalidate_cross_call_PC, invalidate_call_PC, NEXTPC0; simpl; eauto.
             rewrite invalidate_cross_call_PC, invalidate_call_PC; simpl; eauto.
             now destruct δ.
        * right; split.
          -- econstructor; eauto.
             rewrite invalidate_cross_call_PC, invalidate_call_PC, NEXTPC. eauto.
             rewrite invalidate_cross_call_PC, invalidate_call_PC. eauto.
             destruct δ, (s cp'); now auto.
          -- econstructor; eauto;
               [rewrite invalidate_cross_call_PC, invalidate_call_PC, NEXTPC0; eauto
               | destruct δ, (s cp'); now eauto |].
             assert (forall rs rs' j__oppδ' m1 m3 sig args cp cp',
                        Val.inject j__oppδ' (rs PC) (rs' PC) ->
                        (* Val.inject j__oppδ' (rs X1) (rs' X1) -> *)
                        (* Val.inject j__oppδ' (rs X2) (rs' X2) -> *)
                        cp <> cp' ->
             ...
             { clear.
               unfold invalidate_cross_call, invalidate_call.

               intros * inj_pc diff call1 call2 r.
               unfold Genv.type_of_call.
               eapply Pos.eqb_neq in diff. rewrite diff.
               destruct (preg_eq r X1); [subst; simpl; auto|].
               destruct (preg_eq r X2); [subst; simpl; auto|].
               destruct (preg_eq r PC); [subst; simpl; auto|].
               destruct in_dec; simpl; auto.
               unfold LTL.parameters_mregs in *.
               unfold call_arguments in *.
               clear n1 n n0.
               revert args call1 call2 r i. clear.
               remember (loc_parameters sig) as ls. clear Heqls.
               Local Opaque all_mregs.
               induction ls.
               - simpl. now auto.
               - intros [| arg args] H1 H2 r H; try now inv H1.
                 inv H1. inv H2.
                 specialize (IHls args H7 H8 r).
                 simpl in H. admit. }
             simpl.
             assert (has_comp_function f0 = has_comp_function f) as -> by admit.
             eapply H11.
             admit. (* because PC points to same function *)
             simpl in side_f; auto.
             (* admit. (* RA: because the caller was the same, they point to the *)
             (*           same function and as such must be related *) *)
             (* admit. (* SP: *) *)
             eapply ARGS0. admit.
    ...
    - (* Return *)
      ...
      assert (update_stack_return ge3 (frame3 :: st3')
                ...
                Some st3').
      { unfold update_stack_return.
        eapply (agrees_with_init_meminj_find_comp_preserved s W1 W3 _ _ _ (rs PC)) in NEXTCOMP;
          eauto.
        rewrite NEXTCOMP.
      ...
      { inv frame_rel; simpl; eauto.
        ...
        simpl.  specialize (rs_rs3 PC). intros G; rewrite G in rs_rs3; inv rs_rs3.
        rewrite H12. admit. (* use H3 *)
      ...
      assert (Genv.find_comp ge3 (rs3 PC) = Some cp').
      {
        eapply (agrees_with_init_meminj_find_comp_preserved s W1 W3 _ _ _ (rs PC)) in NEXTCOMP;
          eauto. }
      exists (State st3' (invalidate_cross_return (invalidate_return rs3 (sig_of_call (frame3 :: st3'))) cp' rec_cp) m3); exists j__δ, j__oppδ; split; [| split; [| split; [| split; [| split; [| split]]]]].
      + econstructor; [| now eapply star_refl | now traceEq].
        econstructor; eauto.
        admit.
      + eauto.
      ...
      + (* simpl in *. *)
        ...
        * left; split.
          -- econstructor; eauto.
             rewrite invalidate_cross_return_PC, invalidate_return_PC. eauto.
             eapply regset_rel_invalidate_cross_return; eauto.
             assert (sig_of_call (frame1 :: st') = sig_of_call (frame3 :: st3')) as <-.
             { inv frame_rel; auto. }
        ...
        * right; split.
          ...
          -- econstructor; eauto.
             ...
             now destruct δ, (s cp').
             eapply regset_rel_invalidate_cross_return; eauto.
             assert (sig_of_call (frame2 :: st'0) = sig_of_call (frame3 :: st3')) as <-.
             { inv frame_rel; auto. }
             eapply regset_rel_invalidate_return; eauto.
             admit.
...
Section Simulation.
  ...
  Proof.
    apply threeway_simulation_diagram
      with (metadata := (meminj * meminj)%type)
           ...
           (strong_equivalence1 := fun '(j__left, j__right) s1 s3 =>
                                      agrees_with j__left (init_meminj W1 W3) /\
                                      def_on_addressable s ge1 j__left Left /\
                                         strong_equivalence s ge1 ge3 j__left Left s1 s3)
           (strong_equivalence2 := fun '(j__left, j__right) s2 s3 =>
                                      agrees_with j__right (init_meminj W2 W3) /\
                                      def_on_addressable s ge2 j__right Right /\
    ...
    - exploit (globals_symbols_inject s Left W1 W3);
        eauto using init_meminj_preserves_globals.
      exploit (globals_symbols_inject s Right W2 W3);
        eauto using init_meminj_preserves_globals.
      intros [? _] [? ]. eauto.
    - exploit (globals_symbols_inject s Left W1 W3);
        eauto using init_meminj_preserves_globals.
    ...
    - intros s1 ini1 s2 ini2.
      assert (exists m3,
                   Genv.init_mem W3 = Some m3) as [? ?].
        { inv ini1; inv ini2; subst ge ge0.
          unfold Genv.init_mem in *. clear -H H0 match_W1_W3 match_W2_W3.
          revert m0 H m1 H0. admit.
        }

      eexists; eexists (init_meminj W1 W3, init_meminj W2 W3). split.
      +         econstructor; eauto.
      + inv ini1; inv ini2.
        destruct (comp_of_main W1) as [cp |] eqn:ini_comp; [| admit].
        ...
        * eapply match_states_left; simpl; eauto.
          ...
          -- split; eauto.
             ...
             split; eauto.
             eapply def_on_addressable_init; eauto.
             { econstructor.
               - admit.
               - eauto.
               - subst rs0. intros ?. admit.
               - admit. }
          -- split; eauto.
             econstructor; eauto. congruence. congruence.
             split; eauto.
    ...
    - intros s1 s1' H s2 s3 [j__left j__right] (? & ? & ?) (? & ? & ?) ?.
      ...
      intros (s3' & j__left' & ? & ? & ? & ? & ? & ?).
      exists s3'; eexists (j__left', j__right); split; [assumption |].
      split; [| split]; [| | assumption]; eauto.
    - intros s1 s1' H s2 s3 [j__left j__right] (? & ? & ?) (? & ? & ?) ?.
    ...
    - intros s1 s1' H s2 s3 (j__left & j__right) (? & ? & ?) (? & ? & ?) ?.
      ...
      split. split; [| split]; eauto.
      apply stack_rel_comm in S; simpl in S; eauto.
    - intros s1 s1' H s2 s3 (j__left & j__right) (? & ? & ?) (? & ? & ?) ?.
      exploit (step_E0_weak s W1 W2 W3 Left); eauto using stack_rel_comm.
      intros (j__right' & ? & ? & S & ? & ?).
      exists (j__left, j__right').
      split. split; [| split]; eauto.
      split. split; [| split]; eauto.
