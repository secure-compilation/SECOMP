...
Section INVS.
  ...
  Definition wf_stackframe (ge: Asm.genv) (fr: stackframe) :=
    match fr with
    | Stackframe b _ _ _ _ _ _  => match Genv.find_funct_ptr ge b with
  ...
  | match_stack_cons
    ...
    :
    match_stack ge (ir_cont next :: ik_tl) (Stackframe b sg v ofs ofs1 b1 b2 :: sk_tl).
    ...
    { assert (WFASM': wf_asm ge (State st rs' m' cur_comp)).
      ...
      { clear IH. split. auto. split. auto. split. auto. split.
        { unfold match_cur_regset in *. rewrite NEXTPC. rewrite <- ALLOWED. rewrite MTST1.
          unfold Genv.find_comp_in_genv. rewrite H0.
          admit.
          (* unfold Genv.find_comp. rewrite Genv.find_funct_find_funct_ptr. *)
          (* rewrite H1. auto. *)
        }
        split. auto.
        { unfold match_mem. splits; auto.
...
(*      the trace can start with Event_syscall, without a preceding Event_call *)
  ...
  Proof.
    ...
    - (** internal *)
      ...
      (* NOTE: not the right [cp]? *) admit. admit.
