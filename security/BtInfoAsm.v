...
Section FROMASM.
  ...
  Definition public_not_freeable ge m := forall b, (meminj_public ge b <> None) -> (~ Mem.perm m b 0 Max Freeable).
    ...
    { destruct (Z.leb_spec sz 0); cycle 1.
      { match goal with
        ...
        - apply Forall_app; split; auto. constructor; auto. simpl. destruct (meminj_public ge b) eqn:INJPUB; auto. exfalso.
          eapply Mem.free_range_perm in Heqo0. unfold Mem.range_perm in Heqo0. eapply NFREE. erewrite INJPUB. congruence. eapply Mem.perm_cur_max; apply Heqo0. lia.
  ...
  Proof.
    ...
    all: try
           (match goal with
            ...
            | H: context [Mem.free] |- _ => idtac
            | H: Mem.store ?ch ?m ?b ?ofs ?v ?cp = _ |-  _ =>
                unfold public_not_freeable in *; intros b' H' CC; specialize (NFREE _ H'); apply NFREE; eapply Mem.perm_store_2; eauto
            end).
    { unfold public_not_freeable in *; intros b' H' CC; specialize (NFREE _ H'); apply NFREE. eapply Mem.perm_alloc_4; eauto.
      eapply Mem.perm_store_2; eauto. intros EQ; subst b'. apply H'. apply NALLOC. erewrite Mem.alloc_result; eauto. lia.
    }
    { unfold public_not_freeable in *; intros b' H' CC; specialize (NFREE _ H'); apply NFREE. eapply Mem.perm_free_3; eauto. }
  Qed.
...
Section INVS.
  ...
  | match_stack_cons
      ...
      (SIG: match_cur_stack next ge sk_tl)
...
Section PROOF.
  ...
  Lemma asm_to_ir_returnstate_nccc_internal
        ...
        (MTST0 : match_cur_stack cur ge st)
  ...
  Definition extcall_known (ef: external_function) (ge: Senv.t) args :=
    match ef with
    | EF_builtin _ name sg | EF_runtime _ name sg =>
                               match Builtins.lookup_builtin_function name sg with
                               | Some _ => True
                               | None => False
                               end
    | EF_memcpy _ _ _ => EF_memcpy_dest_not_pub ge args
    | EF_external _ _ sg | EF_inline_asm _ _ sg _ => False
    | _ => True
    end.

  Lemma extcall_cases
        ef ge m args
        (VIS: visible_fo_if_unknown ef ge m args)
    :
    (visible_fo_and_unknown ef ge m args) \/ (extcall_known ef ge args).
  Proof. destruct ef; simpl in *; auto. des_ifs; auto. des_ifs; auto. Qed.

  Lemma asm_to_ir_step_nccc_external
        cpm (ge: genv)
        (WFGE: wf_ge ge)
        cur ik
        (WFIR0 : wf_ir_cur ge cur)
  ...
  Proof.
    ...
    apply extcall_cases in VISFO. destruct VISFO as [VISFO | KNOWN].
    ...
    - (* extcall is unknown *)
      ...
      { unfold match_mem. simpl. split; auto. split; auto. split.
        ...
        { pose proof (meminj_not_alloc_delta _ _ MEM2 _ _ MEM5) as NALLOC.
          ...
          specialize (MEM3 _ H). intros CC. apply MEM3; clear MEM3.
  ...
  Proof.
    ...
    { unfold Genv.type_of_call in NCCC. des_ifs. unfold update_stack_return in STUPD. rewrite Pos.eqb_sym, Heq in STUPD. inv STUPD. auto. }
    subst st'.
    exploit asm_to_ir_step_nccc_external.
  ...
  Proof.
    ...
    - (** internal *)
      ...
      { move STAR after NEXTF. inv STAR.
        ...
        exploit asm_to_ir_returnstate0. 2: eapply IH. 11: eapply H. 11: eapply H6. all: auto. 1,2,3: eauto. all: auto.
