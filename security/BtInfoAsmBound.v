...
Section AXIOM.
  ...
  Definition extcall_observable (sem: extcall_sem) (cp: compartment) (sg: signature): Prop :=
    forall ge vargs m1 t vres m2,
      sem ge cp vargs m1 t vres m2 -> t <> E0.

  Definition external_functions_observable :=
    forall id cp sg, extcall_observable (external_functions_sem id sg) cp sg.

  Definition inline_assembly_observable :=
    forall cp id sg, extcall_observable (inline_assembly_sem id sg) cp sg.
...
Section BOUND.
  ...
  Lemma external_call_observable_length
        ef ge cp vargs m1 tr vretv m2
        (EC: external_call ef ge cp vargs m1 tr vretv m2)
        (ECCASES : external_call_unknowns ef ge m1 vargs \/
                     external_call_known_observables ef ge cp m1 vargs tr vretv m2)
    :
    (1 <= length tr)%nat.
  Proof.
    des.
