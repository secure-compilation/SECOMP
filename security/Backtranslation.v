...
Section CODEAUX.
  ...
  (* Faithful C types for the results of external functions.  Cshmgen accepts
     a declaration [External ef targs tres cc] only if
     [signature_of_type targs tres cc = ef_sig ef] (SECOMP issue #13), so
     [rettype_of_type (rettype_to_type_ext rt)] must be [rt], which holds for
     every [rt] except [Tret Tany32] and [Tret Tany64]: [void] must stay
     [void] and small integers must keep their size and signedness.
     ([rettype_to_type] is kept for internal functions, whose return values
     must be passed through unchanged.)  Arguments need no such variant:
     [typ_to_type] already round-trips for every [typ] except
     [Tany32]/[Tany64], which no C declaration produces. *)
  Definition rettype_to_type_ext: rettype -> type :=
    fun rt: rettype =>
      match rt with
  ...
  Lemma proj_rettype_to_type_ext_rettype_of_type_eq
        ge evres rt res
        (EVM: eventval_match ge evres (proj_rettype rt) res)
    :
    proj_rettype (rettype_of_type (rettype_to_type_ext rt)) = proj_rettype rt.
  Proof.
    inv EVM; destruct rt; simpl; auto.
    all: try (destruct t; simpl in *; auto; try congruence).
    all: unfold Tptr in *; destruct Archi.ptr64 eqn:ARCH; simpl in *; try congruence.
    all: destruct t; simpl in *; auto; try congruence.
  Qed.
  ...
  (* For internal functions: the widest C types, so that argument casts at
     cross-compartment calls and the cast at return are the identity. *)
  ...
  (* For external functions: the faithful types, see [rettype_to_type_ext]. *)
  Definition from_sig_fun_data_ext (sig: signature): fun_data :=
    mkfundata (list_typ_to_typelist sig.(sig_args)) (rettype_to_type_ext sig.(sig_res)) (sig.(sig_cc)).
