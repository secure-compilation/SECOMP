...
Then, you can inspect the content of the generated assembly in `ccs1.s`, but
these files no longer include the compartment information. Instead, one can use
the `-interp-asm` flag to both compile a file like `ccs1.c` and run it in a
prototype interpreter for ASM programs with compartments:

    [compartments]$ ../../ccomp -interp-asm ccs1.c

There is still an known issue with `printf` displaying `<missing argument>` in
this ASM interpreter (#5).

For debugging purposes one can also use a highly experimental printer for the
ASM programs with compartments generated by SECOMP:

    [compartments]$ make ccs1.s.intf

This passes the `-dasm` flag to the compiler in order to generate a `.s.intf`
file, which includes information about the compartment interfaces, but which is
at the moment not easy to read and shows `<instr>` for some instructions (#6).
