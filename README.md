...
Then, you can inspect the content of the generated assembly in `ccs1.s`, which
includes the compartment information in comments.

Alternatively, one can use the `-interp-asm` flag to both compile a file like
`ccs1.c` and run it in a prototype interpreter for ASM programs with compartments:
