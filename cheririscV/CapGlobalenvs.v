...
Variable globdef_size: (globdef F V) -> nat.

Context {CF: has_comp F}.

(** The type of global environments. *)

Definition cap_to_pos_lo (ptr: occap) : positive :=
  Z.to_pos (Ptrofs.unsigned (get_lo ptr)).
...
Record t: Type := mkgenv {
  ...
  genv_symb_range: forall id c, PTree.get id genv_symb = Some c -> Ptrofs.lt (get_lo c) genv_next = true;
  genv_defs_range: forall ptr g, PTree.get (cap_to_pos_lo ptr) genv_defs = Some g -> Ptrofs.lt (get_lo ptr) genv_next = true;
...
Definition find_def (ge: t) (ptr: occap) : option (globdef F V) :=
  PTree.get (cap_to_pos_lo ptr) ge.(genv_defs).
...
Definition find_funct_size (ge: t) (lo: positive) : nat :=
...
Definition find_funct (ge: t) (v: ocval) : option F :=
  match v with
  | OCVcap c =>
      if Ptrofs.eq_dec (get_lo c) (get_addr c) &&
