...
Require Export CapConventions1.
...
Definition max_outgoing_1 (accu: Z) (l: caploc) : Z :=
  match l with
  | S Outgoing ofs ty => Z.max accu (ofs + CapAST.typesize ty)
  | _ => accu
  end.

Definition max_outgoing_2 (accu: Z) (rl: rpair caploc) : Z :=
  match rl with
  | One l => max_outgoing_1 accu l
  | Twolong l1 l2 => max_outgoing_1 (max_outgoing_1 accu l1) l2
  end.

Definition size_arguments (s: capsignature) : Z :=
...
(*   forall (s: capsignature) (ofs: Z) (ty: captyp), *)
(*   In (S Outgoing ofs ty) (regs_of_rpairs (loc_arguments s)) -> *)
(*   ofs + CapAST.typesize ty <= size_arguments s. *)
(* Proof. *)
(*   intros until ty. *)
...
Definition parameter_of_argument (l: caploc) : caploc :=
  match l with
  | S Outgoing n ty => S Incoming n ty
  | _ => l
  end.

Definition loc_parameters (s: capsignature) : list (rpair caploc) :=
...
Definition callee_save_loc (l: caploc) :=
  match l with
