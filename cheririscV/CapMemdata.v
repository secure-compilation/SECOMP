...
Definition size_chunk (chunk: cap_memory_chunk) : Z :=
  match chunk with
  | CMint8signed => 1
  | CMint8unsigned => 1
  | CMint16signed => 2
...
Definition size_chunk_nat (chunk: cap_memory_chunk) : nat :=
...
Lemma size_chunk_Mptr: size_chunk CMptr = if Archi.ptr64 then 8 else 4.
Proof.
  unfold CMptr; destruct Archi.ptr64; auto.
Qed.
...
(* TODO: alignment constraints in CHERI? *)


Definition align_chunk (chunk: cap_memory_chunk) : Z :=
  match chunk with
  | CMint8signed => 1
  | CMint8unsigned => 1
  | CMint16signed => 2
...
Lemma align_chunk_Mptr: align_chunk CMptr = if Archi.ptr64 then 8 else 4.
Proof.
  unfold CMptr; destruct Archi.ptr64; auto.
Qed.
Lemma align_chunk_Mcap: align_chunk CMcap = if Archi.ptr64 then 16 else 8.
Proof.
  unfold CMcap; destruct Archi.ptr64; auto.
Qed.

Lemma align_size_chunk_divides:
  forall chunk, (align_chunk chunk | size_chunk chunk).
Proof.
  intros. destruct chunk; simpl; try apply Z.divide_refl;[exists 2; auto..|exists 4;auto].
Qed.
...
Inductive quantity : Type := Q32 | Q64 | Q128.
...
Definition size_quantity_nat (q: quantity) :=
  match q with Q32 => 4%nat | Q64 => 8%nat | Q128 => 16%nat end.

Lemma size_quantity_nat_pos:
  forall q, exists n, size_quantity_nat q = S n.
Proof.
  intros. destruct q; [exists 3%nat | exists 7%nat | exists 15%nat ]; auto.
Qed.
...
Inductive memval: Type :=
  | Undef: memval
  | Byte: byte -> memval
  | Fragment: ocval -> quantity -> nat -> memval.
...
Fixpoint inj_value_rec (n: nat) (v: ocval) (q: quantity) {struct n}: list memval :=
  match n with
  | 0%nat => nil
  | S m => Fragment v q m :: inj_value_rec m v q
  end.

Definition inj_value (q: quantity) (v: ocval): list memval :=
  inj_value_rec (size_quantity_nat q) v q.

Fixpoint check_value (n: nat) (v: ocval) (q: quantity) (vl: list memval)
                       {struct n} : bool :=
  match n, vl with
  | 0%nat, nil => true
...
Definition proj_value (q: quantity) (vl: list memval) : ocval :=
  match vl with
  | Fragment v q' n :: vl' =>
      if check_value (size_quantity_nat q) v q vl then v else OCVundef
  | _ => OCVundef
  end.

Definition encode_val (chunk: cap_memory_chunk) (v: ocval) : list memval :=
  match v, chunk with
  ...
  | OCVptr ofs, CMint32 => if Archi.ptr64 then List.repeat Undef 4%nat else inj_value Q32 v
  | OCVlong n, CMint64 => inj_bytes (encode_int 8%nat (Int64.unsigned n))
  | OCVptr ofs, CMint64 => if Archi.ptr64 then inj_value Q64 v else List.repeat Undef 8%nat
  ...
  | OCVcap c, CMcap64 => if Archi.ptr64 then List.repeat Undef 8%nat else inj_value Q64 v
  | OCVcap c, CMcap128 => if Archi.ptr64 then inj_value Q128 v else List.repeat Undef 16%nat
  | _, CMany32 => inj_value Q32 v
  | _, CMany64 => inj_value Q64 v
  | _, CMany128 => inj_value Q128 v
  | _, _ => List.repeat Undef (size_chunk_nat chunk)%nat
  end.

(* TODO: decoding a capability -> fix (easy) *)
Definition decode_val (chunk: cap_memory_chunk) (vl: list memval) : ocval :=
  match proj_bytes vl with
  | Some bl =>
      match chunk with
      | CMint8signed => OCVint(Int.sign_ext 8 (Int.repr (decode_int bl)))
      | CMint8unsigned => OCVint(Int.zero_ext 8 (Int.repr (decode_int bl)))
      | CMint16signed => OCVint(Int.sign_ext 16 (Int.repr (decode_int bl)))
      | CMint16unsigned => OCVint(Int.zero_ext 16 (Int.repr (decode_int bl)))
      | CMint32 => OCVint(Int.repr(decode_int bl))
      | CMint64 => OCVlong(Int64.repr(decode_int bl))
      | CMfloat32 => OCVsingle(Float32.of_bits (Int.repr (decode_int bl)))
      | CMfloat64 => OCVfloat(Float.of_bits (Int64.repr (decode_int bl)))
      | CMcap64 => OCVundef
      | CMcap128 => OCVundef
      | CMany32 => OCVundef
      | CMany64 => OCVundef
  ...
  | None =>
      match chunk with
      | CMint32 => if Archi.ptr64 then OCVundef else Val.load_result chunk (proj_value Q32 vl)
      | CMany32 => Val.load_result chunk (proj_value Q32 vl)
      | CMint64 => if Archi.ptr64 then Val.load_result chunk (proj_value Q64 vl) else OCVundef
      | CMany64 => Val.load_result chunk (proj_value Q64 vl)
      | CMcap64 => if Archi.ptr64 then OCVundef else Val.load_result chunk (proj_value Q64 vl)
      | CMcap128 => if Archi.ptr64 then Val.load_result chunk (proj_value Q128 vl) else OCVundef
      | CMany128 => Val.load_result chunk (proj_value Q128 vl)
      | _ => OCVundef
      end
  end.
...
Lemma proj_inj_value_mismatch:
  forall q1 q2 v, q1 <> q2 -> proj_value q1 (inj_value q2 v) = OCVundef.
Proof.
  intros. unfold proj_value. destruct (inj_value q2 v) eqn:V. auto. destruct m; auto.
  destruct (in_inj_value (Fragment o q n) v q2) as [n' EQ].
...
Definition decode_encode_val (v1: ocval) (chunk1 chunk2: cap_memory_chunk) (v2: ocval) : Prop :=
  match v1, chunk1, chunk2 with
  | OCVundef, _, _ => v2 = OCVundef
  | OCVint n, CMint8signed, CMint8signed => v2 = OCVint(Int.sign_ext 8 n)
  | OCVint n, CMint8unsigned, CMint8signed => v2 = OCVint(Int.sign_ext 8 n)
  | OCVint n, CMint8signed, CMint8unsigned => v2 = OCVint(Int.zero_ext 8 n)
  | OCVint n, CMint8unsigned, CMint8unsigned => v2 = OCVint(Int.zero_ext 8 n)
  | OCVint n, CMint16signed, CMint16signed => v2 = OCVint(Int.sign_ext 16 n)
  | OCVint n, CMint16unsigned, CMint16signed => v2 = OCVint(Int.sign_ext 16 n)
  | OCVint n, CMint16signed, CMint16unsigned => v2 = OCVint(Int.zero_ext 16 n)
  | OCVint n, CMint16unsigned, CMint16unsigned => v2 = OCVint(Int.zero_ext 16 n)
  | OCVint n, CMint32, CMint32 => v2 = OCVint n
  | OCVint n, CMany32, CMany32 => v2 = OCVint n
  | OCVint n, CMint32, CMfloat32 => v2 = OCVsingle(Float32.of_bits n)
  | OCVint n, CMany64, CMany64 => v2 = OCVint n
  | OCVint n, CMany128, CMany128 => v2 = OCVint n
  ...
  | OCVcap c, CMany128, CMany128 => v2 = OCVcap c
  | OCVcap c, CMany128, CMcap128 => v2 = if Archi.ptr64 then OCVcap c else OCVundef
  | OCVcap c, _,_ => v2 = OCVundef

  | OCVlong n, CMint64, CMint64 => v2 = OCVlong n
  | OCVlong n, CMint64, CMfloat64 => v2 = OCVfloat(Float.of_bits n)
  | OCVlong n, CMany64, CMany64 => v2 = OCVlong n
  | OCVlong n, CMany128, CMany128 => v2 = OCVlong n
  | OCVlong n, (CMint8signed|CMint8unsigned|CMint16signed|CMint16unsigned|CMint32|CMfloat32|CMfloat64|CMany32|CMcap64|CMcap128), _ => v2 = OCVundef
  | OCVlong n, _, _ => True (**r nothing meaningful to say about v2 *)
                      
  | OCVfloat f, CMfloat64, CMfloat64 => v2 = OCVfloat f
  | OCVfloat f, CMfloat64, CMint64 => v2 = OCVlong(Float.to_bits f)
  | OCVfloat f, CMany64, CMany64 => v2 = OCVfloat f
  | OCVfloat f, CMany128, CMany128 => v2 = OCVfloat f
  | OCVfloat f, (CMint8signed|CMint8unsigned|CMint16signed|CMint16unsigned|CMint32|CMfloat32|CMint64|CMany32|CMcap64|CMcap128), _ => v2 = OCVundef
  | OCVfloat f, _, _ => True   (* nothing interesting to say about v2 *)

  | OCVsingle f, CMfloat32, CMfloat32 => v2 = OCVsingle f
  | OCVsingle f, CMfloat32, CMint32 => v2 = OCVint(Float32.to_bits f)
  | OCVsingle f, CMany32, CMany32 => v2 = OCVsingle f
  | OCVsingle f, CMany64, CMany64 => v2 = OCVsingle f
  | OCVsingle f, CMany128, CMany128 => v2 = OCVsingle f
  | OCVsingle f, (CMint8signed|CMint8unsigned|CMint16signed|CMint16unsigned|CMint32|CMint64|CMfloat64|CMany64|CMcap64|CMcap128), _ => v2 = OCVundef
  | OCVsingle f, _, _ => True (* nothing interesting to say about v2 *)
  end.

Remark decode_val_undef:
  forall bl chunk, decode_val chunk (Undef :: bl) = OCVundef.
Proof.
...
Ltac solve_decode_encode_val_general :=
  ...
  | |- OCVint (Int.sign_ext _ (Int.sign_ext _ _)) = OCVint _ => f_equal; apply Int.sign_ext_idem; lia
  | |- OCVint (Int.zero_ext _ (Int.zero_ext _ _)) = OCVint _ => f_equal; apply Int.zero_ext_idem; lia
  | |- OCVint (Int.sign_ext _ (Int.zero_ext _ _)) = OCVint _ => f_equal; apply Int.sign_ext_zero_ext; lia
  end.
...
Proof.
  ...
  destruct chunk1; destruct chunk2; simpl in TY; try discriminate; simpl in SZ; try lia;
  destruct v1; auto.
Qed.
...
Lemma encode_val_int8_signed_unsigned:
  forall v, encode_val CMint8signed v = encode_val CMint8unsigned v.
Proof.
  intros. destruct v; simpl; auto.
Qed.

Lemma encode_val_int16_signed_unsigned:
  forall v, encode_val CMint16signed v = encode_val CMint16unsigned v.
Proof.
  intros. destruct v; simpl; auto.
Qed.

Lemma encode_val_int8_zero_ext:
  forall n, encode_val CMint8unsigned (OCVint (Int.zero_ext 8 n)) = encode_val CMint8unsigned (OCVint n).
Proof.
...
Lemma encode_val_int8_sign_ext:
  forall n, encode_val CMint8signed (OCVint (Int.sign_ext 8 n)) = encode_val CMint8signed (OCVint n).
Proof.
...
Lemma encode_val_int16_zero_ext:
  forall n, encode_val CMint16unsigned (OCVint (Int.zero_ext 16 n)) = encode_val CMint16unsigned (OCVint n).
Proof.
...
Lemma encode_val_int16_sign_ext:
  forall n, encode_val CMint16signed (OCVint (Int.sign_ext 16 n)) = encode_val CMint16signed (OCVint n).
Proof.
...
Definition quantity_chunk (chunk: cap_memory_chunk) :=
  match chunk with
  | CMint64 | CMfloat64 | CMany64 | CMcap64 => Q64
  | CMcap128 | CMany128 => Q128
  | _ => Q32
  end.

Inductive shape_encoding (chunk: cap_memory_chunk) (v: ocval): list memval -> Prop :=
  | shape_encoding_f: forall q i mvl,
      (chunk = CMint32 \/ chunk = CMany32 \/ chunk = CMint64 \/ chunk = CMany64 \/ chunk = CMany128 \/ chunk = CMcap64 \/ chunk = CMcap128) ->
  ...
  | shape_encoding_b: forall b mvl,
      match v with OCVint _ => True | OCVlong _ => True | OCVfloat _ => True | OCVsingle _ => True | _ => False end ->
...
Local Transparent inj_value.
  ...
  (apply B || apply C || apply D || (destruct Archi.ptr64; (apply B || apply D)));auto.
  1,2,3,4,5,6,9: do 4 right; auto.
  do 5 right;auto. do 6 right;auto.
Qed.

Inductive shape_decoding (chunk: cap_memory_chunk): list memval -> ocval -> Prop :=
  ...
  | shape_decoding_u: forall mvl,
      shape_decoding chunk mvl OCVundef.
...
Proof.
  ...
  assert (U: forall mvs, shape_decoding chunk mvs (Val.load_result chunk OCVundef)).
  {
    intros. replace (Val.load_result chunk OCVundef) with OCVundef. constructor.
    destruct chunk; auto.
  }
             ...
             shape_decoding chunk (mv1 :: mvl) (Val.load_result chunk (proj_value q (mv1 :: mvl)))).
  {
    ...
    destruct (check_value sz o q mvl) eqn:CHECK; auto.
    simpl. apply beq_nat_true in EQN. subst n q0. constructor; auto.
    destruct H0 as [E|[E|[E|[E|[E|[E|E]]]]]]; subst chunk; destruct q; auto || discriminate.
...
Lemma repeat_Undef_inject_any:
  forall f vl,
  list_forall2 (memval_inject f) (List.repeat (length vl) Undef) vl.
Proof.
...
Lemma repeat_Undef_inject_encode_val:
  forall f chunk v,
  list_forall2 (memval_inject f) (List.repeat (size_chunk_nat chunk) Undef) (encode_val chunk v).
Proof.
...
Lemma repeat_Undef_inject_self:
  forall f n,
  list_forall2 (memval_inject f) (List.repeat n Undef) (List.repeat n Undef).
Proof.
...
Lemma decode_val_int64:
  ...
  Val.lessdef
    (decode_val CMint64 (l1 ++ l2))
    (Val.longofwords (decode_val CMint32 (if Archi.big_endian then l1 else l2))
                     (decode_val CMint32 (if Archi.big_endian then l2 else l1))).
Proof.
...
Lemma encode_val_int64:
  forall v,
  Archi.ptr64 = false ->
  encode_val CMint64 v =
     encode_val CMint32 (if Archi.big_endian then Val.hiword v else Val.loword v)
  ++ encode_val CMint32 (if Archi.big_endian then Val.loword v else Val.hiword v).
Proof.
...
(* TODO: breaking a 128 access into four 32 accesses, except for capabilities? look into how CHERI loads capabilities, and if it can be possible to break it up (if yes: the tag comes last *)
